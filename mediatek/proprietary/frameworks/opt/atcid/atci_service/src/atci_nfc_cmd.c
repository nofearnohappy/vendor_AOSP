    ...
    while (count < 800)
    {
        ...
        if (rec_bytes > 0)
        {
            ...
            else if ((pnfc_msg_ptr->msg_type == MTK_NFC_FM_READ_DEP_TEST_RSP) && (type == MTK_NFC_FM_READ_DEP_TEST_RSP))
            {
                ...
                pRspData = (s_mtk_nfc_em_als_readerm_opt_rsp*) ((unsigned char*) pnfc_msg_ptr
                        + sizeof(s_mtk_nfc_main_msg));
            ...
            else if ((pnfc_msg_ptr->msg_type == MTK_NFC_FM_VIRTUAL_CARD_RSP) && (type == MTK_NFC_FM_VIRTUAL_CARD_RSP))
            {
                ...
                pRspData =
                        (s_mtk_nfc_em_virtual_card_rsp*) ((unsigned char*) pnfc_msg_ptr + sizeof(s_mtk_nfc_main_msg));
    ...
    switch (type)
    {
        ...
        case MTK_NFC_FM_READ_DEP_TEST_REQ:
        {
            ...
            if (pReqData != NULL)
            {
                ...
                pReqData->typeA_datarate = (EM_ALS_READER_M_SPDRATE_106 + EM_ALS_READER_M_SPDRATE_212
...
void ATCMD_NFC_Read_CNF(void)
{
    ...
    while (retry_count < 5)
    {
        ...
        {
            // check msg id
            ALOGI("ATCMD_NFC_Read_CNF:NFC read (msg_id,dest_mod_id) = (%d,%d)", nfc_ilm_rec.msg_id,
                    nfc_ilm_rec.dest_mod_id);
            ...
            if ((nfc_ilm_rec.msg_id == MSG_ID_NFC_TEST_RSP) && (nfc_ilm_rec.dest_mod_id == MOD_NFC_APP))
            {
                ...
                switch (nfc_msg.msg_type)
                {
                    ...
                    case MSG_ID_NFC_SWP_SELF_TEST_RSP:
                    {
                        ...
                        ALOGI("AT_NFC_CMD:NFC NFC_OP_SWP_SELF_TEST =%d/", nfc_cnf.result.m_script_cnf.result);
                        break;
                    }
                    case MSG_ID_NFC_ANTENNA_SELF_TEST_RSP:
                    {
                        ...
                        memcpy(&nfc_cnf.result.m_script_cnf,
                                (nfc_script_response*) (nfc_ilm_rec.local_para_ptr + nfc_msg_length),
                                sizeof(nfc_script_response));
                        ALOGI("AT_NFC_CMD:NFC NFC_OP_ANTENNA_SELF_TEST =%d/", nfc_cnf.result.m_script_cnf.result);
                        break;
                    }
                    case MSG_ID_NFC_TAG_UID_RW_RSP:
                    {
                        nfc_cnf.op = NFC_OP_TAG_UID_RW;
                        memcpy(&nfc_cnf.result.m_script_uid_cnf, (nfc_ilm_rec.local_para_ptr + nfc_msg_length),
                                sizeof(nfc_script_uid_response));
                        ALOGI("AT_NFC_CMD:NFC NFC_OP_TAG_UID_RW =%d/%d/%X/%X/%X/%X/",
                                nfc_cnf.result.m_script_uid_cnf.result, nfc_cnf.result.m_script_uid_cnf.uid_type,
                                nfc_cnf.result.m_script_uid_cnf.data[0], nfc_cnf.result.m_script_uid_cnf.data[1],
                                nfc_cnf.result.m_script_uid_cnf.data[2], nfc_cnf.result.m_script_uid_cnf.data[3]);
                        break;
                    }
...
void ATCMD_NFC_OP(NFC_REQ *req)
{
    ...
    case NFC_OP_SWP_SELF_TEST:
    {
        ...
        memcpy(nfc_ilm_loc.local_para_ptr, (char*) &nfc_msg, sizeof(nfc_msg_struct));
        memcpy((nfc_ilm_loc.local_para_ptr + sizeof(nfc_msg_struct)), &req->cmd.m_script_req,
                sizeof(nfc_script_request));
        AT_NFC_CMD(&nfc_ilm_loc);
        ATCMD_NFC_Read_CNF();
        break;
    }
    ...
    case NFC_OP_TAG_UID_RW:
    {
        ...
        memcpy(nfc_ilm_loc.local_para_ptr, (char*) &nfc_msg, sizeof(nfc_msg_struct));
        memcpy((nfc_ilm_loc.local_para_ptr + sizeof(nfc_msg_struct)), &req->cmd.m_script_uid_req,
                sizeof(nfc_script_uid_request));
        AT_NFC_CMD(&nfc_ilm_loc);
        ATCMD_NFC_Read_CNF();
        break;
    }
            ...
            case AT_SET_OP:
            {
                ...
                else if (actinoID == 2)
                {
                    ...
                    if (nfc_cnf.op == NFC_OP_TAG_UID_RW)
                    {
                        ...
                        int UIDLength = 7;
                        int index = 0;
                        snprintf(response, 2048, "NFC=2,%d,%d", nfc_cnf.result.m_script_uid_cnf.result,
