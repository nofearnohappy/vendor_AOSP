...
ATOP_t getAtOp(char** cmdLine) {
    ...
    // Because there are many different customized AT command, the filter logic for
    // '+', '*' and '%' may not contain all type AT command. Remove this filter logic.
    //if (c == AT_EXTEND_CHAR || c == AT_EXTEND_STAR_CHAR || c == AT_EXTEND_PER_CHAR) {
