...
import android.tedongle.SubscriptionInfo;
...
public class SubscriptionController extends ISub.Stub {
    static final String LOG_TAG = "3GD-SubscriptionController";
     ...
     */
     ...
     public void notifySubscriptionInfoChanged() {
         /*ITelephonyRegistry tr = ITelephonyRegistry.Stub.asInterface(ServiceManager.getService(
                 "telephony.registry"));
         try {
             if (DBG) logd("notifySubscriptiyyonInfoChanged:");
    ...
    public int addSubInfoRecord(String iccId, int slotId) {
        ...
        try {
                        ...
                        if (currentSubId == null
                            ...
                            if (DBG) {
                                logdl("[addSubInfoRecord]"
                                        ...
                                        + " defaultSubId=" + defaultSubId + " simCount="
                                        + subIdCountMax);
                            }
            ...
            if (subIds == null || subIds.length == 0) {
                if (DBG) {
                    logdl("[addSubInfoRecord]- getSubId failed subIds == null || length == 0 "
                            + "subIds=" + subIds);
                }
                return -1;
            }
            if (setDisplayName) {
                String simCarrierName =
                            mTelephonyManager.getSimOperatorNameForSubscription(subIds[0]);
                String nameToSet;
