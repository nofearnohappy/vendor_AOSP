...
import android.tedongle.SubscriptionManager.OnSubscriptionsChangedListener;
...
import com.android.internal.tedongle.dataconnection.DcSwitchAsyncChannel.RequestInfo;
...
//    private SubscriptionManager mSubMgr;
    ...
    private void updatePhoneBaseForIndex(int index, PhoneBase phoneBase) {
        ...
        mNetworkFilter[index].addTransportType(NetworkCapabilities.TRANSPORT_TEDONGLE);
        ...
        cm.registerNetworkFactory(mNetworkFactoryMessenger[index], "Tedongle");
    }
...
//        mSubMgr.removeOnSubscriptionsChangedListener(mOnSubscriptionsChangedListener);
    ...
    private void trackNetworkRequest(RequestInfo ri) {
        synchronized (mExecutingLock) {
            ...
            logd("trackNetworkRequest SerialNumber" + ri.executionSerialNumber + ", " + ri);
        ...
        synchronized (mExecutingLock) {
            if (mExecutingNetworkRequestTransactions.remove(serialNum) == null) {
                if (mExecutingNetworkReleaseTransactions.remove(serialNum) == null) {
                    Log.e(LOG_TAG, "ackNetworkExecution can't find request for " +
