...
public class TelephonyManagerEx {
   ...
   public int getSvlteCardType(int slotId) {
    ...
    public ArrayList<EccEntry> getUserCustomizedEccList() {
        ...
        String eccList = null;
        try {
            eccList = getITelephonyEx().getUserCustomizedEccList();
        } catch (RemoteException ex) {
            return result;
        } catch (NullPointerException ex) {
            return result;
        ...
        if (eccList != null) {
            String[] tempArr = eccList.split(";");
            if (tempArr != null) {
                for (String pair : tempArr) {
                    String[] values = pair.split(",");
                    if (values != null && values.length == 2) {
                        EccEntry entry = new EccEntry(values[0], values[1]);
                        result.add(entry);
                    }
                }
            }
    ...
    public boolean updateUserCustomizedEccList(ArrayList<EccEntry> eccList) {
        StringBuilder eccString = new StringBuilder("");
        if (eccList != null) {
            for (EccEntry entry : eccList) {
                eccString.append(entry.getName()).append(",").append(entry.getEcc())
                        .append(";");
            }
        }
        Rlog.d(TAG, "updateUserCustomizedEccList, eccString:" + eccString.toString());
        try {
            return getITelephonyEx().updateUserCustomizedEccList(eccString.toString());
        } catch (RemoteException ex) {
