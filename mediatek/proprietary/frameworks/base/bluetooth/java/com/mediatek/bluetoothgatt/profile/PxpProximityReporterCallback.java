...
/**
 * This class is used to implement {@link PxpProximityReporter} callbacks.
 */
public class PxpProximityReporterCallback extends ServerBaseCallback {
    private static final boolean DBG = true;
    private static final String TAG = "PxpProximityReporterCallback";
    ...
    void dispatchCharacteristicReadRequest(BluetoothDevice device, int requestId, int offset,
        ...
        if (srvcUuid.equals(GattUuid.SRVC_LLS)) {
             if (charUuid.equals(GattUuid.CHAR_ALERT_LEVEL)) {
                ...
                this.onLlsAlertLevelReadRequest(
                        device, requestId, offset, (AlertLevel) base);
                return;
            }
        }
        if (srvcUuid.equals(GattUuid.SRVC_TPS)) {
             if (charUuid.equals(GattUuid.CHAR_TX_POWER_LEVEL)) {
                base = new TxPowerLevel(characteristic.getValue(), characteristic);
                this.onTpsTxPowerLevelReadRequest(
                        device, requestId, offset, (TxPowerLevel) base);
                return;
            }
        }
    ...
    void dispatchCharacteristicWriteRequest(BluetoothDevice device, int requestId,
        ...
        if (srvcUuid.equals(GattUuid.SRVC_LLS)) {
             if (charUuid.equals(GattUuid.CHAR_ALERT_LEVEL)) {
                base = new AlertLevel(characteristic.getValue(), characteristic);
                this.onLlsAlertLevelWriteRequest(
    ...
    /**
     * A remote client has requested to read Lls:AlertLevel
     * local characteristic.
     *
     * @param device The remote device that has requested the read operation
     * @param requestId The Id of the request
     * @param offset Offset into the value of the characteristic
     * @param alertLevel Characteristic to be read
     */
    public void onLlsAlertLevelReadRequest(
            BluetoothDevice device, int requestId, int offset,
            AlertLevel alertLevel) {
        if (DBG) {
            Log.d(TAG, "onLlsAlertLevelReadRequest(): offset=" + offset);
        }

        sendResponse(device, requestId, offset,
                alertLevel.getValue(offset));
    }

    /**
     * A remote client has requested to read Tps:TxPowerLevel
     * local characteristic.
     *
     * @param device The remote device that has requested the read operation
     * @param requestId The Id of the request
     * @param offset Offset into the value of the characteristic
     * @param txPowerLevel Characteristic to be read
     */
    public void onTpsTxPowerLevelReadRequest(
            BluetoothDevice device, int requestId, int offset,
            TxPowerLevel txPowerLevel) {
        if (DBG) {
            Log.d(TAG, "onTpsTxPowerLevelReadRequest(): offset=" + offset);
        }

        sendResponse(device, requestId, offset,
                txPowerLevel.getValue(offset));
    }


    /**
     * A remote client has requested to write to Lls:AlertLevel
     * local characteristic.
     *
     ...
     */
    public void onLlsAlertLevelWriteRequest(
            BluetoothDevice device, int requestId,
            AlertLevel alertLevel,
            boolean preparedWrite, boolean responseNeeded, int offset, byte[] value) {
        if (DBG) {
            Log.d(TAG, "onLlsAlertLevelWriteRequest()");
        }
