...
public class BluetoothBpp implements BluetoothProfileManager.BluetoothProfileBehavior{
    ...
    public BluetoothBpp(Context context) {
        mContext = context;
        Intent intent = new Intent(IBluetoothBpp.class.getName());
        intent.setClassName("com.mediatek.bluetooth", "com.mediatek.bluetooth"+".bpp.BluetoothBppManager");
        if (!context.bindService(intent, mConnection, Context.BIND_AUTO_CREATE)) {
            Log.e(TAG, "Could not bind to Bluetooth BPP Service");
        }
    }
    ...
    public int getState(BluetoothDevice device){
        if (mService != null) {
            ...
            Log.e (TAG, "getState:mService is null");
            //if(DBG) { Log.d(TAG,Log.getStackTraceString(new Throwable())); }
	}
    ...
    public Set<BluetoothDevice> getConnectedDevices() {
        ...
        HashSet <BluetoothDevice> devices = new HashSet<BluetoothDevice>(); 	
            ...
            Log.e (TAG, "getConnectedDevices:mService is null");
            //if(DBG) { Log.d(TAG,Log.getStackTraceString(new Throwable())); }
	}
        return null;
    }
