...
/**
 * This class is used to implement {@link HrpHeartRateSensor} callbacks.
 */
public class HrpHeartRateSensorCallback extends ServerBaseCallback {
    private static final boolean DBG = true;
    private static final String TAG = "HrpHeartRateSensorCallback";
    ...
    void dispatchCharacteristicReadRequest(BluetoothDevice device, int requestId, int offset,
        ...
        if (srvcUuid.equals(GattUuid.SRVC_HRS)) {
             if (charUuid.equals(GattUuid.CHAR_BODY_SENSOR_LOCATION)) {
                base = new BodySensorLocation(characteristic.getValue(), characteristic);
                this.onHrsBodySensorLocationReadRequest(
                        device, requestId, offset, (BodySensorLocation) base);
                return;
            }
        }
    ...
    void dispatchCharacteristicWriteRequest(BluetoothDevice device, int requestId,
        ...
        if (srvcUuid.equals(GattUuid.SRVC_HRS)) {
             if (charUuid.equals(GattUuid.CHAR_HEART_RATE_CONTROL_POINT)) {
                base = new HeartRateControlPoint(characteristic.getValue(), characteristic);
                this.onHrsHeartRateControlPointWriteRequest(
                        device, requestId, (HeartRateControlPoint) base,
    ...
    void dispatchDescriptorReadRequest(BluetoothDevice device, int requestId, int offset,
        ...
        if (srvcUuid.equals(GattUuid.SRVC_HRS)) {
             if (charUuid.equals(GattUuid.CHAR_HEART_RATE_MEASUREMENT) &&
                ...
                this.onHrsHeartRateMeasurementCccdReadRequest(
    ...
    void dispatchDescriptorWriteRequest(BluetoothDevice device, int requestId,
        ...
        if (srvcUuid.equals(GattUuid.SRVC_HRS)) {
             if (charUuid.equals(GattUuid.CHAR_HEART_RATE_MEASUREMENT) &&
                    descrUuid.equals(GattUuid.DESCR_CLIENT_CHARACTERISTIC_CONFIGURATION)) {
                this.onHrsHeartRateMeasurementCccdWriteRequest(
    ...
    /**
     * A remote client has requested to read Hrs:BodySensorLocation
     * local characteristic.
     *
     * @param device The remote device that has requested the read operation
     * @param requestId The Id of the request
     * @param offset Offset into the value of the characteristic
     * @param bodySensorLocation Characteristic to be read
     */
    public void onHrsBodySensorLocationReadRequest(
            BluetoothDevice device, int requestId, int offset,
            BodySensorLocation bodySensorLocation) {
        if (DBG) {
            Log.d(TAG, "onHrsBodySensorLocationReadRequest(): offset=" + offset);
        }

        sendResponse(device, requestId, offset,
                bodySensorLocation.getValue(offset));
    }
    ...
    /**
     * A remote client has requested to write to Hrs:HeartRateControlPoint
     * local characteristic.
     *
     * @param device The remote device that has requested the write operation
     * @param requestId The Id of the request
     * @param heartRateControlPoint Characteristic to be written to.
     ...
     */
    public void onHrsHeartRateControlPointWriteRequest(
            BluetoothDevice device, int requestId,
            HeartRateControlPoint heartRateControlPoint,
            boolean preparedWrite, boolean responseNeeded, int offset, byte[] value) {
        if (DBG) {
            Log.d(TAG, "onHrsHeartRateControlPointWriteRequest()");
        }

        if (preparedWrite) {
            prepareWrite(device, heartRateControlPoint, offset, value, false);
            if (responseNeeded) {
        ...
        heartRateControlPoint.setValue(offset, value);
        if (responseNeeded) {
    ...
    /**
     * A remote client has requested to read Hrs:HeartRateMeasurement:
     ...
     */
    public void onHrsHeartRateMeasurementCccdReadRequest(
            BluetoothDevice device, int requestId, int offset, BluetoothGattDescriptor descriptor) {
        if (DBG) {
            Log.d(TAG, "onHrsHeartRateMeasurementCccdReadRequest()");
        }
    ...
    /**
     * A remote client has requested to write Hrs:HeartRateMeasurement:
     * Cccd local descriptor.
     *
     * @param device The remote device that has requested the write operation
     * @param requestId The Id of the request
     ...
     */
    public void onHrsHeartRateMeasurementCccdWriteRequest(
            BluetoothDevice device, int requestId, BluetoothGattDescriptor descriptor,
            boolean preparedWrite, boolean responseNeeded, int offset, byte[] value) {
        if (DBG) {
            Log.d(TAG, "onHrsHeartRateMeasurementCccdWriteRequest()");
        }
