...
/**
 * This class is used to implement {@link BlpBloodPressureSensor} callbacks.
 */
public class BlpBloodPressureSensorCallback extends ServerBaseCallback {
    private static final boolean DBG = true;
    private static final String TAG = "BlpBloodPressureSensorCallback";
    ...
    void dispatchCharacteristicReadRequest(BluetoothDevice device, int requestId, int offset,
        ...
        if (srvcUuid.equals(GattUuid.SRVC_BLS)) {
             if (charUuid.equals(GattUuid.CHAR_BLOOD_PRESSURE_FEATURE)) {
                base = new BloodPressureFeature(characteristic.getValue(), characteristic);
                this.onBlsBloodPressureFeatureReadRequest(
                        device, requestId, offset, (BloodPressureFeature) base);
                return;
            }
        }
    ...
    void dispatchDescriptorReadRequest(BluetoothDevice device, int requestId, int offset,
        ...
        if (srvcUuid.equals(GattUuid.SRVC_BLS)) {
             if (charUuid.equals(GattUuid.CHAR_BLOOD_PRESSURE_MEASUREMENT) &&
                ...
                this.onBlsBloodPressureMeasurementCccdReadRequest(
                        device, requestId, offset, descriptor);
                return;
            } else if (charUuid.equals(GattUuid.CHAR_INTERMEDIATE_CUFF_PRESSURE) &&
                    descrUuid.equals(GattUuid.DESCR_CLIENT_CHARACTERISTIC_CONFIGURATION)) {
                this.onBlsIntermediateCuffPressureCccdReadRequest(
                        device, requestId, offset, descriptor);
                return;
            }
        }
    ...
    void dispatchDescriptorWriteRequest(BluetoothDevice device, int requestId,
        ...
        if (srvcUuid.equals(GattUuid.SRVC_BLS)) {
             if (charUuid.equals(GattUuid.CHAR_BLOOD_PRESSURE_MEASUREMENT) &&
                    descrUuid.equals(GattUuid.DESCR_CLIENT_CHARACTERISTIC_CONFIGURATION)) {
                this.onBlsBloodPressureMeasurementCccdWriteRequest(
                        device, requestId, descriptor, preparedWrite, responseNeeded, offset,
                        value);
                return;
            } else if (charUuid.equals(GattUuid.CHAR_INTERMEDIATE_CUFF_PRESSURE) &&
                    descrUuid.equals(GattUuid.DESCR_CLIENT_CHARACTERISTIC_CONFIGURATION)) {
                this.onBlsIntermediateCuffPressureCccdWriteRequest(
                        device, requestId, descriptor, preparedWrite, responseNeeded, offset,
                        value);
                return;
            }
    ...
    /**
     * A remote client has requested to read Bls:BloodPressureFeature
     * local characteristic.
     *
     * @param device The remote device that has requested the read operation
     * @param requestId The Id of the request
     * @param offset Offset into the value of the characteristic
     * @param bloodPressureFeature Characteristic to be read
     */
    public void onBlsBloodPressureFeatureReadRequest(
            BluetoothDevice device, int requestId, int offset,
            BloodPressureFeature bloodPressureFeature) {
        if (DBG) {
            Log.d(TAG, "onBlsBloodPressureFeatureReadRequest(): offset=" + offset);
        }

        sendResponse(device, requestId, offset,
                bloodPressureFeature.getValue(offset));
    }
    ...
    /**
     * A remote client has requested to read Bls:BloodPressureMeasurement:
     ...
     */
    public void onBlsBloodPressureMeasurementCccdReadRequest(
            BluetoothDevice device, int requestId, int offset, BluetoothGattDescriptor descriptor) {
        if (DBG) {
            Log.d(TAG, "onBlsBloodPressureMeasurementCccdReadRequest()");
        }
    ...
    /**
     * A remote client has requested to read Bls:IntermediateCuffPressure:
     * Cccd local descriptor.
     *
     * @param device The remote device that has requested the read operation
     * @param requestId The Id of the request
     ...
     * @param descriptor Descriptor to be read
     */
    public void onBlsIntermediateCuffPressureCccdReadRequest(
            BluetoothDevice device, int requestId, int offset, BluetoothGattDescriptor descriptor) {
        if (DBG) {
            Log.d(TAG, "onBlsIntermediateCuffPressureCccdReadRequest()");
        }
    ...
    /**
     * A remote client has requested to write Bls:BloodPressureMeasurement:
     * Cccd local descriptor.
     *
     ...
     */
    public void onBlsBloodPressureMeasurementCccdWriteRequest(
            BluetoothDevice device, int requestId, BluetoothGattDescriptor descriptor,
            boolean preparedWrite, boolean responseNeeded, int offset, byte[] value) {
        if (DBG) {
            Log.d(TAG, "onBlsBloodPressureMeasurementCccdWriteRequest()");
        }
    ...
    /**
     * A remote client has requested to write Bls:IntermediateCuffPressure:
     * Cccd local descriptor.
     *
     * @param device The remote device that has requested the write operation
     * @param requestId The Id of the request
     ...
     * @param value The value the client wants to assign to the descriptor
     */
    public void onBlsIntermediateCuffPressureCccdWriteRequest(
            BluetoothDevice device, int requestId, BluetoothGattDescriptor descriptor,
            boolean preparedWrite, boolean responseNeeded, int offset, byte[] value) {
        if (DBG) {
            Log.d(TAG, "onBlsIntermediateCuffPressureCccdWriteRequest()");
        }
