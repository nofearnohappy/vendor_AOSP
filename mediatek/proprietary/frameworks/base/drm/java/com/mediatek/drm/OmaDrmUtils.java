...
public class OmaDrmUtils {
    ...
    /**
     * Get image thumbnail.
     * @param drmClient OmaDrmClient instance.
     * @param path which file should be used
     * @param options width,height of this bitmap
     * @return thumbnail of the bitmap
     */
    public static Bitmap getImageThumbnail(OmaDrmClient drmClient, String path, BitmapFactory.Options options) {
        Log.d(TAG, "getImageThumbnail");
        Bitmap result = null;
        if (null == drmClient) {
            Log.e(TAG, "omadrmclient instance is null.");
            return result;
        }
        int width = 96;
        int height = 96;
        ...
        if (drmClient.canHandle(path, null)) {
           // Log.d(TAG, "getImgThumbnail hongen1");
           ...
           // Log.d(TAG, "hongen, get clear img:beH="+beH+",beW="+beW+",be="+be + ",width="+width + ",height="+height);
           // Log.d(TAG, "hongen, get clear img:h="+h+",w="+w+",be="+be);
            opts.inSampleSize = be;
            Bitmap btm = BitmapFactory.decodeByteArray(buffer, 0, buffer.length, opts);
           // Log.d(TAG, "getImgThumbnail hongen , drm : btm = " + btm);
            return btm;
        }
        /*else {
            ...
            //Log.d(TAG, "hongen, get clear img:h="+h+",w="+w+",be="+be);
            opts.inSampleSize = be;
            //Log.d(TAG, "getImgThumbnail hongen , drm : btm = " + path);
            result = BitmapFactory.decodeFile(path, opts);
            if (w>width || h>height) {
                int max = Math.max(w, h);
                float scale = (float) width / max;
                int tw = Math.round(scale * w) ;
                int th = Math.round(scale * h) ;
     ...
     */
    public static Bitmap getVideoThumbnail(OmaDrmClient drmClient, String path, int targetWidth) {
