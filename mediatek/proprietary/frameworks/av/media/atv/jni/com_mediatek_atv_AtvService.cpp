...
sp<Camera> get_native_camera(JNIEnv *env, jobject thiz, JNIAtvContext** pContext)
{
    ...
    if (context != NULL) {
		#ifdef ATV_LOG_OUT
    	ALOGD("context->getCamera in");
		#endif
        camera = context->getCamera();
		#ifdef ATV_LOG_OUT
    	ALOGD("context->getCamera out");
		#endif
    }
    ALOGV("get_native_camera: context=%p, camera=%p", context, camera.get());
    if (camera == 0) {
		ALOGE("get_native_camera: Method called after release()", context, camera.get());
    }  
	#ifdef ATV_LOG_OUT
    ALOGD("get_native_camera out");
	#endif
    return camera;
}
...
void JNIAtvContext::notify(int32_t msgType, int32_t ext1, int32_t ext2)
{
    ...
    Mutex::Autolock _l(mLock);
    if (mAtvJObjectWeak == NULL) {
        ALOGW("callback on dead atv object");
        return;
    }
    JNIEnv *env = AndroidRuntime::getJNIEnv();
...
void JNIAtvContext::copyAndPost(JNIEnv* env, const sp<IMemory>& dataPtr, int msgType)
{
    ...
    if (dataPtr != NULL) {
        ...
        ALOGD("postData: off=%d, size=%d", (int)offset, size);
        uint8_t *heapBase = (uint8_t*)heap->base();

        if (heapBase != NULL) {
            const jbyte* data = reinterpret_cast<const jbyte*>(heapBase + offset);
            obj = env->NewByteArray(size);
            ALOGD("Allocating callback buffer data[10] = %d,obj = %d",data[10],(int)obj);
                          ...
                          camera_frame_metadata_t *metadata)
{
    ...
    ALOGD("dataCallback(%d, %p)", msgType, dataPtr.get());
...
{
    ALOGD("autoscan_progress_chstate_cb");
	...
	if (mAtvJObjectWeak == NULL) {
	    ...
	    jlong packedEntry = 0;
		ALOGD("ch_latest_updated:%d,freq:%d,sndsys:%d,colsys:%d,flag:%d,chnum:%d",state->ch_latest_updated,entry->freq,entry->sndsys,entry->colsys,entry->flag,chnum);
		if (entry->flag) {
...
void JNIAtvContext::fullscan_progress_cb(kal_uint8 precent,kal_uint32 freq,kal_uint32 freq_start,kal_uint32 freq_end) {
    ALOGE("fullscan_progress_cb is not implemented yet and should not be called");
}
...
static void atv_init(JNIEnv *env, jobject thiz)
{
    ...
    if (ret != true) {		
		ALOGE("ATV init failed with error code %d",ret);	        
...
static void atv_startVideo(JNIEnv *env, jobject thiz) {
	...
	#ifdef ATV_LOG_OUT
        ALOGD("camera->startPreview out");
	#endif	
	
		// HP Cheng: Here means the open is success, make TP single scan
#ifdef ENABLE_TP_CTRL
	#ifdef ATV_LOG_OUT
        ALOGD("ATVCtrl::ATVC_matv_set_tparam in");
	#endif	
		ATVCtrl::ATVC_matv_set_tparam(TP_SINGLE);
	#ifdef ATV_LOG_OUT
        ALOGD("ATVCtrl::ATVC_matv_set_tparam out");
	#endif	
#endif
}
...
static void atv_openVideo(JNIEnv *env, jobject thiz,jboolean connect)
{
	...
	#ifdef ATV_LOG_OUT
    ALOGD("ATVCtrl::ATVC_matv_suspend(0) in");
	#endif
        ATVCtrl::ATVC_matv_suspend(0);	
	#ifdef ATV_LOG_OUT
    ALOGD("ATVCtrl::ATVC_matv_suspend(0) out");
	#endif

	if (connect) {
		#ifdef ATV_LOG_OUT
    	ALOGD("connect true");
    	ALOGD("Camera::connect in");
		#endif
	    ...
	    if (camera == 0) return;
        #ifdef ATV_LOG_OUT
    	ALOGD("camera->setPreviewTexture in");
		#endif
        ...
        #ifdef ATV_LOG_OUT
    	ALOGD("camera->setPreviewTexture out");
		#endif
	}
	
	#ifdef ATV_LOG_OUT
    ALOGD("open Video out");
	#endif
}
...
static void atv_setPreviewDisplay(JNIEnv *env, jobject thiz, jobject jSurface)
{
	...
	#ifdef ATV_LOG_OUT
    ALOGD("camera->setPreviewTexture(buffer) in");
	#endif
	...
	#ifdef ATV_LOG_OUT
    ALOGD("camera->setPreviewTexture(buffer) out");
    ALOGD("setPreviewTexture out");
	#endif
}
...
{
    ALOGD("setHasPreviewCallback: installed:%d", (int)installed);
...
static void atv_setParameters(JNIEnv *env, jobject thiz, jstring params)
{
    ...
    String8 params8;
    if (params) {
        params8 = String8((const char16_t *)str, env->GetStringLength(params));
...
static void atv_setChannelTable(JNIEnv *env, jobject thiz,jint ch,jlong packedEntry)
{
	 ...
	 ALOGD("set channel table packedEntry = %lld,freq = %d sndsys = %d colsys = %d flag = %d",packedEntry,entry.freq,entry.sndsys,entry.colsys,entry.flag);
...
{
    ALOGD("change channel in");
    ATVCtrl::ATVC_matv_change_channel(ch);
	#ifdef ATV_LOG_OUT
    ALOGD("change channel out");
	#endif
}   
...
#define AUDIO_MASK_MPX      (MPX_MONO | MPX_STEREO | MPX_SUB | MPX_MAIN_SUB |MPX_MAIN)
	...
	ALOGD("atv_setPreferAudioFormat:%d",prefer_audio_mode);
...
int register_android_hardware_Atv(JNIEnv *env)
{
    ...
    if (fields.post_event == NULL) {
        ALOGE("Can't find com/mediatek/atv/AtvService.postEventFromNative");
        return -1;
    }
	/*
