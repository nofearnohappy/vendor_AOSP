...
public class Op09NotificationTransactionExt extends DefaultOpNotificationTransactionExt {
    IDownloadManagerCallback mHostCallback;
    public static final int STATE_UNSTARTED = 0x80;

    private static String TAG = "Op09NotificationTransactionExt";
    int CANCEL_DOWNLOAD = 5;

    public Op09NotificationTransactionExt(Context base) {
        super(base);
    }
    ...
    @Override
    public boolean run(boolean isCancelling, Uri uri, Context context,
                            Uri trxnUri, String contentLocation) {
        /// M: For OP09, check if cancel download requested.@{
        if (MessageUtils.isCancelDownloadEnable() && isCancelling) {
            Log.d(TAG, "***Canceling download in processing!(NotificationTransaction)");

            if (MessageUtils.isCancelDownloadEnable()) {
                //Op09MmsFailedNotify.getIntance(context).popupToast(CANCEL_DOWNLOAD, null);
            }
            Op09MmsCancelDownloadExt.getIntance(context)
                            .markStateExt(trxnUri, Op09MmsCancelDownloadExt.STATE_COMPLETE);
