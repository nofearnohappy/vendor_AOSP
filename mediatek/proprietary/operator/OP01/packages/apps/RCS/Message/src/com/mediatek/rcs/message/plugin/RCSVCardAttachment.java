...
public class RCSVCardAttachment {
            ...
            try {
                if (isAddingIpMessageVCardFile) {
                    writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(
                        RcsMessageUtils.getVcardTempPath(mContext) + File.separator + fileName)));
                } else {
                    writer = new BufferedWriter(new OutputStreamWriter(mContext.openFileOutput(fileName,
                        Context.MODE_PRIVATE)));
                }
                composer = new VCardComposer(mContext, VCardConfig.VCARD_TYPE_V30_GENERIC);
    ...
    public String getVCardFileNameByUri(Uri uri) {
        ...
        String filePath = RcsMessageUtils.getVcardTempPath(mContext) + File.separator + filename;
        try {
            InputStream in = null;
