...
public class BackupRestoreNotification {
    ...
    public void initBackupNotification(int type, int currentProgress) {
        ...
        setNotificationProgress(R.drawable.ic_backuprestore_notify, contentTitle, currentProgress,
                intentFilter);
    }
    ...
    private void setNotificationProgress(int iconDrawableId, CharSequence contentTitle,
            int currentProgress, String intentFilter) {
    ...
    public void updateNotification(int moduleType, int currentProgress) {
        if (mNotification != null) {
            mNotification.setProgress(mMaxPercent, currentProgress, false);
            MyLogger.logD(CLASS_TAG, "updateNotification and update currentProgress = "
                    + currentProgress + ",moduleType = " + moduleType);
        }
    }

    /*
     * public boolean updateAppNotification(int moduleType, int currentProgress)
     * { int currentCount = 0; if (currentProgress - currentCount > 2) { if
     * (mNotification != null) { mNotification.setProgress(mMaxPercent,
     * currentProgress, false); MyLogger.logD(CLASS_TAG,
     * "updateNotification and update currentProgress = " + currentProgress); }
     *
     * }
     *
     * return false; }
    ...
    public void LastNotification(int moduleType) {
        // when composer onEnd update progressBar and the progressBarvalue is
        // max
        if (mNotification != null) {
