...
public class OldMtkSmsRestoreComposer extends Composer {
    ...
    private static final Uri[] SMSURI = { Sms.Inbox.CONTENT_URI, Sms.Sent.CONTENT_URI,
            Sms.Draft.CONTENT_URI, Sms.Outbox.CONTENT_URI };
    ...
    private ContentValues parsePdu(byte[] pdu) {
        ...
        try {
            ...
            MyLogger.logD(CLASS_TAG, SMSTAG + "parsePdu timeStamp:" + timeStamp + "timeLength:"
                    + timeLength + ",readStamp:" + readStamp + ",boxStamp:" + boxStamp
                    + ",seenStamp:" + seenStamp + ",simcardStamp:" + simcardStamp);
            ...
            for (int i = 0; i < num; ++i) {
                ...
                MyLogger.logD(CLASS_TAG,
                        SMSTAG + "begin createFromPdu:" + System.currentTimeMillis());
                SmsMessage msg = SmsMessage.createFromPdu(rawPdu);
                MyLogger.logD(CLASS_TAG,
                        SMSTAG + "end createFromPdu:" + System.currentTimeMillis());
                if (msg != null) {
                    bodyBuilder.append(msg.getDisplayMessageBody());
                    if (i == 0) {
                        MyLogger.logD(CLASS_TAG, SMSTAG + "begin extractContentValues:"
                                + System.currentTimeMillis());
                        values = extractContentValues(msg);
                        MyLogger.logD(CLASS_TAG, SMSTAG + "end extractContentValues:"
                                + System.currentTimeMillis());
                    }
                } else {
