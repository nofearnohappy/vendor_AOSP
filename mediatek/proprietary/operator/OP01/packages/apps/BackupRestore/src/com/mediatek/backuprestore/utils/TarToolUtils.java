...
public abstract class TarToolUtils {
    ...
    public static void archive(File srcFile, File destFile) throws IOException {
        ...
        MyLogger.logD(CLASS_TAG, "archiveDir 63 : sFolderName= " + sFolderName
    ...
    private static void archiveDir(File dir, TarArchiveOutputStream taos, String basePath)
        ...
        MyLogger.logD(CLASS_TAG,
                "archiveDir : dir mode ==== " + mode + ", dir name = " + dir.getName());
        TarArchiveEntry entry = new TarArchiveEntry(TARPATH + sFolderName + dir.getName() + PATH);
        entry.setMode(mode);
        MyLogger.logD(CLASS_TAG,
                " name == " + entry.getName() + ", linkname == " + entry.getLinkName()
    ...
    private static void dearchive(File destFile, TarArchiveInputStream tais) throws IOException {
        ...
        while ((entry = tais.getNextTarEntry()) != null) {
            ...
            MyLogger.logD(CLASS_TAG, "dearchive File  : entry.getName()  == " + entry.getName()
                    + ", entry mode === " + entry.getMode() + ", file : " + dirFile.toString());
            fileProber(dirFile);
            if (entry.isDirectory()) {
                dirFile.mkdirs();
                android.os.FileUtils.setPermissions(dirFile.getAbsolutePath(), entry.getMode(), -1,
                        -1);
            } else {
     ...
     * @throws IOException
     */
    private static void dearchiveFile(File destFile, TarArchiveInputStream tais, int mode)
            throws IOException {
        ...
        MyLogger.logD(CLASS_TAG, " dearchiveFile >>> set File Permissions : file name is = "
                + destFile.getName() + ", file path is = " + destFile.getPath()
                + ", the file mode = " + mode);
