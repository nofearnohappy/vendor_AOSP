...
public class PictureRestoreComposer extends Composer {
    ...
    private static final String[] PROJECTION = new String[] { MediaStore.Images.Media._ID,
            ...
            try {
                ...
                if (mFileNameList == null) {
                    MyLogger.logD(CLASS_TAG,
                            "init(): zipFile is error Maybe the FIle is the error Zip ! ");
                    return result;
                }
                if (SDCardUtils.getSDCardDataPath(mContext) != null) {
                    tmppath = SDCardUtils.getSdCardMountPath(mContext);
                } else {
                    tmppath = SDCardUtils.getPhoneStoragePath(mContext);
                }
                if (tmppath != null) {
                    mDestPath = tmppath
                            + File.separator
    ...
    public boolean implementComposeOneEntity() {
        ...
        if (mIndex % Constants.NUMBER_SEND_BROCAST_MUSIC_EACH == 0 || isAfterLast()) {
            ...
            MyLogger.logD(CLASS_TAG, "mIndex = " + mIndex + "scanFile " + "isAfterLast() = "
                    + isAfterLast());
        }
        ...
        try {
            if (mDestPath != null) {
                if (mMediaScannerConnection.isConnected()) {
                    mMediaScannerConnection.scanFile(mDestPath, null);
                    MyLogger.logD(CLASS_TAG, "onEnd mIndex = " + mIndex + ".scanFile "
        ...
        if (file.exists()) {
            if (file.isFile()) {
                try {
                    ...
                    MyLogger.logD(CLASS_TAG,
                            "deleteFolder():" + count + ":" + file.getAbsolutePath());
                    file.delete();
                } catch (Exception e) {
    ...
    private String rename(String name) {
        String tmpName = name.subSequence(name.lastIndexOf(File.separator) + 1, name.length())
                .toString();
