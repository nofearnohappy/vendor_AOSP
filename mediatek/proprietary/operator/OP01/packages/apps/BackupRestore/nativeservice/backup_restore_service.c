    ...
    while(NULL !=(ent=readdir(pDir)))
    {
     ...
     strcpy(path, pathSrc);
        ...
        else
        {
            ...
            strcat(path,ent->d_name);
            if(remove(path) == -1) {
                ALOGI("app_data_del remove path = -1");
                if (pDir != NULL)
                {
                   closedir(pDir);
                }
...
int change_folder_owner(char *pathSrc, uid_t userId , gid_t groupId)
{
    ...
    if(pw != NULL && strcmp(pw->pw_name, "system") == 0){
 ...
 void change_owner(char *path) {
    ...
    if(pwd != NULL) {
        ...
        ALOGI("change_owner error pwd is null");
    }
...
static int backupAppData(const char* dirSrc, const char* dirDest)
{
    ...
    if (result >= 0 && stat(dirDest, &tmpstat) != 0)
    {
        ...
        ALOGI("backupAppData() after chmod dirDest() %s, selfstat.stmode:%d ",
                dirDest, selfstat.st_mode);
    }
           ...
           ((dirent_p = readdir(dirSrc_p)) != NULL))
    {
        ...
        if (dirent_p->d_type == DT_DIR)
        {
            ...
            ALOGI("backupAppData() folder stat %s:result:%d; errno: %s",
...
static int restoreAppData(const char* dirSrc, const char* dirDest, char ** con)
{
    int result = 0;
    DIR* dirSrc_p = NULL;
    struct dirent* dirent_p = NULL;
    ...
    ALOGI("restoreAppData() context %s ", *con);
    if ((dirSrc_p = opendir(dirSrc)) == NULL)
    {
    ...
    {
        stat(dirSrc, &tmpstat);
        if (mkdir(dirDest, tmpstat.st_mode) != 0) {
            ALOGI("restoreAppData() mkdir %s error", dirDest);
            result = -1;
        }
        chmod(dirDest, tmpstat.st_mode);

        if (setfilecon(dirDest, *con) < 0) {
            ALOGE("cannot setfilecon dir '%s': %s\n", dirDest, strerror(errno));
            result = -1;
        }
    }
           ...
           ((dirent_p = readdir(dirSrc_p)) != NULL))
    {
        ...
        if (dirent_p->d_type == 4)
        {
            ...
            if (result != 0) // folder does not exists
            {
                ALOGI("restoreAppData() folder not exit, create it");
                if ((result = stat(dirDest, &dirStat)) == 0)
                {
                    if ((result = mkdir(dest, dirStat.st_mode)) == 0)
                    {
                        ...
                        if (pw != NULL)
                        {
                            ...
                            ALOGI("restoreAppData() chown dest %s : folder result:%d", dest, result);
                            stat(src, &dirStat);
                            result = chmod(dest, dirStat.st_mode);
                            ALOGI("restoreAppData() chmod dest %s : folder result:%d", dest, result);
            ...
            if (result >= 0)
            {
                ALOGI("restoreAppData() stat(dest, &dirStat) dest %s:result:%d", dest, result);
                ALOGI("restoreAppData() dest %s: have exsit no create,", dest);
                result = restoreAppData(src, dest, con);
            }
            else
            {
                ALOGI("restoreAppData() stat(dest, &dirStat) dest %s:result:%d", dest, result);
...
static int restore(char** args, char reply[REPLY_LEN])
{
    ...
    char *con = NULL;
    int result = getfilecon(args[1], &con);
    ALOGI("restore() getfilecon, return %s", con);
    if (result < 0) {
            ALOGI("restore() getfilecon, return %d,", result);
            return -1;
    }

    int ret = restoreAppData(args[0], args[1], &con);
    if(ret<0){
        cleardir(args[1]);
    }
    return (ret >= 0) ? 1 : -1;
