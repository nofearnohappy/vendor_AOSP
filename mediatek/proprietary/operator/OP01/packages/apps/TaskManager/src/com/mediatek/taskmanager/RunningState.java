...
public class RunningState {
    ...
    final Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            case MSG_UPDATE_TIME:
                synchronized (mLock) {
                    if (!mResumed) {
                        Log.d("@M_" + TAG, "No resumed ,return in handler");
                        return;
                    }
                }
                ...
                if (mRefreshUiListener != null) {
                    ...
                    Log.d("@M_" + TAG, "Call Listener in Handler ");
    ...
    static class ProcessItem extends BaseItem {
        ...
        void ensureLabel(PackageManager pm) {
            ...
            try {
                ...
                Log.d("@M_" + TAG, "catch exception, process name not found");
            }
    ...
    void resume(OnRefreshUiListener listener) {
        ...
        synchronized (mLock) {
            ...
            if (!mBackgroundHandler.hasMessages(MSG_UPDATE_CONTENTS)) {
                Log.d("@M_" + TAG, "resume send update content");
        ...
        synchronized (mLock) {
            while (!mHaveData) {
                try {
                    ...
                    Log.d("@M_" + TAG, "catch exception, interrupt exception");
                }
            }
        }
    }
    ...
    private boolean updateMemoryInfo(Context context) {
        ...
        for (int i = 0; i < nrp; i++) {
            ...
            if (proc.mCurSeq != mSequence) {
                ...
                if (proc.mRunningProcessInfo.importance >= ActivityManager.RunningAppProcessInfo.IMPORTANCE_BACKGROUND) {
                    ...
                    Log.d("@M_" + "RunningState", "Unknown non-service process: "
        ...
        try {
            ...
            Log.d("@M_" + TAG, "catch remote exception");
        }
