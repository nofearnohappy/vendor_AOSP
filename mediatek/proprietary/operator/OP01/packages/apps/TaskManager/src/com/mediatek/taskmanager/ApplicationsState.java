...
public class ApplicationsState {
    ...
    void resume(Callbacks callbacks) {
        if (DEBUG_LOCKING) {
            Log.v("@M_" + TAG, "resume about to acquire lock...");
        }
        synchronized (mEntriesMap) {
            ...
            if (DEBUG_LOCKING) {
                Log.v("@M_" + TAG, "...resume releasing lock");
            }
        }
    }
    ...
    void pause() {
        if (DEBUG_LOCKING) {
            Log.v("@M_" + TAG, "pause about to acquire lock...");
        }
        synchronized (mEntriesMap) {
            mCurCallbacks = null;
            mResumed = false;
            if (DEBUG_LOCKING) {
                Log.v("@M_" + TAG, "...pause releasing lock");
            }
        }
       if (mPackageIntentReceiver != null) {
    ...
    void handleRebuildList() {
        ...
        for (int i = 0; i < apps.size(); i++) {
            ...
            if (filter == null || filter.filterApp(info)) {
                synchronized (mEntriesMap) {
                    if (DEBUG_LOCKING) {
                        Log.v("@M_" + TAG, "rebuild acquired lock");
                    }
                    AppEntry entry = getEntryLocked(info);
                    entry.ensureLabel(mContext);
                    if (DEBUG) {
                        Log.i("@M_" + TAG, "Using " + info.packageName + ": " + entry);
                    }
                    filteredApps.add(entry);
                    if (DEBUG_LOCKING) {
                        Log.v("@M_" + TAG, "rebuild releasing lock");
                    }
                }
            }
        }
    ...
    AppEntry getEntry(String packageName) {
        if (DEBUG_LOCKING) {
            Log.v("@M_" + TAG, "getEntry about to acquire lock...");
        }
        synchronized (mEntriesMap) {
            ...
            if (DEBUG_LOCKING) {
                Log.v("@M_" + TAG, "...getEntry releasing lock");
            }
            return entry;
        }
    }
    ...
    void requestSize(String packageName) {
        if (DEBUG_LOCKING) {
            Log.v("@M_" + TAG, "requestSize about to acquire lock...");
        }
        synchronized (mEntriesMap) {
            AppEntry entry = mEntriesMap.get(packageName);
            if (entry != null) {
            ...
            if (DEBUG_LOCKING) {
                Log.v("@M_" + TAG, "...requestSize releasing lock");
            }
        }
    }
    ...
    long sumCacheSizes() {
        long sum = 0;
        if (DEBUG_LOCKING) {
            Log.v("@M_" + TAG, "sumCacheSizes about to acquire lock...");
        }
        synchronized (mEntriesMap) {
            if (DEBUG_LOCKING) {
                Log.v("@M_" + TAG, "-> sumCacheSizes now has lock");
            }
            for (int i = mAppEntries.size() - 1; i >= 0; i--) {
                sum += mAppEntries.get(i).mCacheSize;
            }
            if (DEBUG_LOCKING) {
                Log.v("@M_" + TAG, "...sumCacheSizes releasing lock");
            }
        }
        return sum;
    }
    ...
    void addPackage(String pkgName) {
        try {
            synchronized (mEntriesMap) {
                if (DEBUG_LOCKING) {
                    Log.v("@M_" + TAG, "addPackage acquired lock");
                }
                if (DEBUG) {
                    Log.i("@M_" + TAG, "Adding package " + pkgName);
                }
                if (!mResumed) {
                    ...
                    // here.
                    if (DEBUG_LOCKING) {
                        Log.v("@M_" + TAG, "addPackage release lock: not resumed");
                    }
                    return;
                }
                if (indexOfApplicationInfoLocked(pkgName) >= 0) {
                    if (DEBUG) {
                        Log.i("@M_" + TAG, "Package already exists!");
                    }
                    if (DEBUG_LOCKING) {
                        Log.v("@M_" + TAG, "addPackage release lock: already exists");
                    }
                    return;
                }
                ...
                if (DEBUG_LOCKING) {
                    Log.v("@M_" + TAG, "addPackage releasing lock");
                }
            }
    ...
    void removePackage(String pkgName) {
        synchronized (mEntriesMap) {
            if (DEBUG_LOCKING) {
                Log.v("@M_" + TAG, "removePackage acquired lock");
            }
            int idx = indexOfApplicationInfoLocked(pkgName);
            if (DEBUG) {
                Log.i("@M_" + TAG, "removePackage: " + pkgName + " @ " + idx);
            }
            if (idx >= 0) {
                AppEntry entry = mEntriesMap.get(pkgName);
                if (DEBUG) {
                    Log.i("@M_" + TAG, "removePackage: " + entry);
                }
                if (entry != null) {
            ...
            if (DEBUG_LOCKING) {
                Log.v("@M_" + TAG, "removePackage releasing lock");
            }
        }
    }
    ...
    AppEntry getEntryLocked(ApplicationInfo info) {
        ...
        if (DEBUG) {
            Log.i("@M_" + TAG, "Looking up entry of pkg " + info.packageName + ": " + entry);
        }
        if (entry == null) {
            if (DEBUG) {
                Log.i("@M_" + TAG, "Creating AppEntry for " + info.packageName);
            }
    ...
    class BackgroundHandler extends Handler {
        ...
        final IPackageStatsObserver.Stub mStatsObserver = new IPackageStatsObserver.Stub() {
            public void onGetStatsCompleted(PackageStats stats, boolean succeeded) {
                ...
                synchronized (mEntriesMap) {
                    if (DEBUG_LOCKING) {
                        Log.v("@M_" + TAG, "onGetStatsCompleted acquired lock");
                    }
                    ...
                    if (entry != null) {
                        synchronized (entry) {
                            ...
                            if (entry.mSize != newSize ||
                                ...
                                if (DEBUG) {
                                    Log.i("@M_" + TAG, "Set size of " + entry.mLabel + " " + entry
                                        + ": " + entry.mSizeStr);
                                }
                                sizeChanged = true;
                            }
                    ...
                    if (DEBUG_LOCKING) {
                        Log.v("@M_" + TAG, "onGetStatsCompleted releasing lock");
                    }
                }
            }
        };
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case MSG_LOAD_ENTRIES:
                    ...
                    synchronized (mEntriesMap) {
                        if (DEBUG_LOCKING) {
                            Log.v("@M_" + TAG, "MSG_LOAD_ENTRIES acquired lock");
                        }
                        ...
                        if (DEBUG_LOCKING) {
                            Log.v("@M_" + TAG, "MSG_LOAD_ENTRIES releasing lock");
                        }
                    }
                ...
                case MSG_LOAD_ICONS:
                    ...
                    synchronized (mEntriesMap) {
                        if (DEBUG_LOCKING) {
                            Log.v("@M_" + TAG, "MSG_LOAD_ICONS acquired lock");
                        }
                        ...
                        if (DEBUG_LOCKING) {
                            Log.v("@M_" + TAG, "MSG_LOAD_ICONS releasing lock");
                        }
                ...
                case MSG_LOAD_SIZES:
                    synchronized (mEntriesMap) {
                        if (DEBUG_LOCKING) {
                            Log.v("@M_" + TAG, "MSG_LOAD_SIZES acquired lock");
                        }
                        if (mCurComputingSizePkg != null) {
                            if (DEBUG_LOCKING) {
                                Log.v("@M_" + TAG, "MSG_LOAD_SIZES releasing: currently computing");
                            }
                            return;
                        }
                        ...
                        for (int i = 0; i < mAppEntries.size(); i++) {
                            ...
                            if (entry.mSize == SIZE_UNKNOWN || entry.mSizeStale) {
                                ...
                                if (DEBUG_LOCKING) {
                                    Log.v("@M_" + TAG, "MSG_LOAD_SIZES releasing: now computing");
                                }
                                return;
                            }
                        }
                        ...
                        if (DEBUG_LOCKING) {
                            Log.v("@M_" + TAG, "MSG_LOAD_SIZES releasing lock");
                        }
                    }
                    break;
                default:
