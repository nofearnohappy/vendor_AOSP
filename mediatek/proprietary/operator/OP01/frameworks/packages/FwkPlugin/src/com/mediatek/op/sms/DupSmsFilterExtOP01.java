...
public class DupSmsFilterExtOP01 extends DupSmsFilterExt {
    ...
    public DupSmsFilterExtOP01(Context context) {
        super(context);
        Log.d("@M_" + TAG, "call OP01 constructor");
    }

    protected boolean isDupSms(byte[] newPdu, byte[] oldPdu) {
        Log.d("@M_" + TAG, "call OP01 isDupSms");

        // no SC
        if (newPdu.length != oldPdu.length) {
            Log.d("@M_" + TAG, "OP01 DupSms, different length");
            return false;
        }
        ...
        if (Arrays.equals(newPduByte, oldPduByte)) {
            Log.d("@M_" + TAG, "find a OP01 duplicated sms");
            return true;
        }
        Log.d("@M_" + TAG, "not a OP01 dup sms");
        return false;
    }
}
