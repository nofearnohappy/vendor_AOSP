...
int mt_log_write2(unsigned char c)
{
    uint64_t mpidr = read_mpidr();
    uint32_t linear_id = platform_get_core_pos(mpidr);
    unsigned char *ptr_ex_buf = NULL;
    unsigned int *ptr_write_pos = NULL;

    // Exit if ptr_atf_crash_flag equals NULL
    if(ptr_atf_crash_flag == NULL)
        return 0;
    
    ptr_write_pos = &p_atf_log_ctrl->info.atf_except_write_pos_per_cpu[linear_id];
    ptr_ex_buf = (unsigned char*)((uintptr_t)*ptr_write_pos);

    // Check exception buffer boundary
    if((uintptr_t)ptr_ex_buf <= (uintptr_t)mt_exception_buf_end) {
        *ptr_ex_buf = c;
        (*ptr_write_pos)++;
    } else {
        // out of boundary
...
void mt_log_setup(unsigned int start, unsigned int size, unsigned int aee_buf_size)
{
    ...
    // Initialize those two pointers for mt_log_write2
    ...
    bl31_log_service_register(&mt_log_lock_acquire, &mt_log_write, 
        &mt_log_lock_release);
    bl31_log_service_register2(NULL, &mt_log_write2, NULL);
}
