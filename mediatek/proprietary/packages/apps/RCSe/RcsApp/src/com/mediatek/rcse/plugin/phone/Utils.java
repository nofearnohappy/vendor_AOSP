...
public class Utils {
    ...
    /**
     * The Constant VIRATOR_LOCK.
     */
    private static final Object VIRATOR_LOCK = new Object();

    /**
     * Checks if is in image sharing.
     *
     * @return true, if is in image sharing
     */
    ...
    /**
     * Checks if is in video sharing.
     *
     * @return true, if is in video sharing
     */
    ...
    /**
     * Sets the in image sharing.
     *
     * @param isInImageSharing the new in image sharing
     */
    ...
    /**
     * Sets the in video sharing.
     *
     * @param isInVideoSharing the new in video sharing
     */
    ...
    public static int getNetworkType(Context context) {
        ...
        ConnectivityManager connectivityManager = (ConnectivityManager) context
                .getSystemService(ContextWrapper.CONNECTIVITY_SERVICE);
    ...
    public static void setCameraDisplayOrientation(Activity activity,
        ...
        Logger.v(TAG, "setCameraDisplayOrientation(), activity = "
                + activity + ", cameraId = " + cameraId
                + ", camera = " + camera);
        ...
        if (info.facing == Camera.CameraInfo.CAMERA_FACING_FRONT) {
            Logger.d(TAG, "Front camera info.orientation = "
                    + info.orientation);
            result = (info.orientation + degrees) % 360;
            result = (360 - result) % 360; // compensate the mirror
        } else { // back-facing
            Logger.d(TAG, "Back camera info.orientation = "
                    + info.orientation);
    ...
    public static int getRotationDegrees(Activity activity,
            int cameraId, Camera camera) {
        Logger.v(TAG, "getRotationDegrees(), activity = " + activity
                + ", cameraId = " + cameraId + ", camera = " + camera);
        if (activity == null || camera == null) {
        ...
        if (info.facing == Camera.CameraInfo.CAMERA_FACING_FRONT) {
            Logger.d(TAG,
                    "getRotationDegrees(),front camera info.orientation = "
                            + info.orientation);
            rotation = (info.orientation - degrees + 360) % 360;
        } else { // back-facing
            Logger.d(TAG,
                    "getRotationDegrees(), back camera info.orientation = "
                            + info.orientation);
            rotation = (info.orientation + degrees) % 360;
        }
        Logger.v(TAG, "getRotationDegrees(), return rotation = "
                + rotation);
        return rotation;
    }
    /**
    ...
    public static boolean vibrate(Context context, long time) {
        Logger.e(TAG, "vibrate(), context = " + context + ", time = "
                + time);
        ...
        synchronized (VIRATOR_LOCK) {
            if (sVibratorService == null) {
                Logger.d(TAG,
                        "sVibratorService is null then create it.");
                sVibratorService = (Vibrator) context
    ...
    public static long getFreeStorageSize() {
        ...
        if (isExist) {
            ...
            Logger.d(TAG, "getFreeStorageSize() blockSize: "
