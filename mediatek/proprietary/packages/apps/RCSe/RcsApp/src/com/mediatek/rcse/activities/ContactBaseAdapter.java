...
public abstract class ContactBaseAdapter extends BaseAdapter implements
    ...
    /**
     * Gets the selected contact list.
     *
     * @return the selected contact list
     */
    ...
    /**
     * Instantiates a new contact base adapter.
     *
     ...
     *
     * @param list the new contacts list
     */
    ...
    public Object getItem(int position) {
        if (mContactsList != null && position >= 0
                && position < mContactsList.size()) {
            return mContactsList.get(position);
        } else {
            Logger.w(TAG, "getItem, mContactsList is null or position < 0, "
                    + " position = " + position);
            return null;
        }
    }
    ...
    /**
     * On configuer item view.
     *
     * @param view the view
     * @param position the position
     * @param isIndexer the is indexer
     * @return the view holder
     */
    public abstract ViewHolder onConfiguerItemView(View view, int position,
            boolean isIndexer);
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (mSectionToPosition != null
                && mSectionToPosition.containsValue(position)) {
     ...
     * @param isIndexer the is indexer
     */
    protected void configuerItemView(View convertView, int position,
            boolean isIndexer) {
        ViewHolder holder = onConfiguerItemView(convertView, position,
                isIndexer);
    ...
    public int getPositionForSection(int section) {
        ...
        if (null != mSectionToPosition
                && mSectionToPosition.containsKey(section)) {
    ...
    private void initSectionsAndPositions() {
        ...
        for (int pos = 0; pos < contactSize; ++pos) {
            ...
            String firstLetter = contactItem.mSortKey.trim()
                    .substring(INDEX_LETTER_START_INDEX, INDEX_LETTER_LENGTH)
                    .toUpperCase();
            if (!alphaIndexerList.contains(firstLetter)) {
                ...
                Logger.v(TAG, "mContactsList's size changed, contactSize = "
                        + contactSize);
                ++alphaIndexerNum;
            } else {
            ...
            Logger.v(TAG, " contactItem[" + pos + "].mSortKey = "
                    + contactItem.mSortKey + ",.mSectionIndex = "
                    + contactItem.mSectionIndex);
        }
        mSections = new String[alphaIndexerNum];
        alphaIndexerList.toArray(mSections);
        for (int pos = 0; pos < contactSize; ++pos) {
            Logger.v(TAG, " contactList[" + pos + "].mSortKey = "
                    + mContactsList.get(pos).mSortKey + ",.mSectionIndex = "
                    + mContactsList.get(pos).mSectionIndex);
        }
        Iterator<Map.Entry<Integer, Integer>> iterator = mSectionToPosition
                .entrySet().iterator();
        Logger.v(TAG, "mSectionToPosition[=");
        for (; iterator.hasNext();) {
            Map.Entry<Integer, Integer> mapEntry = (Map.Entry<Integer, Integer>) iterator
                    .next();
