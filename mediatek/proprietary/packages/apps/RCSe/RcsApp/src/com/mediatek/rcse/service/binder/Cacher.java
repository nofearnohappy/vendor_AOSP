...
public class Cacher<K extends Object, V extends Object> {
     ...
     * The m tag cache.
     */
    private final Map<K, TagReference> mTagCache =
            new ConcurrentHashMap<K, TagReference>();
    /**
     * The m tag recycler.
     */
    ...
    public void putValue(K key, V value) {
        ...
        mTagCache
                .put(key, new TagReference(key, value, mTagRecycler));
        clearUnreachableTag();
    }
    /**
    ...
    public V getValue(K key) {
        ...
        if (null != reference) {
            ...
            Logger.d(TAG, "getValue() key: " + key + ", value: "
                    + value);
            return value;
        } else {
            Logger.w(TAG, "getValue() tag for key: " + key
                    + " doesn't exist");
            return null;
        }
    }
    /**
     * Clear unreachable tag.
     */
    private void clearUnreachableTag() {
        TagReference reference = null;
        while (null != (reference = (TagReference) mTagRecycler
                .poll())) {
            K key = reference.mKey;
            Logger.d(TAG,
                    "clearUnreachableTag () remove unreached tag "
                            + key);
    ...
    private class TagReference extends WeakReference<V> {
        ...
        /**
         * Instantiates a new tag reference.
         *
         * @param k the k
         * @param r the r
         * @param q the q
