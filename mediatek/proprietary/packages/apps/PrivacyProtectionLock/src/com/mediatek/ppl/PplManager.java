...
//import com.android.internal.os.storage.ExternalStorageFormatter;//temp toggle comment for build error
...
public class PplManager {
    ...
    private synchronized void executeWipe() {
        ...
        mService.sendBroadcast(intent);
        Intent in = new Intent(Intent.ACTION_MASTER_CLEAR);
     ...
     * lock the phone.
     */
    public synchronized void onSimStateChanged(boolean[] readyNotAbsent) {
        // dirty hack
        ...
        if (mCache.isEnabled()) {
            ...
            for (int i = 0; i < serialNumbers.length; ++i) {
                if ((inserted[i] && serialNumbers[i] == null)
                        || (!inserted[i] &&
                                readyNotAbsent != null && readyNotAbsent[i])) {
