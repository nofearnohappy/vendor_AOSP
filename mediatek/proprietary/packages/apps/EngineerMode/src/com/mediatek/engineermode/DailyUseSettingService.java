...
public class DailyUseSettingService extends Service {
    ...
    private void doSettings() {
        Log.v("@M_" + TAG, String.format("Quick boot: %1$s, IVSR: %2$s, VM Log: %3$s",
        ...
        if (ipoSettingEnabled == mQuickBoot) {
            Log.v("@M_" + TAG, "Skip quick boot setting");
        } else {
        ...
        if (ivsrEnabled == mIvsr) {
            Log.v("@M_" + TAG, "Skip IVSR setting");
        } else {
        ...
        if (0 == ret) {
            ...
            if (0 == ret && mVmLog) {
                ...
                Log.v("@M_" + TAG, "Set EM parameter fail");
            }
        } else {
            Log.v("@M_" + TAG, "Get EM parameter fail");
        }
        vmSetOk = ret == 0;
        Toast.makeText(
                this,
