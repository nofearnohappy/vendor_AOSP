...
public class DeviceMgr extends PreferenceActivity
    ...
    public boolean onPreferenceChange(Preference preference, Object objValue) {
        ...
        if (KEY_SMS_AUTO_REG.equals(key)) {
            try {
                ...
                Log.e("@M_" + TAG, "set sms auto reg exception. ", e);
            }
    ...
    protected Dialog onCreateDialog(int id, Bundle args) {
        Log.d("@M_" + TAG, "Create dialog with id " + id);
    ...
    private int getSavedCTA() {
        if (mAgent == null) {
            Log.e("@M_" + TAG, "get CTA failed, agent is null!");
            return 0;
        }
        int savedCTA = 0;
        try {
            ...
            Log.e("@M_" + TAG, "get cta cmcc switch failed, readCTA failed!");
            e.printStackTrace();
        } catch (NumberFormatException e) {
            Log.e("@M_" + TAG, "number format exception. ", e);
        }
    ...
    private void setSavedCTA(String cta) {
        if (mAgent == null) {
            Log.e("@M_" + TAG, "save CTA switch value failed, agent is null!");
            return;
        }
        try {
            mAgent.setRegisterSwitch(cta.getBytes());
        } catch (RemoteException e) {
            Log.e("@M_" + TAG, "save CTA switch failed, writeCTA failed!");
            e.printStackTrace();
        }
    ...
    private int getSwitchValue() {
        int dmConfSwitch = 0;
        if (mAgent == null) {
            Log.e("@M_" + TAG, "get dm conf switch failed, agent is null!");
            return dmConfSwitch;
        }
        try {
            ...
            Log.e("@M_" + TAG, "get dm conf switch failed, getSwitchValue failed!");
            e.printStackTrace();
        } catch (NumberFormatException e) {
            Log.e("@M_" + TAG, "number format exception. ", e);
        }
        Log.i("@M_" + TAG, "Get getSwitchValue = [" + dmConfSwitch + "]");
        return dmConfSwitch;
    }

    private void setSwitchValue(int value) {
        if (mAgent == null) {
            Log.e("@M_" + TAG, "save dm conf switch failed, agent is null!");
            return;
        }
        final String switchString = Integer.toString(value);
        try {
            mAgent.setSwitchValue(switchString.getBytes());
            Log.d("@M_" + TAG, "dm conf switch is set.");
...
//            mAgent.setDmSwitchValue(markTime.getBytes());
            ...
            Log.d("@M_" + TAG, "dm pending flag is set.");
//            mAgent.setSmsRegSwitchValue(markTime.getBytes());
            mAgent.setSmsRegSwitchValue("1".getBytes());
            Log.d("@M_" + TAG, "sms reg pending flag is set.");
        } catch (RemoteException e) {
            Log.e("@M_" + TAG, "save dm conf switch failed, setSwitchValue failed!");
            e.printStackTrace();
        }
        Log.i("@M_" + TAG, "setSwitchValue [" + switchString + "]");
    }
