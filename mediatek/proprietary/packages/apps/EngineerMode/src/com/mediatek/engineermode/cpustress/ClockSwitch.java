...
public class ClockSwitch extends Activity implements OnCheckedChangeListener,
    ...
    private final Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            ...
            Log.v("@M_" + TAG, "msg.what: " + msg.what + " msg.obj: "
                    + msg.obj.toString());
            switch (msg.what) {
            case INDEX_SET_MODE:
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (ChipSupport.MTK_6589_SUPPORT <= ChipSupport.getChip()) {
            try {
                ...
                Log.w("@M_" + TAG, "Get max freq IOException: " + e.getMessage());
            }
        } else {
    ...
    private void updatePeriodView(String period) {
        Log.v("@M_" + TAG, "Enter updatePeriodView: " + period);
        int begin;
        int end;
    ...
    private void handleEvent(final String value, final int index) {
        Log.v("@M_" + TAG, "handleEvent: " + value + " " + index);
        new Thread(new Runnable() {
    ...
    private void getResponse(int index) {
        ...
        case INDEX_SET_DEBUG_MODE:
            ...
            break;
        default:
            Log.d("@M_" + TAG, "getResponse: index is error, " + index);
            break;
        }
    }
    ...
    private void setCommand(String value, int index) {
        ...
        try {
            command = String.format(SET_COMMAND_FORMAT, value, FILES[index
                    % INDEX_SET_QUERY_DELTA]);
            Log.v("@M_" + TAG, "setCommand: " + command);
        } catch (NullPointerException e) {
            bError = true;
            Log.d("@M_" + TAG, "Command format NullPointerException: "
                    + e.getMessage());
        } catch (IllegalFormatException e) {
            bError = true;
            Log.d("@M_" + TAG, "Command format IllegalFormatException: "
                    + e.getMessage());
        }
        if (bError) {
            Toast.makeText(this,
            ...
            synchronized (this) {
                try {
                    if (ShellExe.RESULT_SUCCESS == ShellExe.execCommand(command)) {
                        Log.d("@M_" + TAG, "Exec command succeed!");
                    }
                } catch (IOException e) {
                    Log.d("@M_" + TAG, "Exec command IOException: " + e.getMessage());
                }
            }
        }
    }
    ...
    private void getCurrentStatus(int index) {
        ...
        case INDEX_QUERY_DEBUG_MODE:
            ...
            break;
        default:
            Log.d("@M_" + TAG, "getCurrentStatus: index is error, " + index);
            break;
        }
    }
    ...
    private void getStatus(int index) {
        Log.v("@M_" + TAG, "Enter getStatus: " + index);
        boolean bError = false;
        String command = null;
        try {
            command = String.format(GET_COMMAND_FORMAT, FILES[index
                    % INDEX_SET_QUERY_DELTA]);
            Log.v("@M_" + TAG, "getCommand: " + command);
        } catch (NullPointerException e) {
            bError = true;
            Log.d("@M_" + TAG, "Command format NullPointerException: "
                    + e.getMessage());
    ...
    private void updateArmPll() {
        ...
        try {
                ...
                while (matcher.find()) {
                    ...
                    if (0 == index) {
                        ...
                        if (name.contains(PLL_NAME_ARM)) {
                            Log.v("@M_" + TAG, "find ARMPLL: id " + mArmPllId
                                    + " value " + value);
                            break;
                        }
                    }
            ...
            Log.w("@M_" + TAG, "updateArmPll IOException: " + e.getMessage());
        }
    ...
    private void setArmPll() {
        ...
        if (m.find()) {
            ...
            try {
                ...
                Log.w("@M_" + TAG, "setArmPll IOException: " + e.getMessage());
            }
            Toast.makeText(this,
    ...
    public void onCheckedChanged(CompoundButton arg0, boolean arg1) {
        if (arg0.isEnabled()) {
            if (mCbDebugMsgEnable.getId() == arg0.getId()) {
                Log.v("@M_" + TAG, "CheckBox is checked: " + arg1);
    ...
    public void onClick(View arg0) {
        if (mTvDebugMsgEnable.getId() == arg0.getId()) {
            Log.v("@M_" + TAG, "TextView is clicked");
    ...
    public void onServiceConnected(ComponentName name, IBinder service) {
        Log.v("@M_" + TAG, "Enter onServiceConnected");
    ...
    public void onServiceDisconnected(ComponentName name) {
        Log.v("@M_" + TAG, "Enter onServiceDisconnected");
        // mBoundService.testObject = null;
        mBoundService = null;
    }
    ...
    public void onUpdateTestResult() {
        Log.v("@M_" + TAG, "Enter onupdateTestResult");
    }
