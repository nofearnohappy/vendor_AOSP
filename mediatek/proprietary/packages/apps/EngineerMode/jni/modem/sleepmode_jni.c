...
JNIEXPORT jboolean Java_com_mediatek_engineermode_modem_SleepMode_setSleepMode(
	...
	ALOGW("open file successful");
	write(file, Message, sizeof(Message));
	ALOGW("write file successful");
	ALOGW("After set SleepMode: %d %d %d %d", Message[0], Message[1],
			Message[2], Message[3]);
	close(file);
	...
	if (-1 == fileRead) {
		ALOGW("open ccci_uem_rx failed");
		return -1;
	}
	ALOGW("open ccci_uem_rx successful");
	ALOGW("Source: %d %d %d %d", Message[0], Message[1], Message[2], Message[3]);
	read(fileRead, Message, sizeof(Message));
	ALOGW("read file successful");
	ALOGW("Recv: %d %d %d %d", Message[0], Message[1], Message[2], Message[3]);
	close(fileRead);
...
JNIEXPORT jboolean Java_com_mediatek_engineermode_modem_SleepMode_setDCM(
	...
	ALOGW("write file successful");
	ALOGW("After set DCM: %d %d %d %d", Message[0], Message[1], Message[2],
			Message[3]);
	close(file);
