...
extern "C" BOOL EM_BT_read(unsigned char *buf, unsigned int len, unsigned int *pu4ResultLen);

extern "C" BOOL EM_BT_write(unsigned char *buf, unsigned int len);
...
//	ALOGI("nPatter = %d", nPatter);
//	ALOGI("nChannels = %d", nChannels);
//	ALOGI("nPocketType = %d", nPocketType);
//	ALOGI("nPocketTypeLen = %d", nPocketTypeLen);
//	ALOGI("nFreq = %d", nFreq);
...
/*	if (false == DoBTReset()) {
		ALOGE("Leave process_txtest() due to DoBTReset() failure...\n");
		return -1;
	}
*/
...
static int process_modetest(JNIEnv *env, jobject thiz) {
	...
	/*	if (false == EM_BT_write(peer_write_buf, sizeof(peer_write_buf))) {
	 ALOGE("Leave process_modetest(): due to EM_BT_write() failure. (1)\n");
	 EM_BT_deinit();
	 return -1;
	 }

	 if (false == EM_BT_read(ucEvent, sizeof(ucEvent), &u4ResultLen)) {
	 ALOGE("Leave process_modetest() due to EM_BT_read() failure... (1)\n");
	 EM_BT_deinit();
	 return -1;
	 }

	if (false == DoBTReset()) {
		ALOGE("Leave process_modetest() due to DoBTReset() failure... \n");
		return -1;
	}
*/
	////////
...
static jcharArray BtTest_HCICommandRun(JNIEnv *env, jobject thiz, jcharArray HCICmdBuffer, jint HCICmdLength) {
	...
	for (int i = 0; i < HCICmdLength; i++) {
		...
		/*
		ALOGD("addr(ucHCICmdBuffer + i) = %1$08x", ucHCICmdBuffer + i);
		ALOGD("value(ucHCICmdBuffer + i) = %1$02x",ucHCICmdBuffer[i]);
		*/
	...
	for (int i = 0; i < u4ResultLen; i++) {
		/*
		ALOGD("addr(ucResultArray + i) = %1$08x", ucResultArray + i);
		ALOGD("value(ucResultArray + i) = %1$02x", ucResultArray[i]);
		*/
		ALOGD("%1$02x", ucResultArray[i]);
		usResultArray[i] = ucResultArray[i];
	}
...
static int BtTest_getChipId(JNIEnv *env, jobject thiz) {
	int chipId = 0x0;
#ifdef MT6620
	ALOGE("MTK_BT_CHIP = MT6620\n");
	chipId = 0x6620;
#endif
#ifdef MT6616
	ALOGE("MTK_BT_CHIP = MT6616\n");
	chipId = 0x6616;
#endif

#ifdef MT6612
	ALOGE("MTK_BT_CHIP = MT6612\n");
	chipId = 0x6612;
#endif
#ifdef MTK_MT6611
	ALOGE("MTK_BT_CHIP = MTK_MT6611\n");
	chipId = 0x6611;
#endif

#ifdef MTK_MT6612
	ALOGE("MTK_BT_CHIP = MTK_MT6612\n");
	chipId = 0x6612;
#endif

#ifdef MTK_MT6616
	ALOGE("MTK_BT_CHIP = MTK_MT6616\n");
	chipId = 0x6616;
#endif

#ifdef MTK_MT6620
	ALOGE("MTK_BT_CHIP = MTK_MT6620\n");
	chipId = 0x6620;
#endif

#ifdef MTK_MT6622
	ALOGE("MTK_BT_CHIP = MTK_MT6622\n");
	chipId = 0x6622;
#endif

#ifdef MTK_MT6626
	ALOGE("MTK_BT_CHIP = MTK_MT6626\n");
	chipId = 0x6626;
#endif

#ifdef MTK_MT6628
	ALOGE("MTK_BT_CHIP = MTK_MT6628\n");
	chipId = 0x6628;
#endif
	return chipId;
}
...
static jboolean BtTest_StartNoSigRxTest(JNIEnv *env, jobject thiz,
	...
	if(u4ResultLen == 14) //receive it again
		{
				memset(ucEvent, 0, sizeof(ucEvent));
	if (false == EM_BT_read(ucEvent, sizeof(ucEvent), &u4ResultLen)) {
		ALOGE("Leave BtTest_NoSigRxTest() due to EM_BT_read() failure...\n");
...
static jboolean BtTest_setSSPDebugMode(JNIEnv *env, jobject thiz,
             jboolean value) {
	ALOGE("Enter BtTest_setSSPDebugMode.");
#ifdef MTK_BLUEANGEL_SUPPORT
...
static int register_com_mediatek_bluetooth_bttest(JNIEnv *env) {
	 ...
	 */
	ALOGE("register_com_mediatek_bluetooth_bttest");
