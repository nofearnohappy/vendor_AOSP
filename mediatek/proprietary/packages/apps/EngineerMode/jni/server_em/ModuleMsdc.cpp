...
int ModuleMsdc::getCurrentInternal(int hostNum, int opcode)
{
    ...
    if(opcode == 0){
	  ...
	  if (chipid >= MTK_6589_SUPPORT) {
	      ...
	      {
		      ALOGD("success: clk_pu=%d,  clk_pd = %d, cmd_pu=%d,  cmd_pd = %d, dat_pu=%d,  dat_pd = %d\r\n", 
		      ...
		      ALOGD("error: clk_pu=%d,  clk_pd = %d, cmd_pu=%d,  cmd_pd = %d, dat_pu=%d,  dat_pd = %d\r\n", 
				l_clkIdx_pu, l_clkIdx_pd, l_cmdIdx_pu, l_cmdIdx_pd, l_datIdx_pu, l_datIdx_pd);
		      return -1;
	    }
	  }
...
{
	ALOGD("hostNum=%d,  sd30Mode = %d, sd30MaxCurrent=%d,  sd30Drive = %d, sd30PowerControl=%d",
