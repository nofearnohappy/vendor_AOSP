...
MSDC_ODC_12MA,
MSDC_ODC_16MA };
JNIEXPORT jint JNICALL Java_com_mediatek_engineermode_io_EmGpio_newGetCurrent(JNIEnv * env,
        jobject obj, jint hostNum, jint opcode) {
    int chipid;
    ...
    if (opcode == 0) {
        ...
        if (chipid >= MTK_6589_SUPPORT) {
            if (l_clkIdx_pu != 0xF && l_cmdIdx_pu != 0xF && l_datIdx_pu != 0xF) {
                ALOGD(
                        "6589 success: clk_pu=%d,  clk_pd = %d, cmd_pu=%d,  cmd_pd = %d, dat_pu=%d,  dat_pd = %d\r\n",
                        l_clkIdx_pu, l_clkIdx_pd, l_cmdIdx_pu, l_cmdIdx_pd, l_datIdx_pu,
                        l_datIdx_pd);
                return ((l_datIdx_pd << 20) | (l_datIdx_pu << 16) | (l_cmdIdx_pd << 12)
                        | (l_cmdIdx_pu << 8) | (l_clkIdx_pd << 4) | l_clkIdx_pu);
            } else {
                ALOGD(
                        "MSDC_JNI: error: clk_pu=%d,  clk_pd = %d, cmd_pu=%d,  cmd_pd = %d, dat_pu=%d,  dat_pd = %d\r\n",
                        l_clkIdx_pu, l_clkIdx_pd, l_cmdIdx_pu, l_cmdIdx_pd, l_datIdx_pu,
                        l_datIdx_pd);
                return -1;
            }
        } else {
            if (l_clkIdx_pu != 0xF && l_clkIdx_pd != 0xF && l_cmdIdx_pu != 0xF && l_cmdIdx_pd != 0xF
                    && l_datIdx_pu != 0xF && l_datIdx_pd != 0xF) {
                ALOGD(
                        "success: clk_pu=%d,  clk_pd = %d, cmd_pu=%d,  cmd_pd = %d, dat_pu=%d,  dat_pd = %d\r\n",
                        l_clkIdx_pu, l_clkIdx_pd, l_cmdIdx_pu, l_cmdIdx_pd, l_datIdx_pu,
                        l_datIdx_pd);
                return ((l_datIdx_pd << 20) | (l_datIdx_pu << 16) | (l_cmdIdx_pd << 12)
                        | (l_cmdIdx_pu << 8) | (l_clkIdx_pd << 4) | l_clkIdx_pu);
        ...
        if (l_hopBitIdx != 0xF && l_hopTimeIdx != 0xF) {
            ...
            ALOGD("MSDC_JNI: error: l_hopBitIdx=%d,  l_hopTimeIdx = %d\r\n", l_hopBitIdx,
                    l_hopTimeIdx);
            return -1;
        }
    }
...
JNIEXPORT jboolean JNICALL Java_com_mediatek_engineermode_io_EmGpio_newSetCurrent(JNIEnv * env,
        jobject obj, jint hostNum, jint clkPU, jint clkPD, jint cmdPU, jint cmdPD, jint datPU,
        jint datPD, jint hopBit, jint hopTime, jint opcode) {

    if (hostNum > MSDC_MAX_HOST_MUM || clkPU > 7 || clkPD > 7 || cmdPU > 7 || cmdPD > 7 || datPU > 7
            || datPD > 7 || hopBit > 3 || hopTime > 5) {
        ALOGD("----error: hostNum or currentIdx too large.\n");
        return JNI_FALSE;
    }
    ...
    ALOGD("set: clk_pu=%d,  clk_pd = %d, cmd_pu=%d,  cmd_pd = %d, dat_pu=%d,  dat_pd = %d\r\n",
...
JNIEXPORT jboolean JNICALL Java_com_mediatek_engineermode_io_EmGpio_setSd30Mode(JNIEnv * env,
        jobject obj, jint hostNum, jint sd30Mode, jint sd30MaxCurrent, jint sd30Drive,
        jint sd30PowerControl) {
    ALOGD(
            "MSDC_JNI: hostNum=%d,  sd30Mode = %d, sd30MaxCurrent=%d,  sd30Drive = %d, sd30PowerControl=%d",
            hostNum, sd30Mode, sd30MaxCurrent, sd30Drive, sd30PowerControl);
    if (hostNum > MSDC_MAX_HOST_MUM || sd30Mode > 5 || sd30MaxCurrent > 3 || sd30Drive > 3
            || sd30PowerControl > 1) {
        ALOGD("----error: hostNum or index error.\n");
        return JNI_FALSE;
    }
    int sd_fd, ret;
    struct msdc_ioctl command;
...
JNIEXPORT jint JNICALL Java_com_mediatek_engineermode_io_EmGpio_getCurrent(JNIEnv * env,
        jobject obj, jint hostNum) {
    if (hostNum > MSDC_MAX_HOST_MUM) {
        ALOGD("----error: hostNum or currentIdx too large.\n");
        return -1;
    }
    ...
    if (sd_fd < 0) {
        ALOGD("----error: can't open mt6573_sd----, error code:%d\n", sd_fd);
        return -1;
    }
    ...
    ret = ioctl(sd_fd, -1, (void *) &command);
    if (ret < 0 || -1 == command.result) {
        ALOGD("----error: can't call mt6573_sd----, return:%d, fd:%d\n", ret, sd_fd);
        close(sd_fd);
        return -1;
    }
    ...
    if (dataIdx != 0xFFFF && cmdIdx != 0xFFFF) {
        ALOGD("success: data=%d,  command = %d\r\n", dataIdx, cmdIdx);
        return (cmdIdx << 16) | dataIdx;
    } else {
        ALOGD("MSDC_JNI: error: data=0x%x,  command = 0x%x\r\n", dataIdx, cmdIdx);
        return -1;
    }
}

JNIEXPORT jboolean JNICALL Java_com_mediatek_engineermode_io_EmGpio_setCurrent(JNIEnv * env,
        jobject obj, jint hostNum, jint currentDataIdx, jint currentCmdIdx) {
    ...
    ret = ioctl(sd_fd, -1, (void *) &command);
    if (ret < 0 || -1 == command.result) {
        ALOGD("----error: can't call mt6573_sd()----, error:%d, fd:%d\n", ret, sd_fd);
        close(sd_fd);
        return JNI_FALSE;
    }
