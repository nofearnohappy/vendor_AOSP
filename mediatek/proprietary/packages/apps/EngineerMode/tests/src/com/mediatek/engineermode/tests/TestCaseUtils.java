...
public class TestCaseUtils {
    ...
    private static void printAtLog(Finger[] fingers) {
        Log.d("@M_" + TAG, "generateDownUpEvents log start");
        int count = 0;
        for (int j = 0; j < fingers.length; j++) {
            if (!fingers[j].isLogging) {
                continue;
            }
            Log.d("@M_" + TAG, "Touch pos: " + (int) fingers[j].coords.x + "," + (int) fingers[j].coords.y);
            if (fingers[j].isOnScreen) {
                count++;
            }
        }
        Log.d("@M_" + TAG, "generateDownUpEvents log end, count = " + count);
    }
