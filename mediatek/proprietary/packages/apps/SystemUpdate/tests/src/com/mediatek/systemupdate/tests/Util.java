...
//        return sAvailablePath;
    ...
    static void setAlarm(Context context, int alarmType, long time, String action) {
        Log.i("@M_" + TAG, "setAlarm enter, time = " + time + ", current time = " + Calendar.getInstance().getTimeInMillis());
    ...
    static void cancelAlarm(Context context, String action) {
        Log.i("@M_" + TAG, "cancelAlarm enter, action = " + action + ", current time = "
    ...
    static String getUpdateTypeFromEM() {
        ...
        File f = new File(PathName.UPDATE_TYPE_IN_DATA);
        if (!f.exists()) {
            Log.v("@M_" + TAG, PathName.UPDATE_TYPE_IN_DATA + "doesn's exist");
            return "";
        }
        ...
        try {
            ...
            Log.v("@M_" + TAG, "Get Type from EM is " + type);
    ...
    private synchronized static IMountService getMountService() {
        if (mMountService == null) {
            ...
            if (service != null) {
                ...
                Log.e("@M_" + TAG, "Can't get mount service");
            }
        }
    ...
    static void unmountSdCard(Instrumentation instr, final String mountPoint) {
        ...
        Thread unmountThread = new Thread() {
            @Override
            public void run() {
                Log.d("@M_" + TAG, "unmountVolume:" + mountPoint);
                try {
                    mountService.unmountVolume(mountPoint, true, false);
                } catch (RemoteException e) {
                    // Informative dialog to user that unmount failed.
                    Log.e("@M_" + TAG, "Unmount, RemoteException");
                }
            }
        };
        ...
        try {
            Log.v("@M_" + TAG, "unmountVolume:sleep 15000: " + Thread.currentThread().getName());
            Thread.sleep(15000);
        } catch (Exception e) {
            Log.e("@M_" + TAG, "Thread sleep 15000, broken");
        }
    }
    ...
    static void mountSdCard(Instrumentation instr, final String mountPoint) {
        ...
        Thread mountThread = new Thread() {
            @Override
            public void run() {
                Log.d("@M_" + TAG, "mountVolume" + mountPoint);
                try {
                    mountService.mountVolume(mountPoint);
                } catch (RemoteException e) {
                    // Informative dialog to user that unmount failed.
                    Log.e("@M_" + TAG, "Mount, RemoteException");
                }
            }
        };
        ...
        try {
            Log.v("@M_" + TAG, "mountVolume:sleep 12000: " + Thread.currentThread().getName());
            Thread.sleep(12000);
        } catch (Exception e) {
            Log.e("@M_" + TAG, "Thread sleep 12000, broken");
        }
    }
}
