...
public class MainEntryTests extends ActivityInstrumentationTestCase2<MainEntry> {
     ...
     * null) { Log.e("@M_" + TAG, "isNetWorkAvailable connetManager = null"); return
     ...
     * } catch (Exception e) {
     *
     * e.printStackTrace(); } Log.i("@M_" + TAG, "isNetWorkAvailable result is : " +
     * ret); return ret; }
     */
    ...
    public void testcase03_Requery() {
        Log.i("@M_" + TAG, "testcase03_Requery");
...
//        Assert.assertEquals(2, mActivity.getPreferenceScreen().getPreferenceCount());
    ...
    public void testcase04_LoadFromFile() {
        Log.i("@M_" + TAG, "testcase04_LoadFromFile");
    ...
    public void testcase05_RefreshUIWithOTA() {
        Log.i("@M_" + TAG, "testcase05_RefreshUIWithOTA");

        // OTA package & SD packages
        Log.w("@M_" + TAG, "list OTA package & SD packages");
        mPreference.edit().putInt(OTA_PRE_STATUS, STATE_NEWVERSION_READY).commit();
        mActivity = getActivity();
        waitForUIready(300);

        // One OTA package and one SD package
        Log.w("@M_" + TAG, "One OTA package & One SD packages");
    ...
    public void testcase06_RefreshUINoOTA() {
        Log.i("@M_" + TAG, "testcase06_RefreshUINoOTA");

        // All SD packages
        Log.w("@M_" + TAG, "list All SD packages");
        mPreference.edit().putInt(OTA_PRE_STATUS, STATE_QUERYNEWVERSION).commit();
        mActivity = getActivity();
        waitForUIready(300);

        // No packages
        Log.w("@M_" + TAG, "No packages");
        this.setmUpdateInfoList(null);
        this.invokeRefreshUI();
        waitForUIready(300);

        // Activity is Stopping, pop notification
        Log.w("@M_" + TAG, "Activity is Stopping, pop notification");
    ...
    public void testcase07_RefreshTimeout() {
        Log.i("@M_" + TAG, "testcase06_RefreshTimeout");
        mActivity = getActivity();
    ...
    public void testcase08_InstallingProcess() {
        Log.i("@M_" + TAG, "testcase07_InstallingProcess");
    ...
    public void testcase09_Navigation() {
        Log.i("@M_" + TAG, "testcase08_Navigation");
        mActivity = getActivity();
        mInst.waitForIdleSync();
        waitForUIready(300);
    ...
    public void testcase10_BackKey() {
        Log.i("@M_" + TAG, "testcase09_BackKey");
        mActivity = getActivity();
        mInst.waitForIdleSync();
    ...
    public void testcase11_SdcardUnmount() {
        Log.i("@M_" + TAG, "testcase10_SdcardUnmount");
        mActivity = getActivity();
        waitForNewVersion();
    ...
    public void testcase12_SdcardMount() {
        Log.i("@M_" + TAG, "testcase11_SdcardMount");
        mActivity = getActivity();
        waitForUIready(300);
    ...
    public void testcase13_Shutdown() {
        Log.i("@M_" + TAG, "testcase12_ShutDown");
    ...
    public void testcase14_BootComplete() {
        Log.i("@M_" + TAG, "testcase13_BootComplete");
    ...
    private void invokeRefreshUI() {
        try {
            ...
            Log.v("@M_" + TAG, "[invokeRefreshUI]get refreshUI method");
            m.setAccessible(true);
            m.invoke(mActivity, (Object[]) null);
            Log.v("@M_" + TAG, "[invokeRefreshUI]refreshUI invoked");
            mInst.waitForIdleSync();
    ...
    private void setmUpdateInfoList(List<UpdatePackageInfo> value) {
        try {
            ...
            Log.v("@M_" + TAG, "get Field : mUpdateInfoList");
            f.setAccessible(true);
            f.set(mActivity, value);
            Log.v("@M_" + TAG, "set value");
        } catch (IllegalAccessException e) {
            e.printStackTrace();
        } catch (NoSuchFieldException e) {
            e.printStackTrace();
