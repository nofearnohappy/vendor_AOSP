...
public class SdcardScanner {
    ...
    private static final String LANG_DEV = SystemProperties.get("ro.product.locale");
    ...
    private SdcardScanner(Context context) {
        Log.i("@M_" + TAG, "[SdcardScanner] construction");
        mContext = context;
    ...
    void setMessageHandler(Handler handler) {
        Log.i("@M_" + TAG, "[setMessageHandler], mHandler = " + handler);
        mHandler = handler;
    }

    void resetMessageHandler(Handler handler) {
        Log.i("@M_" + TAG, "[resetMessageHandler], mHandler = " + handler);
        if (mHandler == handler) {
            mHandler = null;
        }
    }
    ...
    void onSdcardCrashed(String crashPath) {
        ...
        if (mCrashPath.equals(mMountedPath)) {
            Log.i("@M_" + TAG, "crashPath equals mountedPath, set mountedPath to null");
            mMountedPath = null;
        }
    }
    ...
    void onSdcardMounted(String mountedPath) {
        ...
        if (mMountedPath.equals(mCrashPath)) {
            Log.i("@M_" + TAG, "mountedPath equals crashPath, set crashPath to null");
            mCrashPath = null;
        }
    }

    private static String getBuildTime() {
        Log.v("@M_" + TAG, "[getBuildTime]");
    ...
    public List<UpdatePackageInfo> querySdcardPackage() {
        ...
        if (pathList == null || pathList.length == 0) {
            ...
            Log.d("@M_" + TAG, "[querySdcardPackage], mUpdateInfo size" + size);
            if (size > 1) {
                ...
                for (UpdatePackageInfo info : mUpdateInfoList) {
                    try {
                        if (mCrashPath != null && info.path.contains(mCrashPath)) {
                            Log.v("@M_" + TAG, "[querySdcardPackage]remove crash path's package:"
                                    + info.path);
                        ...
                        Log.e("@M_" + TAG, "[unpackAndVerify], WriteInfo fail: " + info.path);
                        e.printStackTrace();
                    }
                }
        ...
        if (!isNeedRefresh) {
            ...
            if (mHandler == null) {
                Log.i("@M_" + TAG, "[querySdcardPackage], background, notify user available packages");
        ...
        if (mHandler != null) {
            ...
            Log.i("@M_" + TAG, "[querySdcardPackage], detected done, mHandler is null");
        }
    ...
    private void unpackAndVerify(File updateFile) {
        ...
        if (!unzip(updateFile)) {
            Log.w("@M_" + TAG, "[unpackAndVerify] unzip fail");
        ...
        if (!checkVersion(sdPkgInfoReader)) {
            Log.w("@M_" + TAG, "[unpackAndVerify] checkVersion fail");
            Util.deleteFile(mTempUnzipDir);
            return;
        }
        ...
        if (otaResult != Util.OTAresult.CHECK_OK) {
            Log.w("@M_" + TAG, "[checkUpgradePackage], check_ota result = " + otaResult);
    ...
    private boolean unzip(File updateFile) {
        ...
        if (zipFile == null) {
            Log.w("@M_" + TAG, "[unzip] zipfile is null");
            return false;
        }
    ...
    private boolean checkVersion(PackageInfoReader sdPkgInfoReader) {
        ...
        if (OEM_DEV == null || !OEM_DEV.equals(sdPkgInfoReader.getOem())) {
            Log.w("@M_" + TAG, "not correct oem for this package, oem = " + OEM_DEV);
            mErrorCode = Util.SDresult.ERROR_VERSION_ERR;
            return false;
        }

        if (PRODUCT_DEV == null || !PRODUCT_DEV.equals(sdPkgInfoReader.getProduct())) {
            Log.w("@M_" + TAG, "not correct product for this package, product = " + PRODUCT_DEV);
            mErrorCode = Util.SDresult.ERROR_VERSION_ERR;
            return false;
        }
        ...
        if (!((TextUtils.isEmpty(OPER_DEV) && TextUtils.isEmpty(operPkg)) || OPER_DEV
                .equals(operPkg))) {
            Log.w("@M_" + TAG, "not correct operator for this package, operator = " + OPER_DEV);
            mErrorCode = Util.SDresult.ERROR_VERSION_ERR;
            return false;
        }
...
//                .equals(flavorPkg))) {
//            Log.w("@M_" + TAG, "not correct flavor for this package, flavor = " + FLAVOR_DEV);
//            mErrorCode = Util.SDresult.ERROR_VERSION_ERR;
//            return false;
//        }
        ...
        if (!((TextUtils.isEmpty(LANG_DEV) && TextUtils.isEmpty(langPkg)) || LANG_DEV
                .equals(langPkg))) {
            Log.w("@M_" + TAG, "not correct language for this package, language = " + LANG_DEV);
            mErrorCode = Util.SDresult.ERROR_VERSION_ERR;
            return false;
        }
        ...
        if (time == null || time.compareTo(BUILDTIME_DEV) <= 0) {
            Log.w("@M_" + TAG, "not correct buildTime for this package, buildTime = " + BUILDTIME_DEV);
            mErrorCode = Util.SDresult.ERROR_VERSION_ERR;
            return false;
        }
        return true;
