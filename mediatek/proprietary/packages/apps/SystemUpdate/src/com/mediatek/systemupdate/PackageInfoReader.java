    ...
    public PackageInfoReader(Context context, String file) {
        ...
        if (configFileInSystem.exists()) {
            ...
            try {
                ...
                Log.e("@M_" + TAG, "[PackageInfoReader] SAXException");
            } catch (IOException e) {
                Log.e("@M_" + TAG, "[PackageInfoReader] IOException");
            }
        }
...
//    }
    ...
    public List<UpdatePackageInfo> getInfoList() {
        Log.d("@M_" + TAG, "[getUpdateInfoList] begin");
        ...
        if (mConfigureParser != null) {
            ...
            Log.d("@M_" + TAG, "[getInfoList]node list's length: " + nodeList.size());
    ...
    private UpdatePackageInfo getInfoFromNode(Node node) {
        if (node == null) {
            Log.w("@M_" + TAG, "[getInfoFromNode]node is null");
            return null;
        }

        Log.v("@M_" + TAG, "[getInfoFromNode]node name: " + node.getNodeName());
        NodeList childNodeList = node.getChildNodes(); /* new ArrayList<Node>(); */
        int length = childNodeList.getLength();
        Log.v("@M_" + TAG, "[getInfoFromNode]childNodeList list's length: " + length);

        if (length != UpdatePackageInfo.FIELD_NUMBER) {
            Log.w("@M_" + TAG, "[getInfoFromNode], child node number is not currect");
            return null;
        }
        ...
        for (int i = 0; i < length; ++i) {
            ...
            if (Util.InfoXmlTags.XML_TAG_PUBLISH_TIME.equals(nodeName)) {
                ...
                Log.v("@M_" + TAG, "[getInfoFromNode],Item (" + i + "): [getInfoList]publish time:"
                        + info.publishTime);
                ...
                Log.v("@M_" + TAG, "[getInfoFromNode],Item (" + i + "): [getInfoList]mAndroidNum:"
                        + info.androidNumber);
            } else if (Util.InfoXmlTags.XML_TAG_VERSION_NAME.equals(nodeName)) {
                info.version = child.getTextContent();
                Log.v("@M_" + TAG, "[getInfoFromNode],Item (" + i + "): [getInfoList]version:"
                        + info.version);
            } else if (Util.InfoXmlTags.XML_TAG_NOTES.equals(nodeName)) {
                info.notes = child.getTextContent();
                Log.v("@M_" + TAG, "[getInfoFromNode],Item (" + i + "): [getInfoList] have notes");
            } else if (Util.InfoXmlTags.XML_TAG_PATH.equals(nodeName)) {
                info.path = child.getTextContent();
                Log.v("@M_" + TAG, "[getInfoFromNode],Item (" + i + "): [getInfoList]mPath:" + info.path);
            } else {
                Log.w("@M_" + TAG, "[getInfoFromNode], child node TAG is not currect");
                return null;
            }
        }
