...
public class WifiSdioTestService extends Service {
    ...
    private final BroadcastReceiver mWifiReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (WifiManager.WIFI_STATE_CHANGED_ACTION.equals(action)) {
                ...
                Log.v("@M_" + TAG, "Wifi state changed: " + sWifiState);
            } else if (WifiManager.NETWORK_STATE_CHANGED_ACTION.equals(action)) {
                NetworkInfo info = (NetworkInfo) intent
                        .getParcelableExtra(WifiManager.EXTRA_NETWORK_INFO);
                sWifiDetailedState = info.getDetailedState();
                Log.v("@M_" + TAG, "NETWORK_STATE_CHANGED_ACTION: "
                        + sWifiDetailedState);
            }
        }
    };
    ...
    private class TestHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            case TEST_RUNNING:
                if (STATUS_STOPPED == sStatus) {
                    Log.v("@M_" + TAG, "status is stop");
                    break;
                }
                //If any command CRC error occurs, it will go back to 1st CMD round.
                if (TEST_DATA_READ_ROUND <= sCurrentTestLoop) {
                    if (sCMDErrorOccur) {
                        Log.w("@M_" + TAG, "There is a CMD CRC error in the DATA read round!!!");
                        Log.w("@M_" + TAG, "Back to CMD round!!!");
                        sCurrentTestLoop = 0;
                        sCMDErrorOccur = false;
                    }
                }
                ...
                Log.v("@M_" + TAG, strBuilder.toString());
                if ((sLocalRetry < 2) && (retWifitest == 0)) { //Local retry < 2 and Return CRC error happened
                    if (((sCurrentTestLoop / TEST_ONE_ROUND) == 0) && (CurResult == BYTE_R || CurResult == BYTE_W)) {
                        ...
                        Log.v("@M_" + TAG, "Holmes There is a DATA CRC error in the CMD round");
                    } else if (((sCurrentTestLoop / TEST_ONE_ROUND) == 0) && (CurResult == BYTE_C)) {
                        //Next gear, There is a CMD CRC error in the CMD round.
                        sLocalRetry = 0;
                        sCurrentTestLoop++;
                        Log.v("@M_" + TAG, "Holmes There is a CMD CRC error in the CMD round");
                        ...
                        Log.v("@M_" + TAG, "Holmes There is a DATA write CRC error in the DATA read round");
                        sLocalRetry++;
                        ...
                        Log.v("@M_" + TAG, "Holmes There is a CMD CRC error in the DATA round " + tmpCMDindex);
                        ...
                        sLocalRetry = 0;
                sCurrentTestLoop++;
                        Log.v("@M_" + TAG, "Holmes There is a DATA read CRC error in the DATA read round");
                        ...
                        sLocalRetry = 0;
                        sCurrentTestLoop++;
                        Log.v("@M_" + TAG, "Holmes Warning~~~Warning~~~Warning~~~Warning~~~Warning~~~Warning~~~");
                    }
                } else { //Next gear for no any CRC error appearing
                    sLocalRetry = 0;
                    sCurrentTestLoop++;
                    Log.v("@M_" + TAG, "Holmes There is a NO any CRC error in this round or too more retry");
                }
                if (sCurrentTestLoop == TEST_LOOP_COUNT) {
                    ...
                    if ((TEST_COMMAND_ROUND <= sCurrentTestLoop) && s4CMDRoundFlag && (!s5CMDInterRoundFlag)) {
                        ...
                        Log.v("@M_" + TAG, "Holmes Enter the 5th CMD Internal round.");
                    } else if ((TEST_4CMD_ROUND == sCurrentTestLoop) && !s4CMDRoundFlag) {
                        //Set the 4th CMD round flag here!
                        s4CMDRoundFlag = true;
                        sCurrentTestLoop = 0;
                        Log.v("@M_" + TAG, "Holmes Enter the 4th CMD round.");
                    }
    ...
    public void stopTestRequest() {
        Log.v("@M_" + TAG, "stopTestRequest: " + sStatus);
        if (STATUS_STOPPED != sStatus) {
            sStatus = STATUS_STOPPED;
    ...
    private void createLogFile() {
        ...
        try {
            ...
            Log.w("@M_" + TAG, "createLogFile FileNotFoundException: "
                    + e.getMessage());
        } catch (IOException e) {
            Log.w("@M_" + TAG, "createLogFile IOException: " + e.getMessage());
        } finally {
            if (null != writer) {
                writer.close();
                writer = null;
    ...
    private void appendLog(String log, boolean newLine) {
        ...
        try {
            ...
            Log.w("@M_" + TAG, "appendLog FileNotFoundException: " + e.getMessage());
        } catch (IOException e) {
            Log.w("@M_" + TAG, "appendLog IOException: " + e.getMessage());
        } finally {
            if (null != writer) {
                writer.close();
                writer = null;
    ...
    boolean doWifiTestPrepare() {
        int index;
        if (!disableWifi()) {
            Log.w("@M_" + TAG, "doWifiTestPrepare: init disableWifi fail");
        }
        Log.w("@M_" + TAG, "doWifiTestPrepare: init fill the pass in the TFBuffer");
        ...
        Log.v("@M_" + TAG, "doWifiTestPrepare command: " + commandBuilder.toString());
        ShellResult result = runShellCommand(commandBuilder.toString(), false);
        if (0 != result.mExitValue) {
            Log.w("@M_" + TAG, "doWifiTestPrepare: enable msdc tune fail");
        }
        commandBuilder = null;
    ...
    private int doWifiTest() {
        ...
        if (null == errorMsg) {
            ...
            if (1 == ret)
            {
                ...
                Log.w("@M_" + TAG, TEST_ERROR + errorMsg + ERROR_OCCURS_WHEN
                        + sCurrentTestLoop);
                appendLog(RESULT_E,
        ...
        do {
            if (sCurrentTestLoop < TEST_COMMAND_ROUND) { //1st CMD round
                ...
                result = runShellCommand(strBuilder.toString(), false);
                 ...
                 Log.w("@M_" + TAG, "Warning~~~~Warning~~~~Warning~~~~Warning~~~~" + sCurrentTestLoop);
            }
        } while (false);
        strBuilder = null;
        return 0 == result.mExitValue;
    ...
    private boolean isConnecting() {
        SupplicantState state = mWifiManager.getConnectionInfo()
                .getSupplicantState();
        Log.v("@M_" + TAG, "SupplicantState: " + state);
        switch (state) {
        case AUTHENTICATING:
        case ASSOCIATING:
        case ASSOCIATED:
    ...
    private boolean connectAp() {
        ...
        for (int i = 0; i < WIFI_AUTO_CONNECT_TIMEOUT / WIFI_WAIT_MAX_SLEEP
            ...
            if (isConnecting()) {
                Log.v("@M_" + TAG, "Wifi auto connect");
                bAutoConn = true;
                break;
            }
        }
        ...
        if (!bAutoConn) {
            Log.v("@M_" + TAG, "Wifi manual connect");
            Intent intent = new Intent(WifiAdmService.WIFI_ADM_ACTION);
        ...
        */
        try {
            if (InetAddress.getByName(sServerIp).isReachable(WIFI_WAIT_MAX_SLEEP * WIFI_PING_TIME)) {
                Log.v("@M_" + TAG, " ip connect");
                result = true;
            }
        } catch (UnknownHostException e) {
             Log.v("@M_" + TAG, "ip UnknownHostException: " + e.getMessage());
        } catch (IOException e) {
            Log.v("@M_" + TAG, "ip IOException :" + e.getMessage());
        }
        return result;
    }
    ...
    private int checkErrorFlag(int mode) {
        ...
        Log.v("@M_" + TAG, "checkErrorFlag command: " + strBuilder.toString());
        ShellResult shellResult = runShellCommand(strBuilder.toString(), true);
        Log.v("@M_" + TAG, "checkErrorFlag result: " + shellResult.mOutput);
        strBuilder = null;
        ...
        try {
            if (null == shellResult.mOutput) {
                ...
                result = RESULT_N;
                ret = 0;
                Log.w("@M_" + TAG, "checkErrorFlag NULL output!!!");
                if (mode == 0)
                {
            ...
            Log.w("@M_" + TAG, "checkErrorFlag NumberFormatException: "
                    + e.getMessage());
            result = RESULT_N;
        } catch (IndexOutOfBoundsException e) {
            Log.w("@M_" + TAG, "checkErrorFlag IndexOutOfBoundsException: "
                    + e.getMessage());
            result = RESULT_I;
        } finally {
            if (mode == 0)
    ...
    private boolean waitWifiReady(int wifiState) {
        ...
        for (; i < WIFI_WAIT_TIME / WIFI_WAIT_MAX_SLEEP
            ...
            Log.v("@M_" + TAG, "waitWifiReady " + sWifiState);
            break;
        }
        if (i == WIFI_WAIT_TIME / WIFI_WAIT_MAX_SLEEP) {
            Log.w("@M_" + TAG, "waitWifiReady time out");
        } else if (STATUS_STOPPED == sStatus) {
            Log.w("@M_" + TAG, "waitWifiReady stopped");
        }
        return bSuccess;
    }
    ...
    private boolean waitWifiConnect(DetailedState connected) {
        ...
        if (i == WIFI_WAIT_TIME / WIFI_WAIT_MAX_SLEEP) {
            Log.w("@M_" + TAG, "waitWifiConnect time out");
        } else if (STATUS_STOPPED == sStatus) {
            Log.w("@M_" + TAG, "waitWifiConnect stopped");
        }
        return bSuccess;
    }
    ...
    private void calcSdioConfigParam() {
        ...
        while ((BYTE_0 == sResultTFBuffer.get(sCurrentTestLoop)) && (0 == sLocalRetry)) {
            ...
            if (excep > TEST_LOOP_COUNT) {
                Log.w("@M_" + TAG, "Error!!! Error!!! Error!!! Error!!! Error!!!");
                break;
            }
        }
        ...
        case 0:
            if (0 == current) {
                ...
                if (0 == group_mod) { //Add this code for back to CMD round, use the better DATA window
                    ...
                    Log.w("@M_" + TAG, "Holmes Better Read DATA :" + sReadEdge + " " + sReadDelay);
                    Log.w("@M_" + TAG, "Holmes Better Write DATA :" + sWriteEdge + " " + sWriteDelay);
                }
            }
            sCmdDelay = current;
            break;
        case 1:
            if (0 == current) {
                ...
                if ((0 == group_mod) && (!sCmdDefaultEnable)) {
                    ...
                    Log.w("@M_" + TAG, "Holmes Better CMD :" + sCmdEdge + " " + sCmdDelay);
                    sCMDErrorOccur = false;
                }
            }
            sReadDelay = current;
    ...
    private int[] findContiSuccess(int indexStart) {
        ...
        while (it.hasNext()) {
            temp = it.next();
            Log.v("@M_" + TAG, Arrays.toString(temp));
    ...
    private ShellResult runShellCommand(String command, boolean recordOutput) {
        Log.v("@M_" + TAG, "runShellCommand: " + command);
        ...
        try {
            ...
            Log.d("@M_" + TAG, "exit value: " + exitValue);
            ...
            e.printStackTrace();
            Log.w("@M_" + TAG, "runShellCommand InterruptedException: "
                    + e.getMessage());
        } catch (IOException e) {
            e.printStackTrace();
            Log.w("@M_" + TAG, "runShellCommand IOException: " + e.getMessage());
        } finally {
            if (null != reader) {
                try {
                    reader.close();
                    ...
                    e.printStackTrace();
                    Log.w("@M_" + TAG, "reader close IOException: " + e.getMessage());
                }
            }
            if (null != process) {
                process.destroy();
