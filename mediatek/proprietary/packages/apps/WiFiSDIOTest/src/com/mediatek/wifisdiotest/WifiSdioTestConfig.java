...
public class WifiSdioTestConfig extends Activity implements OnClickListener,
    ...
    private final BroadcastReceiver mTestUpdateReceiver = new BroadcastReceiver() {
        ...
        @Override
        public void onReceive(Context context, Intent intent) {
            Log.v("@M_" + TAG, "mTestUpdateReceiver onReceive");
            String action = intent.getAction();
            ...
            case HANDLER_ID_UPDATE_AP_LIST:
                if (View.VISIBLE == mPbWaitBar.getVisibility()) {
                    ...
                    while (i.hasNext()) {
                        ...
                        Log.d("@M_" + TAG, "index: " + index + scanResult.toString());
                        index++;
        ...
        public void handleMessage(Message msg) {
            ...
            case HANDLER_ID_INIT_DEVICE:
                ...
                if (btAdapter.getState() != BluetoothAdapter.STATE_OFF) {
                    // Close bluetooth
                    Log.v("@M_" + TAG, "Close BT");
                    btAdapter.disable();
                }
                if (isGPSEnable()) {
                    // close GPS
                    Log.v("@M_" + TAG, "Close GPS");
                    closeGPS();
                }
                break;
            default:
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (null == mWifiManager) {
            Log.e("@M_" + TAG, "Wifi Manager is null");
    ...
    protected void onStart() {
        ...
        Log.v("@M_" + TAG, "send broadcast ACTION_TOFMSERVICE_POWERDOWN");
    ...
    private final Handler mFindDelayHandler = new Handler() {
        ...
        private void configParameter() {
            ...
            if (mCurrentCount < 64) {
                ...
                Log.w("@M_" + TAG, "Warning~~~~Warning~~~~Warning~~~~Warning~~~~" + mCurrentCount);
            }
            if (mCurrentCount != 0) {
                if (meter != null)
        ...
        private void fillResult(int curCount, Boolean isOK)
        {
            if (curCount < GROUP_GAP) {
                ...
                Log.w("@M_" + TAG, "Warning~~~~Warning~~~~Warning~~~~Warning~~~~" + curCount);
            }
        }
        ...
        public void handleMessage(Message msg) {
            ...
            case HANDLER_ID_CHECK_AP:
                if (selectResult()) {
                    ...
                    Log.w("@M_" + TAG, "remove dialog");
                    break;
                }
                if (mCurrentCount == 1) {
                    showDialog(DIALOG_AP_SEARCH);
                }
                 Log.w("@M_" + TAG, "config parmeter");
                configParameter();
            ...
            case HANDLER_ID_AP_FOUND:
                ...
                Log.d("@M_" + TAG, "[GETAP_HANDLER] find ap, count =" + mCurrentCount);
            ...
            case HANDLER_ID_CONNECT_ERROR:
                ...
                this.sendEmptyMessage(HANDLER_ID_CHECK_AP);
                mCurrentCount++;
                Log.d("@M_" + TAG, "[GETAP_HANDLER] Can't get Handler Error Issue");
                break;
            default:
                break;
            }
    ...
    public void onClick(View v) {
        if (mTvApSelect.getId() == v.getId()) {
            Log.v("@M_" + TAG, "onClick: refresh clicked");
            ...
            Log.v("@M_" + TAG, "onClick: start clicked");
            if (validateInput()) {
            ...
            Log.v("@M_" + TAG, "onClick: stop clicked");
            if (null != mService) {
    ...
    private void updateTestConfig() {
        Log.v("@M_" + TAG, "update test config");
        if (null != mService) {
    ...
    private void updateTestUi(boolean bRunning) {
        Log.v("@M_" + TAG, "updateTestUi: " + bRunning);
    ...
    private final ServiceConnection mServiceCon = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            Log.v("@M_" + TAG, "onServiceConnected");
        ...
        public void onServiceDisconnected(ComponentName name) {
            Log.v("@M_" + TAG, "onServiceDisconnected");
            mService = null;
        }
    };
}
