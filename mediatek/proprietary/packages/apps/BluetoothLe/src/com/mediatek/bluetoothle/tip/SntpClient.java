...
public class SntpClient {
     ...
     */
    public int requestTime(final String host, final int timeout) {
        ...
        try {
            ...
            if (originateTime <= 0) {
                Log.i(TAG, "originateTime: " + originateTime);
            ...
            Log.e(TAG, "Timeout: " + e);
            return ReferenceTimeUpdateService.RESULT_TIMEOUT;
        } catch (final UnknownHostException e) {
            Log.e(TAG, "No Connection: " + e);
            return ReferenceTimeUpdateService.RESULT_NO_CONNECTION;
        } catch (final IOException e) {
            Log.e(TAG, "Error: " + e);
            return ReferenceTimeUpdateService.RESULT_ERROR;
        } finally {
            if (socket != null) {
                socket.close();
            }
        }
        return ReferenceTimeUpdateService.RESULT_SUCCESS;
    }
     ...
     */
    private long read32(final byte[] buffer, final int offset) {
     ...
     * 1, 1970).
     */
    private long readTimeStamp(final byte[] buffer, final int offset) {
     ...
     */
    private void writeTimeStamp(final byte[] buffer, int offset, final long time) {
