...
public class DPInstallInactiveExecHandler extends ScomoExecHandler implements NodeExecuteHandler {
    ...
    public int execute(byte[] data, String correlator) throws MdmException {
        ...
        mScomo.getEngine().setSessionAction(MdmScomo.SESSION_ACTION_KEY,
                ScomoAction.INSTALL_INACTIVE_EXECUTED);
        ...
        if (handler != null) {
            if (handler.confirmInstall(mDp)) {
                ...
                if (result.mAsync) {
                    ...
                    saveExecInfo(correlator, MdmTree.makeUri(mDp.getDeliveredUri(), Uri.OPERATIONS,
                            Uri.INSTALLINACTIVE));
                    mScomo.logMsg(MdmLogLevel.DEBUG, "-DPInstallInactiveExecHandler.execute()");
                    return 0;
                } else {
                ...
                saveExecInfo(correlator,
                        MdmTree.makeUri(mDp.getDeliveredUri(), Uri.OPERATIONS, Uri.INSTALLINACTIVE));
