...
public class DLDownloadInstallInactiveExecHandler extends ScomoExecHandler implements
        NodeExecuteHandler {
    ...
    public int execute(byte[] data, String correlator) throws MdmException {
        ...
        saveExecInfo(correlator,
                MdmTree.makeUri(mDp.getDownloadUri(), Uri.OPERATIONS, Uri.DOWNLOADINSTALLINACTIVE));
        ...
        SessionInitiator initiator = new SimpleSessionInitiator(Initiator.makePath(Initiator.DL,
                mDp.getName(), Uri.DOWNLOADINSTALLINACTIVE));
        mScomo.mPendingSession.add(initiator);
        engine.triggerDLSession(mDp.getUrl(), new DPDownloadPromptHandler(mScomo, mDp), initiator);
        // Set action bits.
        engine.setSessionAction(MdmScomo.SESSION_ACTION_KEY,
                ScomoAction.DOWNLOAD_INSTALL_INACTIVE_EXECUTED);
