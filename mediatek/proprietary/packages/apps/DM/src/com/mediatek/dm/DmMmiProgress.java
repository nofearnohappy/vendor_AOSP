...
public class DmMmiProgress implements MmiProgress {
    ...
    public void update(int current, int total) {
        ...
        if (DmService.sSessionType == DmService.SESSION_TYPE_FUMO) {
            Handler fotaHandler = DmService.getInstance().mSessionStateHandler;
            if (fotaHandler != null) {
                fotaHandler.sendMessage(fotaHandler.obtainMessage(IDmPersistentValues.STATE_DOWNLOADING, current, total));
            } else {
                Log.e(TAG.CLIENT, "[DmMmiProgress] update, fotaHandler is null");
            }
        } else if (DmService.sSessionType == DmService.SESSION_TYPE_LAWMO) {
