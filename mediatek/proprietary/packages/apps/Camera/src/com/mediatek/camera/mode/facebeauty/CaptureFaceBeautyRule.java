...
public class CaptureFaceBeautyRule implements ISettingRule {
    ...
    public CaptureFaceBeautyRule(ICameraContext cameraContext) {
        Log.i(TAG, "[CaptureFaceBeautyRule]constructor...");
    ...
    public void execute() {
        ...
        if (index == -1) {
            ...
            Record record = setting.getOverrideRecord(SettingConstants.KEY_FACE_BEAUTY);
            if (record == null) {
                return;
            }

            setting.removeOverrideRecord(SettingConstants.KEY_FACE_BEAUTY);
            ...
            SettingItem setting = mISettingCtrl
                    .getSetting(SettingConstants.KEY_SLOW_MOTION);
            mLastValue = setting.getValue();
            Log.i(TAG, "KEY_SLOW_MOTION mLastValue = " + mLastValue);
            if (VALUE_ON.equals(mLastValue)) {
                setting.setValue(VALUE_ON);
            }
            ...
            Record record = setting.new Record("off",
                    SettingUtils.RESET_STATE_VALUE_DISABLE);
            setting.addOverrideRecord(SettingConstants.KEY_FACE_BEAUTY, record);
        }
