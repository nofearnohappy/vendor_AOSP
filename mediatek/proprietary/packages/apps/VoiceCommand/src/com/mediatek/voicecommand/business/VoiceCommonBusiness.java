...
public class VoiceCommonBusiness extends VoiceCommandBusiness {
    private static final String TAG = "VoiceCommonBusiness";

    public VoiceCommonBusiness(IMessageDispatcher dispatcher, ConfigurationManager cfgMgr,
            Handler handler) {
        super(dispatcher, cfgMgr, handler);
        Log.i(TAG, "[VoiceCommonBusiness]new...");
    }
    ...
    private int sendProcessCommandPath(VoiceMessage message) {
        Log.d(TAG, "[sendProcessCommandPath]...");
        int errorid = VoiceCommandListener.VOICE_NO_ERROR;
    ...
    private int sendProcessState(VoiceMessage message) {
        Log.d(TAG, "[sendProcessState]...");
        int errorid = VoiceCommandListener.VOICE_NO_ERROR;

        boolean isEnalbe = mCfgMgr.isProcessEnable(message.mPkgName);
        message.mExtraData = DataPackage.packageResultInfo(
                VoiceCommandListener.ACTION_EXTRA_RESULT_SUCCESS, isEnalbe, null);
        mDispatcher.dispatchMessageUp(message);
