...
public class Configuration {
    ...
    public String readMCCMNCFromProperty() {
        ...
        if (MCCMNC != null && MCCMNC.length() > 4) {
            Log.d("@M_" + Common.LOG_TAG, "read mcc mnc property from boot anim: " + PROP_PERSIST_BOOTANIM_MNC);
            return MCCMNC;
        }
        // failed to read bootanim property, then try other property
        MCCMNC = SystemProperties.get(PROP_GSM_SIM_OPERATOR_NUMERIC);
        if (MCCMNC != null && MCCMNC.length() > 4) {
            Log.d("@M_" + Common.LOG_TAG, "read mcc mnc property from " + PROP_GSM_SIM_OPERATOR_NUMERIC);
            return MCCMNC;
        }

        Log.d("@M_" + Common.LOG_TAG, "failed to read mcc mnc from property");
        return "";
    }
