...
public final class ContactsProxy extends ContextBasedProxy {
    ...
    /**
     * @param sourceLocation int
     * @return int
     */
    public static int getSimIdBySourceLocation(int sourceLocation) {
        if (sourceLocation == RawContact.SOURCE_PHONE) {
            return sourceLocation;
        }
        int realSlotId = getRealSlotId(sourceLocation);
        ...
        try {
            ...
            if (uri != null) {
                ...
                if (result < 0) {
                    ...
                    Debugger.logI(new Object[] { name, number, sourceLocation },
                            "Successed to insert SIM contact, index is "
                            + result);
                }
            }
     ...
     *         insertion fail.
     *         New id of all data will be set in the RawContact object.
     */
    public long insertContact(final RawContact newContact,
        ...
        if (realSlotId == -1) {
            ...
            values.put(RawContacts.ACCOUNT_NAME, USIMUtils
                    .getSimAccountNameBySimId(info
                            .getSimId()));
            values.put(RawContacts.ACCOUNT_TYPE, USIMUtils
                    .getAccountTypeBySimId(info
                            .getSimId()));
        }
        values.put(RawContacts.SEND_TO_VOICEMAIL, newContact
                .isSendToVoicemail());
...
//            int sourceLocation = newContact.getSourceLocation();
            SimDetailInfo info = Global.getSimInfoBySlot(realSlotId);
            int sourceLocation = info.getSimId();
        ...
        case RawContact.SOURCE_PHONE:
            values.put(RawContactsContent.COLUMN_INDEX_IN_SIM, -1);
            break;
        default:
            Debugger.logW(new Object[] { newContact, withData },
     ...
     * @return How many contacts are updated.
     */
    public int updateContact(final long id, int sourceLocation,
        ...
        sourceLocation = getSimIdBySourceLocation(sourceLocation);
...
//            // / For Contact provider's change, clear all names except DisplayName
            ...
            default:
                Debugger.logW(new Object[] {
                        id, sourceLocation, simName, simNumber, newContact, updatePIMData },
                        "Contact source location can not be changed currently.");
            }
            // // / @}
            // Operations batch has reached the max count,
    ...
    public int updateContact(final long id, int sourceLocation,
        ...
        for (final ContactData data : allData) {
            ...
            // Ensure the rawContactId in data is correct
            data.setRawContactId(id);
            // / For Contact provider's change,
            // clear all names except DisplayName before update structuredName {@ 2012-12-5
     ...
     */
    public boolean[] deleteContacts(final long[] ids,
            final boolean permanently, int sourceLocation,
            final String[] simNames, final String[] simNumbers,
            final String[] emails) {
        sourceLocation = getSimIdBySourceLocation(sourceLocation);
        switch (sourceLocation) {
    ...
    public boolean[] deleteContactsSourcedOnSim(final long[] ids,
        ...
        for (int i = 0; i < ids.length; i++) {
            final int count = deleteSimContact(
                    simNames[i], simNumbers[i], sourceLocation); // Modified
            // by
            // Shaoying
            // Han
            // getSimId(sourceLocation));
...
//                int sourceLocation = newContact.getSourceLocation();
                boolean isUSIM = USIMUtils.isSimUsimType(contactsSimId[i]);
                if (isUSIM) {
                    simInsertionResult = insertSimContact(newSimName, newSimNumber,
                            newSimEmail, contactsSimId[i]);
                } else {
                    simInsertionResult = insertSimContact(newSimName, newSimNumber,
                            contactsSimId[i]);
                }
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (sourceLocation == RawContact.SOURCE_PHONE) {
                ...
                accountName = USIMUtils
                        .getSimAccountNameBySimId(sourceLocation);
                accountType = USIMUtils
                        .getAccountTypeBySimId(sourceLocation);
