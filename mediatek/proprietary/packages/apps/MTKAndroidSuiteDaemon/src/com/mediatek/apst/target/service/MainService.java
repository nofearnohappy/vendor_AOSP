...
//                R.drawable.stat_pcsync_tool, text, System.currentTimeMillis());
        Notification notification = new Notification();
        notification.icon = R.drawable.ic_pcsync_notifcation;
            ...
            if (cmd instanceof GreetingReq) {
                ...
                if (Config.MTK_GEMINI_SUPPORT) {
                    ...
                    rsp.setSim4Info(Global
                            .getSimInfoBySlot(SimDetailInfo.SLOT_ID_FOUR));
                    if (rsp.isSim1Accessible()) {
                        detailInfoList.add(rsp.getSim1Info());
                    }
                    if (rsp.isSim2Accessible()) {
                        detailInfoList.add(rsp.getSim2Info());
                    }
                    if (rsp.isSim3Accessible()) {
                        detailInfoList.add(rsp.getSim3Info());
                    }
                    if (rsp.isSim4Accessible()) {
                        detailInfoList.add(rsp.getSim4Info());
                    }
                    ...
                    //2013-01-30 for Gemini plus 2.0
                    SimDetailInfo detailInfo = Global.getSimInfoBySlot(
                            SimDetailInfo.SLOT_ID_SINGLE);
                    detailInfo.setAccessible(mSysInfoProxy.isSimAccessible());
...
//                rsp.setSimInfoList(Global.getAllSIMList());
                rsp.setSimInfoList(detailInfoList);
...
//                    rsp.setStatusCode(ResponseCommand.SC_FAILED);
                    rsp.setStatusCode(updateGroupCount);
                }
                // Send response command or append it to batch
                onRespond(rsp);
...
//                	rsp.setStatusCode(ResponseCommand.SC_FAILED);
                	rsp.setStatusCode(updateRawContactCount);
                }
                // Send response command or append it to batch
                onRespond(rsp);
...
//                    rsp.setStatusCode(ResponseCommand.SC_FAILED);
                    rsp.setStatusCode(updateDetailedContactCount);
                } else {
...
//                    rsp.setStatusCode(ResponseCommand.SC_FAILED);
                    rsp.setStatusCode(updateSimContactCount);
                }
                // Send response command or append it to batch
                onRespond(rsp);
...
//                    rsp.setStatusCode(ResponseCommand.SC_FAILED);
                    rsp.setStatusCode(updateContactDataCount);
                }
                // Send response command or append it to batch
                onRespond(rsp);
