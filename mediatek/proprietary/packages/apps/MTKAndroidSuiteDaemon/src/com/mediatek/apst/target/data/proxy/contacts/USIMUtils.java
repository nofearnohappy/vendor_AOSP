...
public class USIMUtils {
    ...
    public static final int GEMINI_SLOT1 = com.android.internal.telephony.PhoneConstants.SIM_ID_1;
    public static final int GEMINI_SLOT2 = com.android.internal.telephony.PhoneConstants.SIM_ID_2;
    public static final int GEMINI_SLOT3 = com.android.internal.telephony.PhoneConstants.SIM_ID_3;
    public static final int GEMINI_SLOT4 = com.android.internal.telephony.PhoneConstants.SIM_ID_4;
    ...
    public interface SimSlot {
        int SLOT_NONE = -1;
        int SLOT_SINGLE = 0;
        int SLOT_ID1 = com.android.internal.telephony.PhoneConstants.SIM_ID_1;
        int SLOT_ID2 = com.android.internal.telephony.PhoneConstants.SIM_ID_2;
        int SLOT_ID3 = com.android.internal.telephony.PhoneConstants.SIM_ID_3;
        int SLOT_ID4 = com.android.internal.telephony.PhoneConstants.SIM_ID_4;
    }
...
//        final ITelephony iTel = ITelephony.Stub.asInterface(ServiceManager
//                .getService(Context.TELEPHONY_SERVICE));
    	TelephonyManager telephonyManager = (TelephonyManager) Global.sContext.getSystemService(Context.TELEPHONY_SERVICE);
        if (telephonyManager == null) {
            Debugger.logE(new Object[] { slotId }, "telephonyManager is null, may be phone service hasn't int done!");
            return false;
        }
        boolean isSimInsert = false;
        if (telephonyManager != null) {
        	if (Config.MTK_GEMINI_SUPPORT) {
        		isSimInsert = telephonyManager.hasIccCard(slotId);
        	} else {
        		isSimInsert = telephonyManager.hasIccCard(0);
        	}
        }
        return isSimInsert;
    }
     ...
     * @return String
     */
    public static String getSimAccountNameBySimId(int simId) {
        Debugger.logI("getSimAccountNameBySimId()+ simId:" + simId);
        String accountName = ACCOUNT_NAME_SIM;
        if (isSimUsimType(simId)) {
            accountName = "USIM" + simId;
        }
        Debugger.logI("getSimAccountNameBySimId()+ accountName:" + accountName);
        return accountName;
    }
     ...
     * @return String
     */
    public static String getAccountTypeBySimId(int simId) {
        Debugger.logI("getAccountTypeBySimId()+ simId:" + simId);
        String accountType = ACCOUNT_TYPE_SIM;
        if (isSimUsimType(simId)) {
            accountType = ACCOUNT_TYPE_USIM;
        }
        Debugger.logI("getAccountTypeBySimId()+ accountType:" + accountType);
        return accountType;
    }
...
//                .getService(Context.TELEPHONY_SERVICE));
        ...
        try {
            if (sourceLoacation == 0) {
                ...
                if (SimType.SIM_TYPE_USIM_TAG.equals(iTel
                        .getIccCardType(sourceLoacation ))) {
                    isUsim = true;
                }
            } else {
    ...
    public static int hasExistGroup(int slotId, String grpName)
        ...
        List<UsimGroup> uList = iIccPhb.getUsimGroups(slotId);
        if (null != uList)
        {
    ...
    public static int createUSIMGroup(int slotId, String name)
        ...
        int grpId = -1;
        if (iIccPhb != null) {
            grpId = iIccPhb.insertUsimGroup(slotId, name);
        }
    ...
    public static int updateUSIMGroup(int slotId, int nGasId, String name)
        ...
        int grpId = -1;
        if (iIccPhb != null) {
            grpId = iIccPhb.updateUsimGroup(slotId, nGasId, name);
        }
    ...
    public static int deleteUSIMGroup(int slotId, String name) {
        ...
        try {
            ...
            if (grpId > 0) {
                if (iIccPhb.removeUsimGroupById(slotId, grpId)) {
    ...
    public static boolean addUSIMGroupMember(int slotId, int simIndex, int grpId) {
        ...
        try {
            if (grpId > 0) {
                final IIccPhoneBook iIccPhb = getIIccPhoneBook(slotId);
                if (iIccPhb != null) {
                    succFlag = iIccPhb.addContactToGroup(slotId, simIndex, grpId);
    ...
    public static boolean deleteUSIMGroupMember(int slotId, int simIndex,
        ...
        try {
            if (grpId > 0) {
                final IIccPhoneBook iIccPhb = getIIccPhoneBook(slotId);
                if (iIccPhb != null) {
                    succFlag = iIccPhb.removeContactFromGroup(slotId, simIndex, grpId);
                    succFlag = true;
                }
            }
        } catch (android.os.RemoteException e) {
