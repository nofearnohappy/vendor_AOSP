...
import com.mediatek.bluetooth.IBluetoothSimap;
import com.mediatek.bluetooth.IBluetoothSimapCallback;
...
public class BluetoothSimapSettings 
	...
	private int mMaxSimNum = 2;
	private final static int MSG_SIM_STATE_CHECK = 100;
	private int[] mEntryValueIndex = new int[4];
	...
	private BroadcastReceiver mReceiver = new BroadcastReceiver() {
		public void onReceive(Context context, Intent intent) {
			...
			if(action.equals(BluetoothAdapter.ACTION_STATE_CHANGED)){
				...
				Log.d(TAG,"receiver: TelephonyIntents.ACTION_SIM_STATE_CHANGED");
                ...
                Log.d(TAG, "slotid is " +slotId + "status is "+ iccState);
				if (SystemProperties.get("ro.mtk_gemini_support").equals("1")) {
					mSimapHandler.sendEmptyMessage(MSG_SIM_STATE_CHECK);
				}
			}
	...
	public void onCreate(PreferenceActivity parentActivity) {
		...
		mMaxSimNum = mTelephonyManager.getPhoneCount();
		...
		Intent intent = new Intent(IBluetoothSimap.class.getName());
		intent.setComponent(new ComponentName("com.mediatek.bluetooth", "com.mediatek.bluetooth.simap.BluetoothSimapService"));
		boolean bindSimap = this.parentActivity.bindService(intent, mSimapServerConn, Context.BIND_AUTO_CREATE);
		if (!bindSimap) {
	...
	public void onDestroy() {
        ...
        this.parentActivity.unregisterReceiver(mReceiver);
			...
			try {
				...
				Log.w(TAG, "[BT][SIMAP] Exception triggered when unbinding service: " + ex1);
			} catch (RemoteException ex2) {
				Log.e(TAG, "unregisterCallback(mSimapUICallback) error: " + ex2);
			}
	}
