...
import com.mediatek.bluetooth.IBluetoothFtpCtrl;
import com.mediatek.bluetooth.IBluetoothFtpServer;
import com.mediatek.bluetooth.IBluetoothFtpServerCallback;
...
public class BluetoothFtpService extends Service {
	...
	private BroadcastReceiver mSDRemovalReceiver = new BroadcastReceiver() {
		public void onReceive(Context context, Intent intent) {
			boolean MTK_2SDCARD_SWAP = "1".equals(SystemProperties.get("ro.mtk_2sdcard_swap"));
			boolean MTK_EMMC_SUPPORT = "1".equals(SystemProperties.get("ro.mtk_emmc_support"));
			int msg_flag = 0;
	...
	private synchronized boolean clearService() {
		...
		while (mClientState != BluetoothFtp.BT_FTPC_STATE_ACTIVE ||
			...
			try {
				...
				printErr("Waiting for client disconnect-ind was in illegal thread state.");
 ...
 ********************************************************************************************/
	...
	private void updateActivation(int role, int state) {
		...
		profile = (role == FTP_SERVER) ?
				BluetoothProfileManager.Profile.FTP_SERVER :
				BluetoothProfileManager.Profile.FTP_CLIENT;
