...
*/  
    ...
    private class BppListener extends Thread {
        ...
        public void run() {
            ...
            Log.i("@M_" + TAG, "[BPP Server] listen thread run......");
            BluetoothBppServer.this.startListenNative();
            Log.i("@M_" + TAG, "[BPP Server] listen thread stopped......");
        }
...
//        }
    ...
    public void disableService(){
        ...
        if (mNativeData != 0 )
        {
            try {
                ...
                Log.w("@M_" + TAG, "BppServer mListener close error");
            }
    ...
    public void bppGetPrinterAttr(String address, int bitmask_attr){
        Log.i("@M_" + TAG, "+bppGetPrinterAttr");
	if (mNativeData != 0 )
	{
    ...
    public void bppPrint(String address, PrintObject print_object){
        ...
        Log.i("@M_" + TAG, " filePath: " + print_object.DirName + "\tfileName: " + print_object.FileName +
...
----------------------------------------*/
    ...
    //should there are two handlers? One for server and one for client
        ...
        switch(event)
        {
            ...
            {
                Log.i("@M_" + TAG, "BPP_ENABLE_SUCCESS");
                int cnfCode = 0;
            ...
            {
                Log.i("@M_" + TAG, "BPP_DISABLE_SUCCESS");
                int cnfCode = 0;
            ...
            {
                Log.i("@M_" + TAG, "BPP_CONNECT_SUCCESS");
                int cnfCode = 0;
            ...
            {
                Log.i("@M_" + TAG, "BPP_DISCONNECT_SUCCESS");
                int cnfCode = 0;
            ...
            { 
                Log.i("@M_" + TAG, "BPP_DISCONNECT_FAIL");
                int cnfCode = -1;
                mCallback.obtainMessage(BluetoothBppManager.MSG_ON_BPP_DISCONNECT_CNF, cnfCode, 0).sendToTarget();
            }
            break;
            ...
            {
                Log.i("@M_" + TAG, "BPP_CANCEL_SUCCESS");
                int cnfCode = 0;
            ...
            {
                Log.i("@M_" + TAG, "BPP_GET_PRINT_ATTR_SUCCESS");
                ...
                Log.i("@M_" + TAG, "sides: " + sides + "\torientations: " + orientations + "\tqualities: " + qualities +
            ...
            {
                Log.i("@M_" + TAG, "BPP_GET_PRINT_ATTR_FAIL");
                int cnfCode = -1;
            ...
            case BPP_PROGRESS:
            {
                Log.i("@M_" + TAG, "BPP_PROGRESS");
                int sentDataLen = Integer.parseInt(parameters[0]);
                int totalDataLen = Integer.parseInt(parameters[1]);

                Log.i("@M_" + TAG, "current: "+ mCurSentLength );
                Log.i("@M_" + TAG, "present: "+ mPreSentLength );
            ...
            {
                Log.i("@M_" + TAG, "BPP_PRINT_COMPLETE_SUCCESS");
                int cnfCode = 0;
            ...
            {
                Log.i("@M_" + TAG, "BPP_PRINT_COMPLETE_FAIL");
                int cnfCode = -1;
                mCallback.obtainMessage(BluetoothBppManager.MSG_ON_BPP_PRINT_CNF, cnfCode, 0).sendToTarget();
            }
            break;
...
----------------------------------------*/
    private String[] translateBitMap(byte bitmap, String[] defaultentries) {
         ...
         for (byte i=0; i < ele_number; i++){
             ...
             Log.i("@M_" + TAG, "listEntries: " + listEntries[i]);
         }
         return listEntries;
    }
