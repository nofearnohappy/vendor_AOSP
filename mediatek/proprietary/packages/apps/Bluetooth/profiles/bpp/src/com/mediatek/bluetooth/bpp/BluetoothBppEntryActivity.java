...
public class BluetoothBppEntryActivity extends Activity {
    ...
    private void getFileInfoMain(){
        if (mFileUri != null && mMimeType != null) {
            Log.v("@M_" + TAG, "Get ACTION_SEND intent: Uri = " + mFileUri + "; mimetype = " + mMimeType);
		...
		Log.e("@M_" + TAG, "mFileOperThread close error.");
	    }
	    Log.v("@M_" + TAG, "result = "+result);
            if (result != 0) {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (mAdapter == null) {
            Log.v("@M_" + TAG, "bluetooth service is not started! ");
            finish();
            return;
        }
        ...
        if (mAdapter.isEnabled()){
            Log.v("@M_" + TAG, "bluetooth service is available");
            getFileInfoMain();
            //if (startBppManager()) { 
            ...
            Log.v("@M_" + TAG, "bluetooth service is not available! ");
            Log.v("@M_" + TAG, "turning on bluetooth......");
    ...
    public void onBackPressed() {
		...
		if(!mBack){
			Log.v("@M_" + TAG, "mBack is false");
        	super.onBackPressed();
		}
		else{
			Log.v("@M_" + TAG, "mBack is true");
		}
    }
    ...
    private int getFileInfo () {
        ...
        if ( mMimeType.equals("text/x-vcard")) {
            ...
            try {
                ...
                if ( null == in ) {
   	            Log.e("@M_" + TAG, "input stream is null");
                    //return 3;
                    return 1;
                }
                ...
                Log.v("@M_" + TAG, "File Path: " + mFilePath);
                Log.v("@M_" + TAG, "File Size: " + mFileSize);
                Log.v("@M_" + TAG, "File Mime: " + mMimeType);
                return 0;
            }
            catch( Exception ex ){
                Log.e("@M_" +  TAG, "create v-card file fail: " + ex.getMessage() );
                return 1;
            }
		    ...
		    if(!cursor.moveToFirst()){						
				Log.e("@M_" + TAG, "cursor.moveToFirst() is null");
				return 2;
		        }
...
/*
    private boolean startBppManager() {
        Log.v("@M_" + TAG, "Start Bpp Manager!");
...
*/
    ...
    private void startDevicePicker(){
        Log.v("@M_" + TAG, "Start Device Picker!");
