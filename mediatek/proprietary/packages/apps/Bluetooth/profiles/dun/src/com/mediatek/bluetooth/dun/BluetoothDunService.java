...
import com.mediatek.bluetooth.IBluetoothDun;
...
public class BluetoothDunService extends Service 
{
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (!mHasInitiated) {
            ...
            // Set mTetherOn based on the last saved tethering preference while starting the DUN service
            SharedPreferences tetherSetting = getSharedPreferences(DUN_PREFERENCE_FILE, 0);
    ...
    private synchronized void onDunDialupReq()
    {
        ...
        else if (MobileState == NetworkInfo.State.CONNECTED || WifiState == NetworkInfo.State.CONNECTED)
        {
            
            if (DBG) log("[DUN]: Network isAvailable =("	+ connmgr.getActiveNetworkInfo().isAvailable() + ")");
