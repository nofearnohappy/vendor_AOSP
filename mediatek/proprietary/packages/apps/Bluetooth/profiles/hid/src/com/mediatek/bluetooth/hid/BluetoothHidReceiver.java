...
public class BluetoothHidReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (DEBUG) {
            Log.v("@M_" + TAG, "HID broadcast receiver receives intent");
        }
        ...
        if (action.equals(BluetoothDevicePicker.ACTION_DEVICE_SELECTED)) {
            ...
            if (remoteDevice == null) {
                if (DEBUG) {
                    Log.e("@M_" + TAG, "ERROR: remoteDevice null");
                }
                return;
            }

            if (DEBUG) {
                Log.v("@M_" + TAG, "Received BT device selected intent, bt device: " + remoteDevice.getAddress());
            }
    ...
    private void restorePreferenceList(Context context, BluetoothDevice remoteDevice) {
        ...
        if (DEBUG) {
            Log.i("@M_" + TAG, "restorePreferenceList,preferenceCount=" + preferenceCount);
        }
        ...
        for (preferenceIndex = 0; preferenceIndex < preferenceCount; preferenceIndex++) {
            ...
            if (tmpAddr.equals(remoteDevice.getAddress())) {
                if (DEBUG) {
                    Log.i("@M_" + TAG, "restorePreferenceList,already exist:" + remoteDevice.getName());
                }
