...
public class NetworkUtil{
	...
	public static long getSimIdBySlotId(Context context, int slot){
		...
		//List<SubscriptionInfo> infoList = SubscriptionManager.getSubInfoUsingSlotId(slot);
		SubscriptionInfo infoList = SubscriptionManager.from(context).getSubscriptionInfo(slot);
		
		if (infoList != null){
			simId = infoList.getSubscriptionId();
		} 
		return simId;
	}
	//if no slot for the sim id, return -1;
	public static int getSlotBySimId(Context context, int simId){
		//return SIMInfo.getSlotById(context, simId);
		//SubInfoRecord  info = SubscriptionManager.getSubInfoForSubscriber(simId);
		SubscriptionInfo info = SubscriptionManager.from(context).getSubscriptionInfo(simId);
		if (info != null){
