...
public class BluetoothTethering extends INetworkManagementEventObserver.Stub {
    ...
    private BluetoothTethering() {
        Log.d("@M_" + TAG, "Tethering starting");
    }
    ...
    public void registerBTTether(Context context) {
        if (!sBtTetheringRegistered) {
            Log.d("@M_" + TAG, "BluetoothTethering setContext");
            mContext = context;
            ...
            if (mNetworkManagementService == null) {
                Log.e("@M_" + TAG, "Error get INetworkManagementService");
                return;
            }
            try {
                mNetworkManagementService.registerObserver(this);
            } catch (RemoteException e) {
                Log.e("@M_" + TAG, "Error registering observer :" + e);
            }
    ...
    public void unregisterBTTether() {
        if (sBtTetheringRegistered) {
            Log.d("@M_" + TAG, "unregister Bluetooth Tether");
            try {
    ...
    public void interfaceAdded(String iface) {
        ...
        if (!isBT(iface)) {
            // Log.d("@M_" + mTAG, iface + " is not a BT tetherable iface, ignoring");
            return;
        }

        Log.d("@M_" + TAG, "interfaceAdded :" + iface);
