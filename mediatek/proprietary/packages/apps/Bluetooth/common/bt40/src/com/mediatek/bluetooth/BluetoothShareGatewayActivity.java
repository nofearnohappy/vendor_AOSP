...
public class BluetoothShareGatewayActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (null == uri) {
            Log.e("@M_" + TAG, "uri is null");
        } else {
            Log.v("@M_" + TAG, "uri = " + uri.toString());
            sBip = false;
        ...
        if (null == uris) {
            Log.e("@M_" + TAG, "uris is null");
        } else {
            sBip = false;
            for (Uri tmpUri : uris) {
                if (null == tmpUri) {
                    Log.e("@M_" + TAG, "uri is null in Uris arraylist");
                    finish();
                    return;
                }
                Log.v("@M_" + TAG, "uri = " + tmpUri.toString());
        ...
        if (Intent.ACTION_SEND.equals(action) || Intent.ACTION_SEND_MULTIPLE.equals(action)
            ...
            Log.v("@M_" + TAG, "sType = " + sType);
            sIntent = intent;

            sAdapter = BluetoothAdapter.getDefaultAdapter();
            if (sAdapter == null) {
                Log.e("@M_" + TAG, "bluetooth is not started! ");
                finish();
                return;
            }

            if (sAdapter.isEnabled()) {
                Log.v("@M_" + TAG, "bluetooth is available");

                BluetoothDevice remoteDevice = intent.getParcelableExtra(EXTRA_DEVICE_ADDRESS);
                Log.v("@M_" + TAG, "Received BT device selected intent, bt device: " + remoteDevice);

                if (null == remoteDevice) {
                    Log.i("@M_" + TAG, "remote device is null");
                    startDevicePicker();
                } else {
                ...
                Log.w("@M_" + TAG, "bluetooth is not available! ");
            ...
            Log.v("@M_" + TAG, "return from device picker");
            if (null == sIntent) {
            ...
            if (null == remoteDevice) {
                Log.e("@M_" + TAG, "remote device is null");
            } else {
                profileDispatcher(remoteDevice);
                /*
            ...
            Log.e("@M_" + TAG, "unsupported action: " + action);
            finish();
        }
    }
    ...
    private void profileDispatcher(BluetoothDevice remoteDevice) {
        ...
        if (ConfigHelper.checkSupportedProfiles(ProfileConfig.PROFILE_ID_BIP) && sBip && sType.startsWith("image")
            ...
            Log.v("@M_" + TAG, "Disaptch to default OPP");
