...
public class AdvancedService extends Service {
    ...
    private void startProfileServices(Context context) {
        ...
        if (DBG) log("Profile Config :PROFILE_ID_FMOVERBT" + ConfigHelper.checkSupportedProfiles(ProfileConfig.PROFILE_ID_FMOVERBT));
        if ( ConfigHelper.checkSupportedProfiles(ProfileConfig.PROFILE_ID_FMOVERBT) && btAdapter != null &&
			(btState != BluetoothAdapter.STATE_OFF || btState != BluetoothAdapter.STATE_TURNING_OFF) ) {			
			if (DBG) log("Profile Config : FM over BT supported");
			if (DBG) log("Advanced Service => start fmOverBt service");
            this.startService(context, FMOVERBT_SERVICE_CLASS);
        }
		else{
			if (DBG) log("Profile Config : FM over BT not supported");
		}
    }
    ...
    private void stopProfileServices(Context context) {
		...
		if(ConfigHelper.checkSupportedProfiles(ProfileConfig.PROFILE_ID_FMOVERBT))
