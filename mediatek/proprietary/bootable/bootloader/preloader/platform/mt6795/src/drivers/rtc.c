...
static U16 get_frequency_meter(U16 val, U16 measureSrc, U16 window_size)
{
	...
	if(val!=0) {
		U16 osc32con;
		osc32con = OSC32CON_ANALOG_SETTING | RTC_EMBCK_SEL_OPTION | RTC_REG_OSC32CON_XOSC32_ENB;
	...
	RTC_Write(RG_FQMTR_TAGSET, FQMTR_EN | measureSrc); //enable freq. meter, set measure clock to 26Mhz
	...
	RTC_Write(RG_FQMTR_TAGSET, RTC_Read(RG_FQMTR_TAGSET) & ~(FQMTR_EN)); //enable freq. meter, set measure clock to 26Mhz
...
bool rtc_boot_check(void)
{
	...
	RTC_Write(RG_FQMTR_CLN_CLK_PDN, RTC_Read(RG_FQMTR_CLN_CLK_PDN) | FQMTR_CLK_PDN);
...
void rtc_save_2sec_stat(void)
{
	...
	print("rtc_2sec_reboot_check\n");
	reboot = RTC_Read(RTC_CALI);
	if (reboot & RTC_CALI_BBPU_2SEC_EN) {
		switch((reboot & RTC_CALI_BBPU_2SEC_MODE_MSK) >> RTC_CALI_BBPU_2SEC_MODE_SHIFT) {
			case 0:
			case 1:
			case 2:
				if(reboot & RTC_CALI_BBPU_2SEC_STAT || pmic_IsVbatDrop() == PMIC_VBAT_DROP) {
					rtc_2sec_stat_clear();
					pdn2 = RTC_Read(RTC_PDN2);
...
void rtc_enable_2sec_reboot(void)
{
	...
	reboot = RTC_Read(RTC_CALI) | RTC_CALI_BBPU_2SEC_EN;
	reboot = (reboot & ~(RTC_CALI_BBPU_2SEC_MODE_MSK)) | (RTC_2SEC_MODE << RTC_CALI_BBPU_2SEC_MODE_SHIFT);
	RTC_Write(RTC_CALI, reboot);
	Write_trigger();
}
