...
volatile unsigned int g_uart = UART0;
...
void serial_setbrg (U32 uartclk, U32 baudrate)
{
    ...
    *          use UART0_HIGHSPEED = 0x3 can get more sample count to get better UART sample rate    
    *          based on baud_rate = uart clock frequency / (sampe_count * divisor)
    *          divisor = (DLH+DLL)
    */         

    // In order to get better UART sample rate, set UART0_HIGHSPEED = 0x3. 
...
{
#if !CFG_FPGA_PLATFORM
	if ((CFG_UART_META == UART1) || (CFG_UART_LOG == UART1)) {
...
//--- DSDA Uart1 init ---
    ...
    UART_SET_BITS(UART_FCR_FIFO_INIT | UART_FCR_RXFIFO_12B_TRI | UART_FCR_TXFIFO_1B_TRI, UART_FCR(g_uart)); // Enable UART FIFO
	
    UART_WRITE16(UART_NONE_PARITY | UART_WLS_8 | UART_1_STOP, UART_LCR(g_uart));
    serial_setbrg(uartclk, 115200);
	...
	mt_set_gpio_mode(GPIO_UART_URTS1_PIN, GPIO_MODE_01);
	mt_set_gpio_dir(GPIO_UART_URTS1_PIN, GPIO_DIR_OUT);
	
	mt_set_gpio_mode(GPIO_UART_UCTS1_PIN, GPIO_MODE_01);
	mt_set_gpio_dir(GPIO_UART_UCTS1_PIN, GPIO_DIR_IN);
	mt_set_gpio_pull_enable(GPIO_UART_UCTS1_PIN, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(GPIO_UART_UCTS1_PIN, GPIO_PULL_UP);
//-------------------
#endif

    mtk_serial_set_current_uart(CFG_UART_META);
    //PDN_Power_CONA_DOWN(PDN_PERI_UART0, 0);

    //UART_CLR_BITS(1 << 0, AP_PERI_GLOBALCON_RST0); /* Release UART0 reset signal */
    //UART_SET_BITS(1 << 24, AP_PERI_GLOBALCON_PDN0); /* Power on UART0 */
    UART_CLR_BITS(1 << 19, AP_PERI_GLOBALCON_PDN0); /* Power on UART0 */
...
#if (CFG_DT_MD_DOWNLOAD && CFG_DT_MD_DOWNLOAD_BY_RELAY)
int relay_GetUARTBytes(u8 *buf, u32 size, u32 tmo_ms)
{
    u32 LSR;
    int tmo_en = (tmo_ms) ? 1 : 0;
    ulong start_time = get_timer(0);
...
void relay_PutUARTByte (const char c)
{
    while (!(UART_READ32 (UART_LSR(g_uart)) & UART_LSR_THRE))
    {
    }
...
u32 UART_read_LSR(void)
{
    return UART_READ32(UART_LSR(g_uart));
}
#endif
