...
#define VMAX_VAL		MV_TO_VAL(1130)
...
static void ptp_init_det(struct ptp_det *det, struct ptp_devinfo *devinfo)
{
	...
	case PTP_DET_VCORE_PDN:
		...
		/* get DVFS frequency table */
		det->num_freq_tbl = 2;
		det->freq_tbl[0] = 800 * 100 / 800; // XXX: percentage, 800/800, dedicated for VCORE only
		det->freq_tbl[1] = 646 * 100 / 800; // XXX: percentage, 600/800, dedicated for VCORE only
...

	{
		...
		if (   ptp_detectors[PTP_DET_VCORE_AO].VBOOT == ptp_detectors[PTP_DET_VCORE_PDN].VBOOT
			...
			for (i = 1; (ptp_detectors[PTP_DET_VCORE_AO].VBOOT - i) >= vcore_volt; i++) {
				...
				pmic_config_interface(0x24E,(ptp_detectors[PTP_DET_VCORE_PDN].VBOOT - i),0x7F,0);
