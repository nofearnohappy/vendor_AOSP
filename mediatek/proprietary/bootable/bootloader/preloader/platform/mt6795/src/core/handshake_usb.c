...
extern bool b_is_enter_relay_mode;
#endif
...
#if (CFG_DT_MD_DOWNLOAD && CFG_DT_MD_DOWNLOAD_BY_RELAY)
//==== RE-ENUM USB by Jemery ====
extern int g_enable_u3;
extern struct device_descriptor* get_device_descriptor(int enableU3);

bool enum_in_relay_mode(void)
{
    bool result = FALSE;
    print("%s %s+\n", MOD, __func__);
    struct device_descriptor* dev_desc = get_device_descriptor(g_enable_u3);

    dev_desc->idVendor = 0x0E8D;
    dev_desc->idProduct = 0x0003;

	usbdl_init();
    udelay(1000);
    usb_disconnect();

    if (usb_connect(USB_ENUM_TIMEOUT))
        result = TRUE;
	print("%s USB RE-ENUM: %s\n", MOD, result ? "PASS!" : "FAIL.");
    print("%s %s-\n", MOD, __func__);
    return result;
}
#endif
...
bool usb_handshake(struct bldr_command_handler *handler)
{
    ...
    if (FALSE == usb_handshake_handler(handler, handshake_tmo)) {
        ...
        if(b_is_enter_relay_mode) {
            print("%s Enter relay mode.\n", MOD);
            log_buf_ctrl(0);
            log_ctrl(0); // Disable uart log while relaying
            platform_set_6261_dwl_rst();
            if(uart_handshake_6261_brom()) {
                // 1. re-connect with 6261 flash tool usb
                enum_in_relay_mode();
                // 2. handshake with 6261 flash tool
                if(usb_handshake_6261_flashtool()){
                // 3. relay
                    print("%s 6261 Flash tool Handshake PASS! Starting Relay...\n", MOD);
                    usb_uart_relay();
                }
                else
                    print("%s Handshake with 6261 Flash Tool FAIL.\n", MOD);
            }
            log_ctrl(1);
            b_is_enter_relay_mode = FALSE; //finish
        }
    }
#endif
    result = TRUE;
