...
int matv_init(void)
{
    SLOGD("_UT_ matv_init()");
    return 1;
}

int matv_suspend(int on)
{
    SLOGD("_UT_ matv_suspend() on=%d", on);
    return 1;
}

int matv_shutdown(void)
{
    SLOGD("_UT_ matv_shutdown()");
    return 1;
}

void matv_chscan(kal_uint8 mode)
{
    SLOGD("_UT_ matv_chscan() mode=%d", mode);
}
void matv_chscan_stop(void)
{
    SLOGD("_UT_ matv_chscan_stop()");
}
                            ...
                            matv_audioformat_cb audfmt_cb)
{
    ...
    SLOGD("_UT_ matv_register_callback() cb_param=%x, auto_cb=%x, full_cb=%x, finish_cb=%x, audfmt_cb=%x", cb_param, auto_cb, full_cb, finish_cb, audfmt_cb);
}

kal_bool matv_get_chtable(kal_uint8 ch, matv_ch_entry *entry)
{
    SLOGD("_UT_ matv_get_chtable() ch=%d, entry=%x", ch, entry);
    return 1;
}

kal_bool matv_set_chtable(kal_uint8 ch, matv_ch_entry *entry)
{
    SLOGD("_UT_ matv_set_chtable() ch=%d, entry=%x", ch, entry);
    return 1;
}

int matv_clear_chtable(void)
{
    SLOGD("_UT_ matv_clear_chtable()");
    return 1;
}

void matv_change_channel(kal_uint8 ch)
{
    SLOGD("_UT_ matv_change_channel() ch=%d", ch);
}
void matv_set_country(kal_uint8 country)
{
    SLOGD("_UT_ matv_set_country() country=%d", country);
}
...
void matv_chscan_query(matv_chscan_state *scan_state)
{
    ...
    SLOGD("_UT_ matv_chscan_query() scan_state=%x", scan_state);
}
void matv_audio_play(void)
{
    SLOGD("_UT_ matv_audio_play()");
}
void matv_audio_stop(void)
{
    SLOGD("_UT_ matv_audio_stop()");
}
kal_uint32 matv_audio_get_format(void)
{
    SLOGD("_UT_ matv_audio_get_format()");
    return 1;
}

void matv_audio_set_format(kal_uint32 val)
{
    SLOGD("_UT_ matv_audio_set_format() val=%d", val);
}
kal_uint8 matv_audio_get_sound_system(void)
{
    SLOGD("_UT_ matv_audio_get_sound_system()");
    return 1;
}

kal_uint8 matv_audio_get_fm_detect_status(kal_uint32 *qulity)
{
    SLOGD("_UT_ matv_audio_get_fm_detect_status()");
    return 1;
}

int matv_get_chipdep(kal_uint8 item)
{
    SLOGD("_UT_ matv_get_chipdep() item=%d", item);
    return 1;
}

int matv_set_chipdep(kal_uint8 item, kal_int32 val)
{
    SLOGD("_UT_ matv_set_chipdep() item=%d, val=%d", item, val);
    return 1;
}

int matv_adjust(kal_uint8 item, kal_int32 val)
{
    SLOGD("_UT_ matv_adjust() item=%d, val=%d", item, val);
    return 1;
}
#endif
...
{
    SLOGD("_UT_ fm_powerup() parm=%x", parm);
    return 1;
}
kal_bool fm_powerdown(void)
{
    SLOGD("_UT_ fm_powerdown()");
    return 1;
}
int fm_getrssi(void)
{
    SLOGD("_UT_ fm_getrssi()");
    return 1;
}
kal_bool fm_tune(struct fm_tune_parm *parm)
{
    SLOGD("_UT_ fm_tune() parm=%x", parm);
    return 1;
}
kal_bool fm_seek(struct fm_seek_parm *parm)
{
    SLOGD("_UT_ fm_seek() parm=%x", parm);
    return 1;
}
kal_bool fm_scan(struct fm_scan_parm *parm)
{
    SLOGD("_UT_ fm_scan() parm=%x", parm);
    return 1;
}
kal_bool fm_mute(kal_uint32 val)
{
    SLOGD("_UT_ fm_mute() val=%d", val);
    return 1;
}
int fm_getchipid(void)
{
    SLOGD("_UT_ fm_getchipid()");
    return 1;
}
...
ATVCtrlService::ATVCtrlService()
    : BnATVCtrlService()
{
    SLOGD("ATVCtrlService()");
    atvaud_Flag = 0;
    parameterb = 0;
#ifdef POWER_SAVING
    SLOGD("[73+92PS] In constructor, force to ps init state");
    matv_ps_init(1);
...
{
    SLOGD("ATVCtrlService::instantiate()");

    ATVCtrlService *mATVCtrlService = new ATVCtrlService();
    SLOGD("Pass verification");
...
{
    SLOGD("registerClient() %p, tid %d, calling tid %d", client.get(), gettid(), IPCThreadState::self()->getCallingPid());
    ...
    {
        SLOGD("Adding notification client %p", binder.get());
...
{
    SLOGD("registerClient_FM() %p, tid %d, calling tid %d", client.get(), gettid(), IPCThreadState::self()->getCallingPid());
    Mutex::Autolock _l(mLock);
...
{
    SLOGD("CLI() tid %d, calling tid %d", gettid(), IPCThreadState::self()->getCallingPid());
    Mutex::Autolock _l(mLock);
...
{
    SLOGD("binderDied() %p, tid %d, calling tid %d", who.unsafe_get(), gettid(), IPCThreadState::self()->getCallingPid());
    Mutex::Autolock _l(mLock);
    ...
    if (binder != NULL)
    {
        ...
        if (index >= 0)
        {
            SLOGD("Removing notification client %p", binder);
            mNotificationClients.removeAt(index);
        }
    }

    sp<IBinder> binder_atv = IInterface::asBinder(mClient);
    sp<IBinder> binder_fm = IInterface::asBinder(mClient_FM);

    if (binder == binder_atv.get())
    {
        SLOGD("binderDied: atv client");
        ...
        {
            kal_bool ret;
            SLOGW("Binder died when state = %d, force to shutdown", mState);
            ret = matv_shutdown();

            if (ret == false)
            {
                SLOGE("ATVCS_matv_shutdown fail");
            }
    ...
    {
        SLOGD("binderDied: fm client");

        if (mState == MATVCTRL_STATE_FM_OPEN)
        {
            kal_bool ret;
            SLOGW("Binder died when state = %d, force to shutdown FM", mState);
            ret = fm_powerdown(0);

            if (ret == false)
            {
                SLOGE("ATVCS_fm_powerdown fail");
            }
...
kal_bool ATVCtrlService::ATVCS_matv_init(void)
{
    ...
    /*
       SLOGD("time before matv_ps_init(1) = %lld", getTimeMs());
       ret = matv_ps_init(1);
       SLOGD("time after matv_ps_init(1) = %lld", getTimeMs());

       usleep(100000);

       SLOGD("time before matv_ps_init(0) = %lld", getTimeMs());
       ret = matv_ps_init(0);
       SLOGD("time after matv_ps_init(0) = %lld", getTimeMs());
    */

    //if calling init in wrong state, do not change state, just return
    if (mState == MATVCTRL_STATE_OPEN)
    {
        SLOGW("Calling ATVCS_matv_init when state = %d, skip", mState);
        return true;
    }
    else if (mState == MATVCTRL_STATE_PS)
    {
#ifdef POWER_SAVING
        SLOGD("[73+92PS] Calling ATVCS_matv_init when state ps, leave ps_init and then do init");
        matv_ps_init(0);
#else
        SLOGW("Calling ATVCS_matv_init when state = %d, skip with return false", mState);
        return false;
#endif
    }
    else if (mState == MATVCTRL_STATE_FM_OPEN)
    {
        SLOGW("Calling ATVCS_matv_init when state = %d, powerdown fm before matv init", mState);
        ret = fm_powerdown(0);

        if (ret == true)
        {
            isfmdrvpd = 1;
            SLOGD("ATVCS_fm_powerdown success");

            if (mClient_FM.get() != NULL)
            {
                mClient_FM->notify(0, 0, 0);
                SLOGD("notify FM application");
            }
        }
        else
        {
            SLOGD("ATVCS_fm_powerdown fail");
            return false;
        }
    }
...
kal_bool ATVCtrlService::ATVCS_matv_ps_init(kal_bool on)
{
    ...
    kal_bool ret;
    SLOGD("ATVCS_matv_ps_init");

#ifdef POWER_SAVING
    SLOGD("[73+92PS] Calling ATVCS_matv_ps_init [%d] is useless, skip");
    return true;
#endif
    ...
    if (mState == MATVCTRL_STATE_OPEN)
    {
        SLOGW("Calling ATVCS_matv_ps_init when state = %d, skip", mState);
        return false;
    }
    else if (mState == MATVCTRL_STATE_FM_OPEN)
    {
        SLOGW("Calling ATVCS_matv_ps_init when state = %d, keep psFlag", mState);
        fm_psFlag = on;
        return false;
    }
...
kal_int32 ATVCtrlService::ATVCS_matv_set_parameterb(kal_int32 in)
{
    ...
    SLOGD("ATVCS_matv_set_parameterb");
    ...
    if (parameterb == 1)
    {
        SLOGD("ATVCS_matv_set_parameterb success");
        ret = (in + a) * b + c;
    }
    else
    {
        SLOGD("ATVCS_matv_set_parameterb fail");
        ret = (in + a) * b - c;
    }
...
kal_bool ATVCtrlService::ATVCS_matv_shutdown(void)
{
    ...
    {
        SLOGD("[73+92PS] Calling Shutdown when state = %d, skip ", mState);
        return true;
    }

    SLOGD("[73+92PS] state = %d, Shutdown then, enter ps init ");
    ret = matv_shutdown();
    ret = matv_ps_init(1);

    if (ret == true)
    {
        SLOGD("Force to callback for shutdown");
        atv_shutdown_cb(this, 0);
    }
    else
    {
        SLOGD("ATVCS_matv_shutdown fail");
    }
    ...
    {
        SLOGW("Calling ATVCS_matv_powerdown when state = %d && isatvdrvpd, skip", mState);
        isatvdrvpd = 0;
        return true;
    }
    ...
    {
        SLOGW("Calling ATVCS_matv_shutdown when state = %d, skip", mState);
        return false;
    }
...
kal_bool ATVCtrlService::ATVCS_matv_get_chtable(int ch, void *entry, int len)
{
    ...
    if (ret == false)
    {
        SLOGD("ATVCS_matv_get_chtable fail");
    }
...
kal_bool ATVCtrlService::ATVCS_matv_set_chtable(int ch, void *entry, int len)
{
    ...
    if (ret == false)
    {
        SLOGD("ATVCS_matv_set_chtable fail");
    }
...
kal_bool ATVCtrlService::ATVCS_matv_clear_chtable(void)
{
    ...
    if (ret == false)
    {
        SLOGD("ATVCS_matv_clear_chtable fail");
    }
...
{
    SLOGD("ATVCS_matv_clear_chtable ch=%d", ch);
...
kal_bool ATVCtrlService::ATVCS_matv_set_chipdep(int item, kal_int32 val)
{
    ...
    if (ret == false)
    {
        SLOGD("ATVCS_matv_set_chipdep fail");
    }
...
kal_bool ATVCtrlService::ATVCS_fm_powerup(void *parm, int len)
{
    ...
    if (mState == MATVCTRL_STATE_FM_OPEN)
    {
        SLOGW("Calling ATVCS_fm_powerup when state = %d, skip", mState);
        return true;
    }
    else if (mState == MATVCTRL_STATE_PS)
    {
        SLOGW("Calling ATVCS_fm_powerup when state = %d, skip with return false", mState);
        return false;
    }
    else if (mState == MATVCTRL_STATE_OPEN)
    {
        SLOGW("Calling ATVCS_fm_powerup when state = %d, powerdown atv before fm init", mState);
        ret = matv_shutdown();

        if (ret == true)
        {
            isatvdrvpd = 1;
            SLOGD("ATVCS_matv_shutdown success");
        }
        else
        {
            SLOGD("ATVCS_matv_shutdown fail");
...
kal_bool ATVCtrlService::ATVCS_fm_powerdown(void)
{
    ...
    {
        SLOGW("Calling ATVCS_fm_powerdown when state = %d && isfmdrvpd, skip", mState);
        isfmdrvpd = 0;
        return true;
    }
    //if calling shutdown in wrong state, do not change state, just return
    ...
    {
        SLOGW("Calling ATVCS_fm_powerdown when state = %d, skip", mState);
        return false;
    }
    ...
    if (ret == true)
    {
        SLOGD("Force to callback for powerdown");
        //atv_shutdown_cb(this, 0);
    }
    else
    {
    ...
    if (fm_psFlag)
    {
        ...
        {
            SLOGD("ATVCS_matv_ps_init success");
            mState = MATVCTRL_STATE_PS;
        }
...
{
    SLOGD("atv_autoscan_progress_cb cb_param=%p, precent=%d, ch=%d, chnum=%d", cb_param, precent, ch, chnum);
    ...
    if (c != NULL)
    {
        SLOGD("atv_autoscan_progress_cb AutoScan_CB ptr=%p, len=%d", ptr, len);
...
{
    SLOGD("atv_fullscan_progress_cb cb_param=%p, precent=%d, freq=%d, freq_start=%d, freq_end=%d", cb_param, precent, freq, freq_start, freq_end);
    ...
    if (c != NULL)
    {
        SLOGD("atv_fullscan_progress_cb FullScan_CB");
...
{
    SLOGD("atv_scanfinish_cb cb_param=%p, chnum=%d", cb_param, chnum);
    ...
    if (c != NULL)
    {
        SLOGD("atv_scanfinish_cb ScanFinish_CB");
...
{
    SLOGD("atv_audioformat_cb format=%d", format);
    ...
    if (c != NULL)
    {
        SLOGD("atv_audioformat_cb AudioFmt_CB");
...
{
    SLOGD("atv_shutdown_cb source=%d", source);
    ...
    if (c != NULL)
    {
        SLOGD("atv_shutdown_cb Shutdown_CB");
