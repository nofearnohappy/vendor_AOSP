...
//        waitpid(pid,0,0); /* wait for child to exit */
	  do{
	   ...
	   if(wpid == 0){
		 	if(waittime < timeout)
		 	{
				icusb_print(PRINT_INFO, "[ICUSB][INFO] waittime=%d\n",waittime);
				sleep(1);
				waittime++;
		 	}
			else
			{
				icusb_print(PRINT_INFO, "[ICUSB][INFO] Kill process\n");
				kill(pid,SIGKILL);
    }
	   	 }
	  } while (wpid == 0 && waittime <= timeout);
	  if (WIFEXITED(status)) {
   		icusb_print(PRINT_INFO, "Child exited, status=%d\n", WEXITSTATUS(status));	
	    return ICUSB_OK;
	  }
      else if (WIFSIGNALED(status)) {
   		icusb_print(PRINT_INFO, "Child %d terminated status : %d \n", pid, WTERMSIG(status));
		return ICUSB_DEV_GENERAL_ERR;
      }	  
    }
    return ICUSB_OK;
