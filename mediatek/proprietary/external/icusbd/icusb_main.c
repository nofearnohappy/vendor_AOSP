...
//        unsigned int polling_cnt = 500 ;
		...
		usleep(ICUSB_USB_DISCONNECT_POLL_INTERVAL_US); // delay 100 ms for USB reset
			...
			if(g_ic_usb_status == (USB_PORT1_DISCONNECT_DONE << USB_PORT1_STS_SHIFT))
			{
				icusb_print(PRINT_INFO,"USB_PORT1_DISCONNECT_DONE\n");
				break;
			}

			if(g_ic_usb_status == (USB_PORT1_DISCONNECTING << USB_PORT1_STS_SHIFT))
			{
				icusb_print(PRINT_INFO,"USB_PORT1_DISCONNECTING\n");
			}
