...
#include "../libipsec_good/libpfkey.h"
#include "vchar.h"
#include "extern.h"
			...
			if (status < 0)
                        {
                                ...
                                fprintf(stderr,"add sa error:%d",errno);
				return -1;
                        }
		}
    break;
...
#line 759 "parse.y"
    {
                        fprintf(stderr,"(yyvsp[(1) - (1)].val).buf:%s\n",(yyvsp[(1) - (1)].val).buf);
			(yyval.res) = parse_addr((yyvsp[(1) - (1)].val).buf, NULL);
			if ((yyval.res) == NULL) {
				/* yyerror already called by parse_addr */
				return -1;
	...
	struct addrinfo *dsts;
{
	...
	if (!srcs || !dsts)
        {
                fprintf(stderr,"setkeymsg_add !srcs || !dsts");
		return -1;
        }
	/* fix up length afterwards */
	setkeymsg0(msg, type, satype, 0);
...
#endif
			default:
                                fprintf(stderr,"setkeymsg_add p_natt_oa,sa->sa_family:%d",sa->sa_family);
				return -1;
			}
	...
	for (s = srcs; s; s = s->ai_next) {
		for (d = dsts; d; d = d->ai_next) {
			/* rewind pointer */
			l = l0;
                        fprintf(stderr,"setkeymsg_add p_natt_oa,s->ai_addr->sa_family:%d,d->ai_addr->sa_family:%d",s->ai_addr->sa_family,d->ai_addr->sa_family);
			if (s->ai_addr->sa_family != d->ai_addr->sa_family)
				continue;
			switch (s->ai_addr->sa_family) {
			case AF_INET:
