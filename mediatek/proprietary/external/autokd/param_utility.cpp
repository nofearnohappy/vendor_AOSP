...
int get_param_data_from_buf(struct autok_predata *test_predata, char *buf)
{
    ...
    test_predata->param_count = param_count;
    offset += 2;
    //LOGI("vol_count:%d, param_count:%d", vol_count, param_count);
...
struct autok_predata get_param(char *filename)
{
    ...
    LOGI("Get Param from [%s]\n", filename);
    data_buf = NULL;
    if(NULL != strstr(filename, AUTOK_RES_PATH)){
        read_from_file(filename, &data_buf, &data_count);
    } else {
...
{
    int i;
    AUTOK_FREE(predata->vol_list);
    for(i=0; i<predata->vol_count; i++){
        AUTOK_FREE(predata->ai_data[i]);
    }   
    AUTOK_FREE(predata->ai_data);
}