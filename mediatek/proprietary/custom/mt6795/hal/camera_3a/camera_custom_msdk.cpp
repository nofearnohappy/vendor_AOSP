						  ...
						  PAE_PLINETABLE_STRUCT pCameraAEPlineData)
{
    ...
    if (pCameraISPDefault!=NULL)
        pstSensorInitFunc[i].getCameraDefault(CAMERA_NVRAM_DATA_ISP,
                 (VOID*)pCameraISPDefault,sizeof(NVRAM_CAMERA_ISP_PARAM_STRUCT));

    if (pCamera3ADefault != NULL)
        pstSensorInitFunc[i].getCameraDefault(CAMERA_NVRAM_DATA_3A,
                 (VOID*)pCamera3ADefault,sizeof(NVRAM_CAMERA_3A_STRUCT));

    if (pCameraShadingDefault!=NULL)
        pstSensorInitFunc[i].getCameraDefault(CAMERA_NVRAM_DATA_SHADING,
                 (VOID*)pCameraShadingDefault,sizeof(NVRAM_CAMERA_SHADING_STRUCT));

    if (pCameraAEPlineData != NULL)
        pstSensorInitFunc[i].getCameraDefault(CAMERA_DATA_AE_PLINETABLE,
                 (VOID*)pCameraAEPlineData,sizeof(AE_PLINETABLE_T));
...
MUINT32 GetCameraCalData(MUINT32 SensorId, MUINT32* pGetSensorCalData)
{
    ...
    if(pstSensorInitFunc == NULL) //Len shading page --> Save to NVRAM --> CCT reboot failed
    {
...
MUINT32 LensCustomGetInitFunc(MSDK_LENS_INIT_FUNCTION_STRUCT *a_pLensInitFunc)
{
    if (a_pLensInitFunc != NULL) {
        memcpy(a_pLensInitFunc, &LensInitFunc[0],
               sizeof(MSDK_LENS_INIT_FUNCTION_STRUCT) * MAX_NUM_OF_SUPPORT_LENS);
        return 0;
    }
    return -1;
}
...
MINT32 GetCameraTsfDefaultTbl(MUINT32 SensorId, PCAMERA_TSF_TBL_STRUCT pCameraTsfDefault)
{
    ...
    if (pCameraTsfDefault!=NULL)
        ret = pstSensorInitFunc[i].getCameraDefault(CAMERA_DATA_TSF_TABLE,
                    (VOID*)pCameraTsfDefault,sizeof(CAMERA_TSF_TBL_STRUCT));
...
MINT32 GetCameraFeatureDefault(MUINT32 SensorId, NVRAM_CAMERA_FEATURE_STRUCT *pCamFeatureDefault)
{
    ...
    if (pCamFeatureDefault!=NULL)
        ret = pstSensorInitFunc[i].getCameraDefault(CAMERA_NVRAM_DATA_FEATURE,
                 (VOID*)pCamFeatureDefault,sizeof(NVRAM_CAMERA_FEATURE_STRUCT));
