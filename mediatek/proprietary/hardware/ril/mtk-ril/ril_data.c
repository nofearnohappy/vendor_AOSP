...
extern int deactivatePdnThruEpdg(int cid,int interfaceId,int reason);
    ...
    if (!isReqFromMAL) {
        ...
        if (isEpdgSupport()) {
            ...
            case PDN_SETUP_THRU_WIFI: {
                    reqSetupConf.availableCid = getAvailableCid();
    ...
    for(i = 0; i < pdnInfoSize; i++) {
        if (pdn_info[i].interfaceId == interfaceId) {
            if (!pdn_info[i].isDedicateBearer) {
                if ( 0 != deactivatePdnThruEpdg(pdn_info[i].cid, interfaceId, reason)) {
