...
{
    ALOGD("[%s] TxDataRate11n = %d , FrameFormat = %d , GI = %d\n",
        __func__, TxDataRate11n, FrameFormat, GI);

    uint32_t u4Rate, u4Preamble;
    // TxDataRate11n--; // Index Mismatch
    FrameFormat--;
    GI--;
    ...
    if (LGE_RFT_SetBandWidth(HT20) == false) {
        return false;
    }
...
bool LGE_RFT_TxDataRate11n40M(int TxDataRate11n, int FrameFormat, int GI)
{
    if (LGE_RFT_TxDataRate11n(TxDataRate11n, FrameFormat, GI) == false) {
        return false;
    }
    if (LGE_RFT_SetBandWidth(HT40) == false) {
        return false;
    }
...
static int g_LGE_RFT_OTAenable = 0;
bool LGE_RFT_OtaEnable(void)
{
    ALOGE("[%s]\n", __func__);
    if(g_LGE_RFT_OTAenable == 1) {
        //return true;
    }
    ...
    //if(0 == wifi_test_set_swctrl(0xffff1234, 1, NULL, NULL))
    if (LGE_RFT_AutoTxPower(false) && LGE_RFT_PowerSave(false) && LGE_RFT_Roaming(false))
    {
        g_LGE_RFT_OTAenable = 1;

        //set force country code to EU for using 12/13ch
        if (!LGE_RFT_OTACountryCode(true))
        {
            return false;
        }
        return true;
...
bool LGE_RFT_OtaDisable(void)
{
    ALOGE("[%s]\n", __func__);
    if(g_LGE_RFT_OTAenable == 0) {
        //return true;
    }

    //if(0 == wifi_test_set_swctrl(0xffff1235, 1, NULL, NULL))
    if (LGE_RFT_Roaming(true) && LGE_RFT_PowerSave(true) && LGE_RFT_AutoTxPower(true))
    {
        /* close socket if necessary */
        if(skfd > 0) {
            close(skfd);
...
bool LGE_RFT_Roaming_Disable_for_ORG(void)
{
    bool ret = false;
...
*/
bool LGE_RFT_SetBandWidth(int bandWidth)
{
    ALOGD("[%s] bandWidth = %d\n", __func__, bandWidth);

    if (bandWidth != HT20 && bandWidth != HT40) {
        return false;
...
bool LGE_RFT_AutoTxPower(bool enable)
{
    ...
    uint32_t funcIndex = enable ? 0xF4FA786A : 0x0000786A;

    if (0 == wifi_test_set_swctrl(0xa0210000, funcIndex, NULL, NULL)) {
        ret = true;
    }
...
bool LGE_RFT_OTACountryCode(bool enable)
{
    int ret=-1;
    struct iwreq iwr;
    char buf[11] = "COUNTRY EU";

    if(enable)
    {
        memset(&iwr, 0, sizeof(iwr));
        strncpy( iwr.ifr_name, WIFI_IF_NAME, IFNAMSIZ);
        iwr.u.data.pointer = buf;
        iwr.u.data.length = 11;
        iwr.u.data.flags = 0x0001;//country code set    // 0x0002 : scan-passive
        ...
        if ((ret = ioctl(skfd, 0x8B0C, &iwr)) < 0) {//SIOCSIWPRIV
            ALOGE("ioctl[SIOCSIWPRIV]: %s %s",buf, strerror(errno));
            close(skfd);
            return false;
        }
