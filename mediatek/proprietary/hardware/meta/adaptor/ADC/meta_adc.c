...
void Meta_AUXADC_OP(AUXADC_REQ *req, char *peer_buff, unsigned short peer_len)
{
	...
	if ((req->dwChannel >= 0)&&(req->dwChannel <= 66)) {
		...
		if (ret == -1) {
			...
			ADCMetaReturn.dwData = adc_out_data[1];
			ADCMetaReturn.status = META_SUCCESS;
		}
	} else if (req->dwChannel == 98){
		adc_in_data[0] = 98;
		adc_in_data[1] = req->dwCount;

		ret = ioctl(meta_adc_fd, Set_META_BAT_CAR_TUNE_VALUE, adc_in_data);
	...
	printf("Meta_AUXADC_OP in   : CH[%d] in %d times : %d\n", req->dwChannel, req->dwCount, adc_in_data[0]);
	printf("Meta_AUXADC_OP out  : CH[%d] in %d times : %d\n", req->dwChannel, req->dwCount, adc_out_data[0]);
	printf("Meta_AUXADC_OP data : CH[%d] in %d dwData : %d : status : %d\n", req->dwChannel, req->dwCount, ADCMetaReturn.dwData, ADCMetaReturn.status);
	ALOGD("Meta_AUXADC_OP in   : CH[%d] in %d times : %d\n", req->dwChannel, req->dwCount, adc_in_data[0]);
	ALOGD("Meta_AUXADC_OP out  : CH[%d] in %d times : %d\n", req->dwChannel, req->dwCount, adc_out_data[0]);
	ALOGD("Meta_AUXADC_OP data : CH[%d] in %d dwData : %d : status : %d\n", req->dwChannel, req->dwCount, ADCMetaReturn.dwData, ADCMetaReturn.status);
