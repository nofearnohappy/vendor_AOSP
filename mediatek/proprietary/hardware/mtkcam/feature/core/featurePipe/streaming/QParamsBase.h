...
template <typename T>
void QParamsBase<T>::staticCBFunc(NSCam::NSIoPipe::NSPostProc::QParams &param)
{
  QParamsBase *parent;
  ...
  if( !parent )
  {
    // TODO: invalid parent pointer
  }
  else if( !parent->retrieveQParamsData(data) )
  {
    // TODO: invalid parent backup queue
  }
  else
  {
    param.mpCookie = data.mQParamsCookie;
    param.mpfnCallback = data.mQParamsCB;
    parent->onQParamsCB(param, data.mData);
  }
}

template <typename T>
MBOOL QParamsBase<T>::enqueQParams(NSCam::NSIoPipe::NSPostProc::INormalStream *stream, NSCam::NSIoPipe::NSPostProc::QParams &param, T &data)
{
  android::Mutex::Autolock lock(mMutex);
  MBOOL ret = MFALSE;
  param.mpCookie = (void*)this;
  param.mpfnCallback = QParamsBase::staticCBFunc;
  if( stream )
  {
    ret = stream->enque(param);
    if( ret )
    {
      mQueue.push(BACKUP_DATA_TYPE(param.mpCookie, param.mpfnCallback, data));
    }
  }
  return ret;
}
