...
#define BuildStream(_type_, _IStreamInfo_)                                     \
    ...
    BuildStream(eStreamType_IMG_HAL_POOL   , mpHalImage_Hdr_YUV);
...
#if ENABLE_HDRNODE
        aCollector.addOut(eMetaPathP2, mpAppMeta_DynamicHdr);
#else
...
#if ENABLE_HDRNODE
    updateIOMap(Meta , eNODEID_HdrNode    , eMetaPathP2);
#else
