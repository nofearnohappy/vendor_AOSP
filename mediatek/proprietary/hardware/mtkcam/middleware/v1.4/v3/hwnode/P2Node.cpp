 ...
 ******************************************************************************/
#define SUPPORT_3A               (1)
#define SUPPORT_EIS              (0)//(1)/*[EP_TEMP]*/
...
)
{
...

    {
        ...
        if (mp3A) {
            ...
            if (0 > mp3A->setIsp(0, inMetaSet, pTuning,
...
)
{
    ...
    if (mbEnableLog)
    {
        ...
        for( size_t i = 0; i < params.vOut.size(); i++ )
        {
            ...
            if ((muDequeCnt > 0) && (muDequeCnt < 2))
            {
                char filename[256] = {0};
                sprintf(filename, "/sdcard/yuv/p2-%dx%d-%d.yuv",
                    pImgBuf->getImgSize().w,
                    pImgBuf->getImgSize().h, muDequeCnt);
