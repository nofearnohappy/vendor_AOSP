...
queryGpuProcEn() const
{
    ...
    //
    MY_LOGD("GPU process enable: %d", enGPUProc);
    //
    MUINT32 mRawFmtType;
    ...
    if(pIHalSensorList)
    {
        ...
        mRawFmtType = sensorStaticInfo.rawFmtType;
    }
    MY_LOGD("common - RawFmtType = %d",mRawFmtType);
    //

    if(mRawFmtType == SENSOR_RAW_RWB)
    {
        switch(get_capture_rwbProc_type())
        {
            case 1:
    ...
    {
        char value[PROPERTY_VALUE_MAX] = {'\0'};
        property_get("debug.shot.forcegpu", value, "-1");
        MINT32 force = atoi(value);
        if(force!=-1) {
            MY_LOGD("force gpu %d", force);
            enGPUProc = force; //0 disable ; 1 enable
        }
    }
