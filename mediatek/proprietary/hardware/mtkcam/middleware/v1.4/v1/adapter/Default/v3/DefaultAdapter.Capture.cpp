...
getFlashQuickCalibrationResult()
{
    ...
    result = pHal3a->getFlashQuickCalibrationResult();
...
onCaptureThreadLoop()
{
    ...
    else
    {
        ...
        mEngParam.u4Exposuretime =  pParamsMgr->getInt(MtkCameraParameters::KEY_ENG_MANUAL_SHUTTER_SPEED);
        mEngParam.u4SensorGain = pParamsMgr->getInt(MtkCameraParameters::KEY_ENG_MANUAL_SENSOR_GAIN);

        mEngParam.u4EVvalue = pParamsMgr->getInt(MtkCameraParameters::KEY_ENG_EV_VALUE); //

        //EVBreacket enable
        mEngParam.mi4EngRawEVBEn = pParamsMgr->getInt(MtkCameraParameters::KEY_ENG_EVB_ENABLE);

        MY_LOGD("get ev-value from ap: %d ", mEngParam.u4EVvalue);

        // Multi-NR
        mEngParam.mi4ManualMultiNREn = pParamsMgr->getInt(MtkCameraParameters::KEY_ENG_MANUAL_MULTI_NR_ENABLE);
        if (1 == mEngParam.mi4ManualMultiNREn)
        {
            mEngParam.mi4ManualMultiNRType = pParamsMgr->getInt(MtkCameraParameters::KEY_ENG_MANUAL_MULTI_NR_TYPE);
        }

        // CCT MFLL
        mEngParam.mi4CamShotType = EngParam::ENG_CAMSHOT_TYPE_SINGLE; // default value
        if (pParamsMgr->getStr(MtkCameraParameters::KEY_MFB_MODE) == MtkCameraParameters::KEY_MFB_MODE_MFLL)
        {
            mEngParam.mi4CamShotType = EngParam::ENG_CAMSHOT_TYPE_MFLL;
        }
        MY_LOGD("mEngParam.mi4CamShotType = %d", mEngParam.mi4CamShotType);

        String8 ms8IspMode = pParamsMgr->getStr(MtkCameraParameters::KEY_ISP_MODE);
        const char *strIspMode = ms8IspMode.string();
        mEngParam.mi4EngIspMode = EngParam::ENG_RAW_TYPE_PURE_RAW; // Default value
        switch (strIspMode[0])
        {
            case EngParam::ENG_ISP_MODE_PROCESSED_RAW:
                //engParam.mi4EngIspMode = EngParam::ENG_RAW_TYPE_PROCESSED_RAW;
                mEngParam.mi4EngIspMode = EngParam::ENG_RAW_TYPE_FORCE_PROCESSED_RAW;
                break;
            case EngParam::ENG_ISP_MODE_PURE_RAW:
                mEngParam.mi4EngIspMode = EngParam::ENG_RAW_TYPE_PURE_RAW;

                if (pParamsMgr->getStr(MtkCameraParameters::KEY_MFB_MODE) == MtkCameraParameters::KEY_MFB_MODE_MFLL)
                {
                    pParamsMgr->set(MtkCameraParameters::KEY_ISP_MODE, EngParam::ENG_ISP_MODE_PROCESSED_RAW - '0');
                    mEngParam.mi4EngIspMode = EngParam::ENG_RAW_TYPE_PROCESSED_RAW;
                    MY_LOGW("MFLL forces to process raw from pure raw");
                }
                break;
        }
        MY_LOGD("mEngParam.mi4EngIspMode = %d", mEngParam.mi4EngIspMode);

        mEngParam.u4VHDState = pParamsMgr->getVHdr();
        MY_LOGD("u4VHDState = %d", mEngParam.u4VHDState);
        ...
        switch (atoi(strSaveMode))
        {
            case 1: // 1: "Preview Mode",
                mEngParam.mi4EngSensorMode = EngParam::ENG_SENSOR_MODE_NORMAL_PREVIEW;
                mEngParam.mi4EngRawSaveEn = 1;
                break;
            case 2: // 2: "Capture Mode",
                mEngParam.mi4EngSensorMode = EngParam::ENG_SENSOR_MODE_NORMAL_CAPTURE;
                mEngParam.mi4EngRawSaveEn = 1;
                break;
            case 4: // 4: "Video Preview Mode"
                mEngParam.mi4EngSensorMode = EngParam::ENG_SENSOR_MODE_NORMAL_VIDEO;
                mEngParam.mi4EngRawSaveEn = 1;
                break;
        ...
        MY_LOGD("mi4EngRawSaveEn, mi4EngSensorMode =(%d, %d)", mEngParam.mi4EngRawSaveEn, mEngParam.mi4EngSensorMode);
        ...
        else
        {
            ...
            // ShotParam shotParam;
            mEngParam.miPictureFormat           = NSCam::Utils::Format::queryImageFormat(pParamsMgr->getStr(CameraParameters::KEY_PICTURE_FORMAT));
            if  ( 0 != iPictureWidth && 0 != iPictureHeight )
            {
                mEngParam.mi4PictureWidth       = iPictureWidth;
                mEngParam.mi4PictureHeight      = iPictureHeight;
            }
            else
            {   //  When 3rd-party apk sets "picture-size=0x0", replace it with "preview-size".
                mEngParam.mi4PictureWidth       = iPreviewWidth;
                mEngParam.mi4PictureHeight      = iPreviewHeight;
            }
            mEngParam.miPostviewDisplayFormat   = NSCam::Utils::Format::queryImageFormat(s8DisplayFormat.string());
            mEngParam.miPostviewClientFormat    = NSCam::Utils::Format::queryImageFormat(pParamsMgr->getStr(CameraParameters::KEY_PREVIEW_FORMAT));
            mEngParam.mi4PostviewWidth          = iPreviewWidth;
            mEngParam.mi4PostviewHeight         = iPreviewHeight;
            mEngParam.ms8ShotFileName           = pParamsMgr->getStr(MtkCameraParameters::KEY_CAPTURE_PATH);
            mEngParam.mu4ZoomRatio              = pParamsMgr->getZoomRatio();
            mEngParam.mu4ShotCount              = pParamsMgr->getInt(MtkCameraParameters::KEY_BURST_SHOT_NUM);
            mEngParam.mu4Transform              = transform;
            mEngParam.mu4MultiFrameBlending     = pParamsMgr->getMultFrameBlending();
            //
            ...
            ret =
                ...
                &&  pShot->sendCommand(eCmd_setShotParam, (MUINTPTR)&mEngParam, sizeof(EngParam))
    ...
    int flashCaliEn = mspParamsMgr->getInt(MtkCameraParameters::KEY_ENG_FLASH_CALIBRATION);
    if (flashCaliEn == 1)
    {
