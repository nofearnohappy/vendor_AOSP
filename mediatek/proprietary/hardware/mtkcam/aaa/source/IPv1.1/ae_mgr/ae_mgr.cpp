...
MBOOL AeMgr::setSensorMode(MINT32 i4NewSensorMode , MUINT32 width, MUINT32 height)
{
    ...
    if (m_eSensorMode != eNewSensorMode) {
        MY_LOG("m_eSensorMode/Hbinwidth/Hbinheight: %d/%d/%d -> %d/%d/%d\n", m_eSensorMode,m_u4Hbinwidth,m_u4Hbinheight,eNewSensorMode,width,height );
        m_u4Hbinwidth = width;
...
//    MY_LOG("%s() : AEMode:%d AEState:%d i4ActiveAEItem:%d u4AAOUpdate:%d\n", __FUNCTION__, m_eAEMode, m_eAEState, i4ActiveAEItem, u4AAOUpdate);
    if(m_eAETargetMode == AE_MODE_MVHDR_TARGET) {
        ...
        aaoSeparation(pAEStatBuf,AAO_STT_BLOCK_NUMX, AAO_STT_BLOCK_NUMY,
                      m_pAAOSeparatedBuf,
                      (m_pAAOSeparatedBuf+AAO_SEP_AWB_SIZE),
                      (m_pAAOSeparatedBuf+AAO_SEP_AWB_SIZE+AAO_SEP_AE_SIZE),
                      (m_pAAOSeparatedBuf+AAO_SEP_AWB_SIZE+AAO_SEP_AE_SIZE+AAO_SEP_HIST_SIZE));
...
MRESULT AeMgr::AEInit(Param_T &rParam)
{
    ...
    if(m_pAAOSeparatedBuf== NULL) {
        m_pAAOSeparatedBuf = (MVOID *)malloc(AAO_SEP_AWB_SIZE+AAO_SEP_AE_SIZE+AAO_SEP_HIST_SIZE+AAO_SEP_OVEREXPCNT_SIZE);
        if(m_pAAOSeparatedBuf == NULL) {
