...
init(MINT32 i4SensorDevId, MINT32 i4SensorOpenIndex) //can be called only once by RAW or YUV, no user count
{
    ...
    //IEventIrq::ConfigParam IrqConfig(i4SensorDevId, i4SensorOpenIndex, 5000, IEventIrq::E_Event_Vsync);
    ...
    mpHal3ASttCtrl = Hal3ASttCtrl::createInstance(i4SensorDevId, i4SensorOpenIndex);
#endif
...
#if (CAM3_3ATESTLVL > CAM3_3AUT)
    mpHal3ASttCtrl->destroyInstance();
#endif
...
MBOOL Hal3AFlowCtrl::postCommand(ECmd_T const eCmd, MINTPTR const i4Arg)
{
    ...
    if (eCmd == ECmd_CameraPreviewStart)
    {
        IEventIrq::ConfigParam IrqConfig(m_i4SensorDev, m_i4SensorOpenIdx, 5000, IEventIrq::E_Event_Vsync);
        mpIEventIrq = IEventIrq::createInstance(IrqConfig, "VSIrq");
        mpHal3ASttCtrl->initStt(m_i4SensorDev, m_i4SensorOpenIdx);
        mpHal3ASttCtrl->startStt();
    }
    ...
    {
        mpHal3ASttCtrl->stopStt();
        mpHal3ASttCtrl->uninitStt();
    }
