...
init()
{
    MY_LOG("[%s] Hal3AAdapter3", __FUNCTION__);
...
_reprocess(const MetaSet_T& control, NSIspTuning::RAWIspCamInfo& output)
{
    ...
    {
        output.rCropRzInfo.i4OfstX    = rP1Crop.p.x;
        output.rCropRzInfo.i4OfstY    = rP1Crop.p.y;
        output.rCropRzInfo.i4Width    = rP1Crop.s.w;
        output.rCropRzInfo.i4Height   = rP1Crop.s.h;
        output.rCropRzInfo.i4RzWidth  = rRzSize.w;
        output.rCropRzInfo.i4RzHeight = rRzSize.h;
        output.rCropRzInfo.fgOnOff    = MTRUE;
    }
    else
    {
        output.rCropRzInfo.fgOnOff = MFALSE;
    }

    MUINT8 u1EdgeMode;    
    if (QUERY_ENTRY_SINGLE(control.appMeta, MTK_EDGE_MODE, u1EdgeMode))
    {
        output.eEdgeMode = (mtk_camera_metadata_enum_android_edge_mode_t)u1EdgeMode;
    }

    MUINT8 u1NrMode;    
    if (QUERY_ENTRY_SINGLE(control.appMeta, MTK_NOISE_REDUCTION_MODE, u1NrMode))
    {
        output.eNRMode = (mtk_camera_metadata_enum_android_noise_reduction_mode_t)u1NrMode;
    }
}

static
inline MVOID
_reprocResult(const NSIspTuning::RAWIspCamInfo& rCamInfo, MetaSet_T* pResult)
{
    if (pResult == NULL)
        return;
    
    MUINT8 u1EdgeMode = rCamInfo.eEdgeMode;
    UPDATE_ENTRY_SINGLE(pResult->appMeta, MTK_EDGE_MODE, u1EdgeMode);

    MUINT8 u1NrMode = rCamInfo.eNRMode;
...
{
    MINT32 i4P2En = 0;
    GET_PROP("debug.hal3av3.p2", "0", i4P2En);

if (i4P2En == 0)
{
    MY_LOG("[%s] IT: flowType(%d), pRegBuf(%p)", __FUNCTION__, flowType, pRegBuf);
    return -1;
}
else
{
...
*/
        _reprocResult(rRawInfo, pResult);
    }
