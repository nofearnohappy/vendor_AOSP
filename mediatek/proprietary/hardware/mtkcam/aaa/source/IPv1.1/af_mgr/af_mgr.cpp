...
MBOOL AfMgr::Start()
{
  ...
  // Checking sensor mode.
    MY_LOG("[AF][%15s] SensorDev %d, TGSZ:[W]%d,[H]%d, BINSZ:[W]%d,[H]%d\n", __FUNCTION__, m_i4CurrSensorDev, m_i4TGSzW,  m_i4TGSzH, m_i4BINSzW, m_i4BINSzH);
  ...
  if( m_PDBuf_Type==EPDBuf_NotDef)
  {
    ...
    m_pIAfAlgo->setAFParam(m_sAFParam, getAFConfig(), m_NVRAM_LENS.rAFNVRAM);
  ...
  //-----------
    ...
    cfgHWReg( m_sAFOutput.sAFStatConfig);
...
MRESULT AfMgr::setAFArea(CameraFocusArea_T a_sAFArea)
{
    ...
    else
    {
      m_sAFInput.sLaserInfo.i4AfWinPosCnt = 0;
        ...
        {
            a_sAFArea.rAreas[i].i4Left   = (a_sAFArea.rAreas[i].i4Left   +1000) * m_i4TGSzW / 2000;
            a_sAFArea.rAreas[i].i4Right  = (a_sAFArea.rAreas[i].i4Right  +1000) * m_i4TGSzW / 2000;
            a_sAFArea.rAreas[i].i4Top    = (a_sAFArea.rAreas[i].i4Top    +1000) * m_i4TGSzH / 2000;
            a_sAFArea.rAreas[i].i4Bottom = (a_sAFArea.rAreas[i].i4Bottom +1000) * m_i4TGSzH / 2000;
...
MVOID AfMgr::applyZoomInfo()
{
    ...
    {
        if (m_i4TGSzW != m_sEZoom.i4W)
            m_i4Factor = m_i4TGSzW * 100 / m_sEZoom.i4W;
        else
            m_i4Factor = 100;
    }
...
MRESULT AfMgr::setFDInfo(MVOID* a_sFaces)
{
  ...
  m_FDArea.i4Score=0; /*for algorithm to check input is FD or object, 0:face, 100:object*/
    ...
    sFDArea.rAreas[0].i4Left   = (pFaces->faces->rect[0]+1000) * m_i4TGSzW / 2000;
    sFDArea.rAreas[0].i4Right  = (pFaces->faces->rect[2]+1000) * m_i4TGSzW / 2000;
    sFDArea.rAreas[0].i4Top    = (pFaces->faces->rect[1]+1000) * m_i4TGSzH / 2000;
    sFDArea.rAreas[0].i4Bottom = (pFaces->faces->rect[3]+1000) * m_i4TGSzH / 2000;
...
MRESULT AfMgr::setOTInfo(MVOID* a_sOT)
{
    ...
    sOBTArea.rAreas[0].i4Left   = (pObtinfo->faces->rect[0]+1000) * m_i4TGSzW / 2000;
    sOBTArea.rAreas[0].i4Right  = (pObtinfo->faces->rect[2]+1000) * m_i4TGSzW / 2000;
    sOBTArea.rAreas[0].i4Top    = (pObtinfo->faces->rect[1]+1000) * m_i4TGSzH / 2000;
    sOBTArea.rAreas[0].i4Bottom = (pObtinfo->faces->rect[3]+1000) * m_i4TGSzH / 2000;
...
AF_FULL_STAT_T AfMgr::getFLKStat()
{
    ...
    m_sAFFullStat.bValid = MTRUE;
    // no use
    for (MINT32 i=0; i<(MAX_AF_HW_WIN-1); i++)
    {
        m_sAFFullStat.i8StatH[i] = 0;
        m_sAFFullStat.i8StatV[i] = 0;
    }

    MY_LOG_IF((m_bDebugEnable&1), "[%s] FLK_bValid %d\n", m_sAFFullStat.bValid);
...
MRESULT AfMgr::doPDTask()
{
...

  {
    ...
    if( m_pdaf_rawbuf)
    {
      ...
      SPDInputData_t PDInput( pdWinNum, m_i4TGSzH*m_i4TGSzW*10/8, reinterpret_cast<MUINT8 *>(m_pdaf_rawbuf));
...
MVOID AfMgr::setFlkWinConfig()
{
    ...
    if(m_bDebugAFwinW)   {    m_i4TGSzW=m_bDebugAFwinW;}
    if(m_bDebugAFwinH)   {    m_i4TGSzH=m_bDebugAFwinH;}
    MINT32 i4WOri = Boundary((WIN_SIZE_MIN*6 +WIN_POS_MIN*2),m_i4TGSzW, (WIN_SIZE_MAX*6+WIN_POS_MIN*2));
    MINT32 i4HOri = Boundary((WIN_SIZE_MIN*6 +WIN_POS_MIN*2),m_i4TGSzH, (WIN_SIZE_MAX*6+WIN_POS_MIN*2));
...
MVOID AfMgr::getTGSZ()
{
    ...
    MUINTPTR handle;
    if (MFALSE==sendAFNormalPipe(NSImageio::NSIspio::EPIPECmd_GET_TG_OUT_SIZE, m_i4SensorIdx, (MINTPTR)(&m_i4TGSzW), (MINTPTR)(&m_i4TGSzH)))
        MY_ERR("EPIPECmd_GET_TG_OUT_SIZE fail");

    MY_LOG_IF((m_bDebugEnable&1),"[setAF_IN_HSIZE][SensorDev]%d [H]%d [V]%d\n", m_i4CurrSensorDev, m_i4TGSzW,  m_i4TGSzH);
    ...
    if(m_eSensorTG ==ESensorTG_1)
    {
        ...
        {
            IOPIPE_SET_MODUL_REG(handle,  CAM_AF_IMAGE_SIZE, m_i4TGSzW);
            if (MFALSE==sendAFNormalPipe(NSImageio::NSIspio::EPIPECmd_SET_MODULE_CFG_DONE, handle, MNULL, MNULL))
    ...
    else
    {
        ...
        {
            IOPIPE_SET_MODUL_REG(handle,  CAM_AF_D_IMAGE_SIZE, m_i4TGSzW);
            if (MFALSE==sendAFNormalPipe(NSImageio::NSIspio::EPIPECmd_SET_MODULE_CFG_DONE, handle, MNULL, MNULL))
                    MY_ERR("EPIPECmd_SET_MODULE_CFG_DONE setAF_IN_HSIZE fail");
        }
...
MBOOL AfMgr::setSensorMode(MINT32 i4NewSensorMode, MUINT32 i4BINInfo_SzW, MUINT32 i4BINInfo_SzH)
{
  ...
  MY_LOG("cmd-%s DEV(%d) : %d, BINSZ(%d, %d)\n", __FUNCTION__, m_i4CurrSensorDev, m_i4SensorMode);
    return MTRUE;
}
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT AfMgr::cfgHWReg( AF_CONFIG_T &sAFHWCfg)
{
    // workaround for iVHDR
    sAFHWCfg.i4SGG_GAIN = (m_AEsetPGN!=sAFHWCfg.i4SGG_GAIN) ? m_AEsetPGN : sAFHWCfg.i4SGG_GAIN;
    ...
    ISP_MGR_AF_STAT_CONFIG_T::getInstance(static_cast<ESensorDev_T>(m_i4CurrSensorDev)).config(testcfg);
