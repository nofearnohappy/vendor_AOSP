...
MBOOL VHdrDrvImp::SaveToFile(char const *filepath,const MUINTPTR &addr)
{
    ...
    VHDR_LOG("[1-th plane] write %d bytes to %s",size,filepath);
...
void VHdrP1Cb::p1TuningNotify(MVOID *pInput,MVOID *pOutput)
{
    ...
    //No need to handle crop beacuse we only support EIS1.2 if user turn on ivHDR
    //crop is only affected by EIS2.0. Supported combination is EIS1.2+ivHDR.
    {
        //> LCS_ST
