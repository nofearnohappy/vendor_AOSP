...
#define DF_LOGD(fmt, arg...)    ALOGD("[%s]" fmt, __func__, ##arg)
#define DF_LOGI(fmt, arg...)    ALOGI("[%s]" fmt, __func__, ##arg)
#define DF_LOGW(fmt, arg...)    ALOGW("[%s] WRN(%5d):" fmt, __func__, __LINE__, ##arg)
#define DF_LOGE(fmt, arg...)    ALOGE("[%s] %s ERROR(%5d):" fmt, __func__,__FILE__, __LINE__, ##arg)
...
StereoDepthHal::StereoDepthRunLearning(MUINT16 u2NumHwfeMatch, MFLOAT *pfHwfeMatchData, P_AF_WIN_COORDINATE_STRUCT pstAfWinCoordinate)
{
    MBOOL Result = MTRUE;
    MINT32 err = 0; // 0: no error. other value: error.
    DF_LOGD("+. curr_p2_frm_num: %d (DafTbl CurrIdx: %d). u2NumHwfeMatch: %d. pfHwfeMatchData: %p (value: %f). sxsyexey(%d,%d,%d,%d)",
...
StereoDepthHal::StereoDepthRunQuerying(MUINT16 u2NumHwfeMatch, MFLOAT *pfHwfeMatchData, P_AF_WIN_COORDINATE_STRUCT pstAfWinCoordinate)
{
    MBOOL Result = MTRUE;
    MINT32 err = 0; // 0: no error. other value: error.
    DF_LOGD("+. curr_p2_frm_num: %d (Idx: %d). is_query_happen: %d (Idx: %d). u2NumHwfeMatch: %d. pfHwfeMatchData: %p (value: %f). sxsyexey(%d,%d,%d,%d)",
        m_prDafTbl->curr_p2_frm_num,
        m_prDafTbl->curr_p2_frm_num % DAF_TBL_QLEN,
...
//    gAF_Info[6] = (MFLOAT) m_prDafTbl->daf_vec[u4CurrIdx].af_win_end_y;   // gAF_end_y.       // get original value from DAF_TBL, should be remapped
    ...
    {
        DF_LOGD("input: %d, result: %d, AF: %f; distance: %d cm (valid: %.3f)",
            0/*input_dac*/,
