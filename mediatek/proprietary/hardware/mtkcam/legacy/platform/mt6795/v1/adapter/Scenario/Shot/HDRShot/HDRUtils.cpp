...
getCaptureInfo(MUINT32 sensorId, std::vector<NS3A::CaptureParam_T> & vCap3AParam, MUINT32 &hdrFrameNum)
{
    ...
    if(rSensorInfo.sensorType == SENSOR_TYPE_YUV) {
        ...
        MY_LOGD(" send3ACtrl bDetectFace(%d)", strHDRInputSetting.bDetectFace);
        MY_LOGD(" send3ACtrl E3ACtrl_GetCapPLineTable aeTableCurrentIndex(%d)", strHDRInputSetting.i4aeTableCurrentIndex);
        MY_LOGD(" send3ACtrl E3ACtrl_GetCapPLineTable u4TotalIndex(%d)", strHDRInputSetting.PLineAETable.u4TotalIndex);
        MY_LOGD(" send3ACtrl E3ACtrl_GetCapPLineTable eID(%d)", strHDRInputSetting.PLineAETable.eID);
        p3AHal->destroyInstance(LOG_TAG);
#endif
