...
#define MY_LOGA(fmt, arg...)        ALOGE("[%s] " fmt, __FUNCTION__, ##arg)
...
{
    MY_LOGD_IF((0<gLogLevel), "enter queryStaticInfo");
    struct Extra {
        int     format;
        int     usage;
...
#define MY_QUERY(_extra_get_, _field_) \
    ...
    err |= ::gralloc_extra_query(bh, GRALLOC_EXTRA_GET_##_extra_get_, &extra._field_)
        ...
        MY_LOGD_IF((0<gLogLevel), "rowStrideInPixels:%d", extra.rowStrideInPixels);
        //
#undef  MY_QUERY
        //
    ...
    //
    MY_LOGD_IF((0<gLogLevel), "info.format:%d", info.format);
    ...
    case HAL_PIXEL_FORMAT_YCbCr_422_I:
        {
            MY_LOGD_IF((0<gLogLevel), "HAL_PIXEL_FORMAT_YCbCr_422_I");
            info.planes.resize(1);
            //
            {
                MyStaticInfo::Plane& plane = info.planes.editItemAt(0);
                ...
                plane.sizeInBytes = info.allocSize;
                MY_LOGD_IF((0<gLogLevel), "rowStrideInBytes:%d", plane.rowStrideInBytes);
                //
                checkPlaneSize(info, 0, (extra.rowNum));
    ...
    case HAL_PIXEL_FORMAT_YCrCb_420_SP:
        {
            MY_LOGD_IF((0<gLogLevel), "HAL_PIXEL_FORMAT_YCrCb_420_SP");
            struct android_ycbcr ycbcr;
            if  ( OK != gQueryYCbCr(ycbcr, bh, info.widthInPixels, info.heightInPixels) ) {
                return UNKNOWN_ERROR;
            }
            MY_LOGD_IF((0<gLogLevel), "0 rowStrideInBytes:%d", ycbcr.ystride);
            MY_LOGD_IF((0<gLogLevel), "1 rowStrideInBytes:%d", ycbcr.cstride);
            MY_LOGD_IF((0<gLogLevel), "info.widthInPixels:%d, info.heightInPixels:%d", info.widthInPixels, info.heightInPixels);
            if(ycbcr.ystride == 0){
                ycbcr.ystride = info.widthInPixels;
                MY_LOGW("Force ystride not 0, set to %d", info.widthInPixels);
            }
            if(ycbcr.cstride == 0){
                ycbcr.cstride = info.widthInPixels;
                MY_LOGW("Force cstride not 0, set to %d", info.widthInPixels);
            }
            //
            ...
            {// Y
                ...
                if(plane.sizeInBytes == 0)
                {
                    plane.sizeInBytes = info.widthInPixels * info.heightInPixels;
                    MY_LOGW("force size plane.sizeInBytes:%d", plane.sizeInBytes);
                }
                //
                checkPlaneSize(info, 0, (extra.rowNum));
            }
            {// CrCb
                ...
                MY_LOGD_IF((0<gLogLevel), "1 rowStrideInBytes:%d", plane.rowStrideInBytes);
    ...
    case HAL_PIXEL_FORMAT_YV12:
        {
            ...
            MY_LOGD_IF((0<gLogLevel), "HAL_PIXEL_FORMAT_YV12");
            //
