...
void AudioBTCVSDControl::BT_SCO_TX_Open(void)
{
    ...
    strftime(mDumpFileName, 128, "/sdcard/mtklog/audio_dump/CVSDTXOut_%Y_%m_%d_%H%M%S.pcm", timeinfo);
    mTXSRCPCMDumpFile = NULL;
...
void AudioBTCVSDControl::BT_SCO_RX_Open(void)
{
    ...
    struct tm *timeinfo = localtime(&rawtime);
    char mDumpFileName[128];
	strftime(mDumpFileName, 128, "/sdcard/mtklog/audio_dump/CVSDRXOut_%Y_%m_%d_%H%M%S.pcm", timeinfo);
