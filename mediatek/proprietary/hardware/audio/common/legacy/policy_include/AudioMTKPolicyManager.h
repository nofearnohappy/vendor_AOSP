...
class AudioMTKPolicyManager: public AudioMTKPolicyInterface
...
public:
        ...
        status_t checkAndSetAnalogGain(int stream, int index, audio_io_handle_t output,
                                          audio_devices_t device, int delayMs = 0, bool force = false );
        int selectActiveStream(int requestStream);
        float computeCustomVolume(int stream, int index, audio_io_handle_t output,audio_devices_t device);
private:
       int                 mVolumeStream;
       int                 mVolumeIndex;
