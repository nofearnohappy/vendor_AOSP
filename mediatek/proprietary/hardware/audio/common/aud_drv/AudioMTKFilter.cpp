...
void AudioMTKFilter::start(bool bFirstDataWrite)
{
    ...
    if (mFilter && !mActive)
    {
        SLOGD("AudioMTKFilter::start() type %d mode %d bFirstDataWrite %d", mType, mMode,bFirstDataWrite);
...
void AudioMTKFilter::stop()
{
    ...
    if (mFilter && mActive)
    {
        SLOGD("AudioMTKFilter::stop() type %d mode %d", mType, mMode);
        //mFilter->Stop();
        mFilter->ResetBuffer();
        mFilter->Close();
        mStart  = false;
...
void AudioMTKFilter::pause()
{
    ...
    if (mFilter && mActive)
    {
        SLOGD("AudioMTKFilter::pause() type %d mode %d", mType, mMode);
...
void AudioMTKFilter::resume()
{
    ...
    if (mFilter && !mActive)
    {
        SLOGD("AudioMTKFilter::resume() type %d mode %d", mType, mMode);
...
void AudioMTKFilter::setParameter(void *param)
{
    ...
    if (mFilter)
    {
        SLOGD("AudioMTKFilter::setParameter type %d mode %d mActive %d", mType, mMode,mActive);
        mFilter->ResetBuffer();
        mFilter->Close();
        mFilter->SetParameter(BLOUD_PAR_SET_CHANNEL_NUMBER, (void *)((long)mChannel));
        mFilter->SetParameter(BLOUD_PAR_SET_SAMPLE_RATE, (void *)((long)mSampleTate));
...
void AudioMTKFilter::setParameter2Sub(void *param)
{
    ...
    if (mFilter)
    {
        SLOGV("AudioMTKFilter::setParameter2Sub type %d mode %d mActive %d", mType, mMode,mActive);
        mFilter->ResetBuffer();
        mFilter->Close();
        mFilter->SetParameter(BLOUD_PAR_SET_CHANNEL_NUMBER, (void *)((long)mChannel));
        mFilter->SetParameter(BLOUD_PAR_SET_SAMPLE_RATE, (void *)((long)mSampleTate));
...
#else
    SLOGD("UnSupport Stereo Speaker.");
#endif
}
...
//#endif
    ...
    SLOGD("init() fixedParam %d", mFixedParam);
...
void AudioMTKFilterManager::start(bool bFirstDataWrite)
{
    ...
    SLOGV("start() device 0x%x", device);
#endif
...
#endif
#else
    SLOGW("Unsupport AudEnh Feature");
#endif
}
...
#endif
#if defined(CONFIG_MT_ENG_BUILD) 
    SLOGV("-process() outsize %u", outputSize);
#endif
    return outputSize;
...
{
    SLOGV("setParameter() type %u", type);
    ...
    else
    {
        ...
        SLOGW("MTKFilter doesn't process FLDID [%d]",type);
        return;
    }
}
...
{
    SLOGV("setSpkMntrGain() target gain %d, sample %d", gain, ramp_sample_cnt);
...
//        memcpy(&cali_param.bes_loudness_bpf_coeff_L, &AUD_VIBR_FILTER_COEF_Table[(VIBSPK_DEFAULT_FREQ - VIBSPK_FREQ_LOWBOUND + 1) / VIBSPK_FILTER_FREQSTEP], sizeof(uint32_t)*VIBSPK_AUD_PARAM_SIZE);
        ...
        SetAudioCompFltCustParamToNV(AUDIO_COMP_FLT_VIBSPK, pCali_param);
